...
void Read_parameters(SDDFT_OBJ* pSddft)
{
  ...
  FILE *fConfFile;
  PetscInt p,a;   
  PetscScalar delta_x,delta_y,delta_z;
  char ConfigFile[100]="./";
  ...
  pSddft->PseudochargeRadiusTOL=1e-6; // default tolerance for pseudocharge density radius
  ...
  strcpy(pSddft->XC,"LDA"); // default exchange correlation=lda
  pSddft->BC=1; // default is Dirichlet boundary condition
  pSddft->Nkpts=1; // default number of k points of periodic calculation
  /*
  ...
  do 
      ...
      if(strcmp(str,"FD_ORDER:")==0)	
	{
	  ...
	  pSddft->order = (pSddft->order)/2; // store half order                                              
	...
	{
	  fscanf(fConfFile,"%d",&pSddft->Kx);
	  fscanf(fConfFile,"%d",&pSddft->Ky);
	  fscanf(fConfFile,"%d",&pSddft->Kz);
	  pSddft->Nkpts = pSddft->Kx*pSddft->Ky*pSddft->Kz;
	  /*
	   * currently only takes into account the time reversal symmetry
	   */
	  pSddft->Nkpts_sym = pSddft->Kx*pSddft->Ky*ceil(pSddft->Kz/2.0);
	}
...
void Read_ion(SDDFT_OBJ* pSddft)
{
  ...
  do 
	...
	{  
	  ...
	  VecSetValues(pSddft->Atompos,1,&index,&z0,INSERT_VALUES);index++;
	}
  ...
  if(at!=pSddft->Ntype)
    {
      cout << "Number of blocks: " << at << " in .ion file not same as number of atom type:" << pSddft->Ntype << "in .inpt file\n";
      exit(1);
    }
      ...
      do 
	{
	  ...
	  for(poscnt=0;poscnt<noa;poscnt++)
	    {  
	      ...
	      VecSetValues(pSddft->mvAtmConstraint,1,&index,&xct,INSERT_VALUES);index++;
	      VecSetValues(pSddft->mvAtmConstraint,1,&index,&yct,INSERT_VALUES);index++;
	      VecSetValues(pSddft->mvAtmConstraint,1,&index,&zct,INSERT_VALUES);index++;
	    }
	  at++;
	}while(!feof(fConfFile));
      fclose(fConfFile);
      ...
      if(at!=pSddft->Ntype)
	{
	  cout<< "Number of blocks: "<< at <<" in .relax file not same as number of atom type: "<< pSddft->Ntype << " in .inpt file\n";
	  exit(1);
	}
    }
...
void Read_pseudopotential(SDDFT_OBJ* pSddft)
{
  ...
  for(at=0;at<pSddft->Ntype;at++)
      ...
      do
	{
      ...
      count = count-1;         
      fclose(fRvsVFile);
      pSddft->psd[at].size = count;  // store the size of the pseudopotential list for future
