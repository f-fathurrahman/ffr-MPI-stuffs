  ...
  if (pSPARC->dmcomm_phi != MPI_COMM_NULL) {
    ...
    if (pSPARC->mGGAflag == 1) {
      ...
      if (pSPARC->spin_typ == 0)
        ...
        VectorDotProduct(pSPARC->KineticTauPhiDomain + pSPARC->Nd_d, pSPARC->vxcMGGA3, 2 * pSPARC->Nd_d, &Emgga,
                         pSPARC->dmcomm_phi);
      Emgga *= pSPARC->dV;
      E3 += Emgga;
    }
...
double Calculate_Eband(SPARC_OBJ *pSPARC) {
  ...
  if (pSPARC->isGammaPoint) { // for gamma-point systems
    for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
      for (n = 0; n < Ns; n++) {
        // Eband += 2.0 * smearing_FermiDirac(pSPARC->Beta, pSPARC->lambda[n],
        // pSPARC->Efermi) * pSPARC->lambda[n];
    ...
    for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
      for (k = 0; k < Nk; k++) {
        for (n = 0; n < Ns; n++) {
          // Eband += 2.0 * pSPARC->kptWts_loc[k] *
          // smearing_FermiDirac(pSPARC->Beta, pSPARC->lambda[n+k*Ns],
          // pSPARC->Efermi)
          //         * pSPARC->lambda[n+k*Ns];
          Eband += occfac * pSPARC->kptWts_loc[k] * pSPARC->occ[n + k * Ns + spn_i * Nk * Ns] *
                   pSPARC->lambda[n + k * Ns + spn_i * Nk * Ns];
        }
      }
    }
    Eband /= pSPARC->Nkpts;
...
double Calculate_electronicEntropy(SPARC_OBJ *pSPARC) {
  ...
  if (pSPARC->isGammaPoint) { // for gamma-point systems
    for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
      Entropy += Calculate_entropy_term(pSPARC->lambda + spn_i * Ns, pSPARC->occ + spn_i * Ns, pSPARC->Efermi, 0,
                                        Ns - 1, pSPARC->Beta, pSPARC->elec_T_type);
    }
    ...
    for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
      for (k = 0; k < pSPARC->Nkpts_kptcomm; k++) {
        double Entropy_k =
            Calculate_entropy_term(pSPARC->lambda + k * Ns + spn_i * Nk * Ns, pSPARC->occ + k * Ns + spn_i * Nk * Ns,
                                   pSPARC->Efermi, 0, Ns - 1, pSPARC->Beta, pSPARC->elec_T_type);
 ...
 *          of degree n, A_N is given in ref: M. Methfessel and A.T. Paxton
 * (1989). Note: when N = 0, MP is equivalent to gaussian smearing. Currently
 *          not implemented.
 *
 ...
 * @param type       Smearing type. type = 0: Fermi-Dirac, type = 1: Gassian
 * (MP0), type > 1: Methfessel-Paxton (N = type - 1).
 */
double Calculate_entropy_term(const double *lambda, const double *occ, const double lambda_f, const int n_start,
                              const int n_end, const double beta, const int type) {
  int n;
 ...
 */
double Calculate_Escc(SPARC_OBJ *pSPARC, const int DMnd, const double *Veff_out, const double *Veff_in,
