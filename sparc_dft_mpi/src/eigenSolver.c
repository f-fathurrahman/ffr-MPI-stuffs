 ...
 *          Alfredo Metere (GPU support), Lawrence Livermore National Laboratory <metere1@llnl.gov>,
 * <alfredo.metere@xsilico.com>
 *
 ...
 */
void init_guess_Lanczos(double *x0, double cellsizes[3], int gridsizes[3], double meshes[3], int DMVert[6],
                        int RandFlag, MPI_Comm comm) {
...
void eigSolve_CheFSI(int rank, SPARC_OBJ *pSPARC, int SCFcount, double error) {
  ...
  if (SCFcount == 0) {
    ...
    int RandFlag = (pSPARC->cell_typ != 0 || pSPARC->chefsibound_flag == 0 || pSPARC->chefsibound_flag == 1);
    // set up initial guess for Lanczos
    init_guess_Lanczos(x0, cellsizes, gridsizes, meshes, pSPARC->DMVertices_kptcomm, RandFlag, pSPARC->kptcomm_topo);
    t2 = MPI_Wtime();
#ifdef DEBUG
    if (!rank)
  ...
  if (pSPARC->CheFSI_Optmz) {
    ...
    if (error > 1e-2) {
      ...
      pSPARC->ChebDegree =
          pSPARC->npl_min + (int)((pSPARC->npl_max - pSPARC->npl_min) / (log_err0 - log_TOL) * (log(error) - log_TOL));
...
void CheFSI(SPARC_OBJ *pSPARC, double lambda_cutoff, double *x0, int count, int k, int spn_i) {
  ...
  Chebyshevfilter_constants(pSPARC, x0, &lambda_cutoff, &pSPARC->eigmin[spn_i], &pSPARC->eigmax[spn_i], count, k,
                            spn_i);
...
#ifdef USE_EVA_MODULE
  if (CheFSI_use_EVA == 1) {
    EVA_Chebyshev_Filtering(pSPARC, pSPARC->DMVertices_dmcomm, pSPARC->Nband_bandcomm, pSPARC->ChebDegree,
                            lambda_cutoff, pSPARC->eigmax[spn_i], pSPARC->eigmin[spn_i], pSPARC->dmcomm,
                            pSPARC->Xorb + spn_i * size_s, pSPARC->Yorb);
  } else {
#endif
#ifdef SPARCX_ACCEL
    if (pSPARC->useACCEL == 1 && pSPARC->cell_typ < 20 && pSPARC->usefock <= 1 && pSPARC->Nd_d_dmcomm == pSPARC->Nd) {
      ACCEL_ChebyshevFiltering(pSPARC, pSPARC->DMVertices_dmcomm, pSPARC->Xorb + spn_i * size_s, pSPARC->Yorb,
                               pSPARC->Nband_bandcomm, pSPARC->ChebDegree, lambda_cutoff, pSPARC->eigmax[spn_i],
                               pSPARC->eigmin[spn_i], k, spn_i, pSPARC->dmcomm);
    } else
#endif // SPARCX_ACCEL
    {
      ChebyshevFiltering(pSPARC, pSPARC->DMVertices_dmcomm, pSPARC->Xorb + spn_i * size_s, pSPARC->Yorb,
                         pSPARC->Nband_bandcomm, pSPARC->ChebDegree, lambda_cutoff, pSPARC->eigmax[spn_i],
                         pSPARC->eigmin[spn_i], k, spn_i, pSPARC->dmcomm, &t_temp);
    }
#ifdef USE_EVA_MODULE
  }
#endif
...
// ** calculate projected Hamiltonian and overlap matrix ** //
  ...
  if (pSPARC->StandardEigenFlag == 1) {
    DP_Project_Hamiltonian_std(pSPARC, pSPARC->DMVertices_dmcomm, pSPARC->Yorb, spn_i);
  } else {
    DP_Project_Hamiltonian(pSPARC, pSPARC->DMVertices_dmcomm, pSPARC->Yorb, pSPARC->Hp, pSPARC->Mp, spn_i);
  }
#else
  ...
  Project_Hamiltonian(pSPARC, pSPARC->DMVertices_dmcomm, pSPARC->Yorb, pSPARC->Hp, pSPARC->Mp, k, spn_i,
                      pSPARC->dmcomm);
#endif
  t2 = MPI_Wtime();
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("Total time for projection: %.3f ms\n", (t2 - t1) * 1e3);
#endif
...
// SPARCX_ACCEL_NOTE Need to add this to propagate GPU calculated eigenvalues back to the other MPI tasks
  ...
  if (pSPARC->useLAPACK == 1 && nproc_kptcomm > 1) {
    MPI_Bcast(pSPARC->lambda, pSPARC->Nstates * pSPARC->Nspin_spincomm, MPI_DOUBLE, 0,
              pSPARC->kptcomm); // TODO: bcast in blacscomm if possible
  }
#endif // SPARCX_ACCEL
  ...
  if (!rank && spn_i == 0) {
    ...
    printf("    first calculated eigval = %.15f\n"
           ...
           pSPARC->lambda_sorted[0], pSPARC->lambda_sorted[pSPARC->Nstates - 1]);
...
// ** subspace rotation ** //
  ...
  // find Y * Q, store the result in Xorb (band+domain) and Xorb_BLCYC (block cyclic format)
  Subspace_Rotation(pSPARC, pSPARC->Yorb_BLCYC, pSPARC->Q, pSPARC->Xorb_BLCYC, pSPARC->Xorb + spn_i * size_s, k, spn_i);
  if (pSPARC->npband > 1) {
...
#ifdef DEBUG
  if (!rank)
    printf("Total time for subspace rotation: %.3f ms\n", (t2 - t1) * 1e3);
#endif
}
...
void Solve_standard_EigenProblem(SPARC_OBJ *pSPARC, int k, int spn_i) {
...
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("Start solving standard eigenvalue problem ...\n");
#endif
  ...
  if (pSPARC->useACCEL == 1) {
    int info = 0;
    t1 = MPI_Wtime();
    if ((!pSPARC->is_domain_uniform && !pSPARC->bandcomm_index) || (pSPARC->is_domain_uniform && !rank_kptcomm)) {
      info = DSYEV(LAPACK_COL_MAJOR, 'V', 'U', pSPARC->Nstates, pSPARC->Hp, pSPARC->Nstates,
    ...
    pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, pSPARC->Hp, &ONE, &ONE, pSPARC->desc_Hp_BLCYC, pSPARC->Q, &ONE, &ONE,
  ...
  {    // SPARCX_ACCEL_NOTE Enclose the if block below and locate at bottom
    if (pSPARC->useLAPACK == 1) {
      ...
      t1 = MPI_Wtime();
      if ((!pSPARC->is_domain_uniform && !pSPARC->bandcomm_index) || (pSPARC->is_domain_uniform && !rank_kptcomm)) {
        info = LAPACKE_dsyevd(LAPACK_COL_MAJOR, 'V', 'U', pSPARC->Nstates, pSPARC->Hp, pSPARC->Nstates,
                              pSPARC->lambda + spn_i * pSPARC->Nstates);
      }
      t2 = MPI_Wtime();
#ifdef DEBUG
      ...
      t1 = MPI_Wtime();
      // distribute eigenvectors to block cyclic format
      pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, pSPARC->Hp, &ONE, &ONE, pSPARC->desc_Hp_BLCYC, pSPARC->Q, &ONE,
                &ONE, pSPARC->desc_Q_BLCYC, &pSPARC->ictxt_blacs_topo);
      t2 = MPI_Wtime();
#ifdef DEBUG
      if (!rank_spincomm && spn_i == 0) {
        printf("==standard eigenproblem: "
      ...
      t1 = MPI_Wtime();
      automem_pdsyev_("V", "U", &N, pSPARC->Hp, &ONE, &ONE, pSPARC->desc_Hp_BLCYC, pSPARC->lambda + spn_i * N,
                      pSPARC->Q, &ONE, &ONE, pSPARC->desc_Q_BLCYC, &info);
  ...
  double et = MPI_Wtime();
  if (rank == 0)
    printf("rank = %d, Solve_standard_EigenProblem used %.3lf ms\n", rank, 1000.0 * (et - st));
#endif
  ...
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  if (rank == 0)
    printf("[FATAL] Subspace eigenproblem are using ScaLAPACK routines but ScaLAPACK is not compiled\n");
  if (rank == 0)
 ...
 */
void Chebyshevfilter_constants(SPARC_OBJ *pSPARC, double *x0, double *lambda_cutoff, double *eigmin, double *eigmax,
  ...
  if (count == 0) {
    ...
    if (pSPARC->chefsibound_flag == 0 || pSPARC->chefsibound_flag == 1) { // 0 - default, 1 - always call Lanczos on H
      ...
      if (pSPARC->spin_typ == 0 && pSPARC->is_phi_eq_kpt_topo) {
        Lanczos(pSPARC, pSPARC->DMVertices_kptcomm, pSPARC->Veff_loc_dmcomm_phi, pSPARC->Atom_Influence_nloc_kptcomm,
                pSPARC->nlocProj_kptcomm, eigmin, eigmax, x0, pSPARC->TOL_LANCZOS, pSPARC->TOL_LANCZOS, 1000, k, spn_i,
        ...
        *eigmax *= 1.01; // add 1% buffer
        ...
        D2D(&pSPARC->d2d_dmcomm_lanczos, &pSPARC->d2d_kptcomm_topo, gridsizes, pSPARC->DMVertices_dmcomm,
            pSPARC->Veff_loc_dmcomm + sg * pSPARC->Nd_d_dmcomm, pSPARC->DMVertices_kptcomm,
            pSPARC->Veff_loc_kptcomm_topo, pSPARC->bandcomm_index == 0 ? pSPARC->dmcomm : MPI_COMM_NULL, sdims,
            pSPARC->kptcomm_topo, rdims, pSPARC->kptcomm);
        // If exchange-correlation is SCAN, GGA_PBE will be the exc used in 1st SCF; it is unnecessary to transform a
        // zero vector
        Lanczos(pSPARC, pSPARC->DMVertices_kptcomm, pSPARC->Veff_loc_kptcomm_topo, pSPARC->Atom_Influence_nloc_kptcomm,
                pSPARC->nlocProj_kptcomm, eigmin, eigmax, x0, pSPARC->TOL_LANCZOS, pSPARC->TOL_LANCZOS, 1000, k, spn_i,
                pSPARC->kptcomm_topo, &pSPARC->req_veff_loc);
        *eigmax *= 1.01; // add 1% buffer
      }
      ...
      if (pSPARC->cell_typ == 0 && (pSPARC->BC == 2 || pSPARC->BC == 0)) {
        ...
        *eigmin = -2.0; // TODO: tune this value
        ...
        Lanczos_laplacian(pSPARC, pSPARC->DMVertices_kptcomm, &eigmin_lap, &temp, x0, pSPARC->TOL_LANCZOS, 1e10, 1000,
                          k, spn_i, pSPARC->kptcomm_topo);
    ...
    *eigmin = pSPARC->lambda_sorted[spn_i * pSPARC->Nstates]; // take previous eigmin
    ...
    if (pSPARC->chefsibound_flag == 1 ||
        ((count == pSPARC->rhoTrigger) &&
         (strcmpi(pSPARC->XC, "SCAN") == 0))) { // 1 - always call Lanczos on H; the other condition is for SCAN
      ...
      if (pSPARC->spin_typ == 0 && pSPARC->is_phi_eq_kpt_topo) {
        Lanczos(pSPARC, pSPARC->DMVertices_kptcomm, pSPARC->Veff_loc_dmcomm_phi, pSPARC->Atom_Influence_nloc_kptcomm,
                pSPARC->nlocProj_kptcomm, &temp, eigmax, x0, 1e10, pSPARC->TOL_LANCZOS, 1000, k, spn_i,
                pSPARC->kptcomm_topo, &pSPARC->req_veff_loc);
        // *eigmax += 10 * pSPARC->TOL_LANCZOS;
        *eigmax *= 1.01; // add 1% buffer
      } else {
        D2D(&pSPARC->d2d_dmcomm_lanczos, &pSPARC->d2d_kptcomm_topo, gridsizes, pSPARC->DMVertices_dmcomm,
            ...
            pSPARC->Veff_loc_kptcomm_topo, pSPARC->bandcomm_index == 0 ? pSPARC->dmcomm : MPI_COMM_NULL, sdims,
            pSPARC->kptcomm_topo, rdims, pSPARC->kptcomm);
        if (strcmpi(pSPARC->XC, "SCAN") ==
            0) { // transfer vxcMGGA3 of this spin to kptcomm, it is moved from file mgga/mgga.c to here.
          // printf("rank %d, joined SCAN Lanczos, pSPARC->countPotentialCalculate %d\n", rank,
          // pSPARC->countPotentialCalculate);
          D2D(&pSPARC->d2d_dmcomm_lanczos, &pSPARC->d2d_kptcomm_topo, gridsizes, pSPARC->DMVertices_dmcomm,
              pSPARC->vxcMGGA3_loc_dmcomm +
                  sg * pSPARC->Nd_d_dmcomm, // processors in dmcomm does not save vxcMGGA3 of both spins; they just
                                            // saved which their spincomm needs
              pSPARC->DMVertices_kptcomm, pSPARC->vxcMGGA3_loc_kptcomm,
              pSPARC->bandcomm_index == 0 ? pSPARC->dmcomm : MPI_COMM_NULL, sdims, pSPARC->kptcomm_topo, rdims,
              pSPARC->kptcomm);
        }
        Lanczos(pSPARC, pSPARC->DMVertices_kptcomm, pSPARC->Veff_loc_kptcomm_topo, pSPARC->Atom_Influence_nloc_kptcomm,
                pSPARC->nlocProj_kptcomm, &temp, eigmax, x0, 1e10, pSPARC->TOL_LANCZOS, 1000, k, spn_i,
 ...
 */
void ChebyshevFiltering(SPARC_OBJ *pSPARC, int *DMVertices, double *X, double *Y, int ncol, int m, double a, double b,
                        double a0, int k, int spn_i, MPI_Comm comm, double *time_info) {
  if (comm == MPI_COMM_NULL || pSPARC->bandcomm_index < 0)
    return;
...
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("Start Chebyshev filtering ... \n");
#endif
  ...
  *time_info = 0.0;
  ...
  DMnd =
      (1 - DMVertices[0] + DMVertices[1]) * (1 - DMVertices[2] + DMVertices[3]) * (1 - DMVertices[4] + DMVertices[5]);
  len_tot = DMnd * ncol;
  e = 0.5 * (b - a);
  c = 0.5 * (b + a);
  sigma = sigma1 = e / (a0 - c);
  ...
  // find Y = (H - c*I)X
  int sg = pSPARC->spin_start_indx + spn_i;
  Hamiltonian_vectors_mult(pSPARC, DMnd, DMVertices, pSPARC->Veff_loc_dmcomm + sg * pSPARC->Nd_d_dmcomm,
...
void init_DP_CheFSI(SPARC_OBJ *pSPARC) {
  ...
  MPI_Allgather(&pSPARC->band_start_indx, 1, MPI_INT, Ns_bp_displs, 1, MPI_INT, pSPARC->blacscomm);
  ...
  descinit_(DP_CheFSI->desc_Hp_local, &Ns_dp, &Ns_dp, &Ns_dp, &Ns_dp, &ZERO, &ZERO, &pSPARC->ictxt_blacs_topo, &Ns_dp,
            &info);
  assert(info == 0);
  descinit_(DP_CheFSI->desc_Mp_local, &Ns_dp, &Ns_dp, &Ns_dp, &Ns_dp, &ZERO, &ZERO, &pSPARC->ictxt_blacs_topo, &Ns_dp,
            &info);
  assert(info == 0);
  descinit_(DP_CheFSI->desc_eig_vecs, &Ns_dp, &Ns_dp, &Ns_dp, &Ns_dp, &ZERO, &ZERO, &pSPARC->ictxt_blacs_topo, &Ns_dp,
            &info);
  assert(info == 0);
#endif
...
void DP_Project_Hamiltonian(SPARC_OBJ *pSPARC, int *DMVertices, double *Y, double *Hp, double *Mp, int spn_i) {
...
#endif // SPARCX_ACCEL
  {
    Hamiltonian_vectors_mult(pSPARC, pSPARC->Nd_d_dmcomm, DMVertices, Veff_loc_sg, pSPARC->Atom_Influence_nloc,
                             pSPARC->nlocProj, pSPARC->Nband_bandcomm, 0.0, Y, HY, spn_i, pSPARC->dmcomm);
  }
  et = MPI_Wtime();
#ifdef DEBUG
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian, rank 0, calc HY used %.3lf ms\n", 1000.0 * (et - st));
#endif
  ...
  BP2DP(pSPARC->blacscomm, DP_CheFSI->nproc_row, DP_CheFSI->Nd_bp, DP_CheFSI->Ns_bp, DP_CheFSI->Nd_dp_displs,
        DP_CheFSI->bp2dp_sendcnts, DP_CheFSI->bp2dp_sdispls, DP_CheFSI->dp2bp_sendcnts, DP_CheFSI->dp2bp_sdispls,
...
#ifdef DEBUG
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian, rank 0, convert BP to DP used %.3lf ms\n", 1000.0 * (et - st));
#endif
...
// SPARCX_ACCEL_NOTE: ADD HERE THE MATRIX PROJECTION GPU STUFF
#ifdef SPARCX_ACCEL
  if (pSPARC->useACCEL == 1) {
    ACCEL_DGEMM(CblasColMajor, CblasTrans, CblasNoTrans, Ns_dp, Ns_dp, Nd_dp, 1.0, Y_dp, Nd_dp, Y_dp, Nd_dp, 0.0,
                Mp_local, Ns_dp);
    ACCEL_DGEMM(CblasColMajor, CblasTrans, CblasNoTrans, Ns_dp, Ns_dp, Nd_dp, 1.0, Y_dp, Nd_dp, HY_dp, Nd_dp, 0.0,
                Hp_local, Ns_dp);
  } else
#endif // SPARCX_ACCEL
  {    // Need to enclose CPU-only original statements between brackets
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, Ns_dp, Ns_dp, Nd_dp, 1.0, Y_dp, Nd_dp, Y_dp, Nd_dp, 0.0,
                Mp_local, Ns_dp);
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, Ns_dp, Ns_dp, Nd_dp, 1.0, Y_dp, Nd_dp, HY_dp, Nd_dp, 0.0,
                Hp_local, Ns_dp);
  } // End
  et = MPI_Wtime();
#ifdef DEBUG
  ...
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian, rank 0, local %s for Hp & Mp used %.3lf ms\n", STR_DGEMM, 1000.0 * (et - st));
#else
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian, rank 0, local cblas_dgemm for Hp & Mp used %.3lf ms\n", 1000.0 * (et - st));
#endif // SPARCX_ACCEL
#endif // DEBUG
  ...
  if (rank_kpt == 0) {
    MPI_Ireduce(MPI_IN_PLACE, Mp_local, Ns_dp_2, MPI_DOUBLE, MPI_SUM, 0, DP_CheFSI->kpt_comm, &req0);
    MPI_Ireduce(MPI_IN_PLACE, Hp_local, Ns_dp_2, MPI_DOUBLE, MPI_SUM, 0, DP_CheFSI->kpt_comm, &req1);
  } else {
    MPI_Ireduce(Mp_local, NULL, Ns_dp_2, MPI_DOUBLE, MPI_SUM, 0, DP_CheFSI->kpt_comm, &req0);
    MPI_Ireduce(Hp_local, NULL, Ns_dp_2, MPI_DOUBLE, MPI_SUM, 0, DP_CheFSI->kpt_comm, &req1);
  }
  MPI_Wait(&req0, &sta0);
  MPI_Wait(&req1, &sta1);
  et = MPI_Wtime();
...
void DP_Project_Hamiltonian_std(SPARC_OBJ *pSPARC, int *DMVertices, double *Y, int spn_i) {
...
#ifdef DEBUG
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian_std, rank 0, convert BP to DP (Y) used %.3lf ms\n", 1000.0 * (et - st));
#endif
...
#ifdef SPARCX_ACCEL
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian_std, rank 0, local %s for Mp used %.3lf ms\n", STR_DGEMM, 1000.0 * (et - st));
#else
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian_std, rank 0, local dgemm for Mp used %.3lf ms\n", 1000.0 * (et - st));
#endif // SPARCX_ACCEL
#endif // DEBUG

  st = MPI_Wtime();
  MPI_Iallreduce(MPI_IN_PLACE, Mp_local, Ns_dp_2, MPI_DOUBLE, MPI_SUM, DP_CheFSI->kpt_comm, &req0);
  MPI_Wait(&req0, &sta0);
  et = MPI_Wtime();
#ifdef DEBUG
  if (rank_kpt == 0 && spn_i == 0)
  ...
  cblas_dtrsm(CblasColMajor, CblasRight, CblasUpper, CblasNoTrans, CblasNonUnit, Nd_dp, Ns_dp, 1.0, Mp_local, Ns_dp,
              Y_dp, Nd_dp);
...
#endif
  st = MPI_Wtime();
  DP2BP(pSPARC->blacscomm, DP_CheFSI->nproc_row, DP_CheFSI->Nd_bp, DP_CheFSI->Ns_bp, DP_CheFSI->Nd_dp_displs,
        DP_CheFSI->bp2dp_sendcnts, DP_CheFSI->bp2dp_sdispls, DP_CheFSI->dp2bp_sendcnts, DP_CheFSI->dp2bp_sdispls,
  ...
  st = MPI_Wtime();
  Hamiltonian_vectors_mult(pSPARC, pSPARC->Nd_d_dmcomm, DMVertices, Veff_loc_sg, pSPARC->Atom_Influence_nloc,
                           pSPARC->nlocProj, pSPARC->Nband_bandcomm, 0.0, pSPARC->Xorb + spn_i * size_s, Y, spn_i,
                           pSPARC->dmcomm);
  et = MPI_Wtime();
#ifdef DEBUG
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian_std, rank 0, calc HY used %.3lf ms\n", 1000.0 * (et - st));
#endif
...
#ifdef DEBUG
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian_std, rank 0, convert BP to DP used %.3lf ms\n", 1000.0 * (et - st));
#endif
...
#ifdef SPARCX_ACCEL
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian_std, rank 0, local %s for Hp used %.3lf ms\n", STR_DGEMM, 1000.0 * (et - st));
#else
  if (rank_kpt == 0 && spn_i == 0)
    printf("DP_Project_Hamiltonian_std, rank 0, local dgemm for Hp used %.3lf ms\n", 1000.0 * (et - st));
#endif // SPARCX_ACCEL
#endif // DEBUG

  st = MPI_Wtime();
  MPI_Iallreduce(MPI_IN_PLACE, Hp_local, Ns_dp_2, MPI_DOUBLE, MPI_SUM, DP_CheFSI->kpt_comm, &req1);
  MPI_Wait(&req1, &sta1);
  et = MPI_Wtime();
#ifdef DEBUG
  if (rank_kpt == 0 && spn_i == 0)
  ...
  if (pSPARC->useACCEL == 1) {
    ...
    if (rank_kpt == 0) {
      ...
      if (pSPARC->StandardEigenFlag == 0)
        info = DSYGV(LAPACK_COL_MAJOR, 1, 'V', 'U', Ns_dp, Hp_local, Ns_dp, Mp_local, Ns_dp, eig_val);
      else
    ...
    if (pSPARC->StandardEigenFlag == 0) {
      if (rank_kpt == 0)
        printf("DP_Solve_Generalized_EigenProblem rank 0 used %.3lf ms, %s used %.3lf ms\n", 1000.0 * (et1 - st),
               STR_DSYGV, 1000.0 * (et0 - st));
    } else {
      if (rank_kpt == 0)
        printf("DP_Solve_Generalized_EigenProblem rank 0 used %.3lf ms, %s used %.3lf ms\n", 1000.0 * (et1 - st),
               STR_DSYEV, 1000.0 * (et0 - st));
    }
#endif
  } else
#endif // SPARCX_ACCEL
  {    // Enclose in brackets original CPU if(useLAPACK...) block and put it at the bottom
    ...
    if (pSPARC->useLAPACK == 1) {
      ...
      if (rank_kpt == 0) {
        ...
        int info = 0;
        if (pSPARC->StandardEigenFlag == 0)
          info = LAPACKE_dsygvd(LAPACK_COL_MAJOR, 1, 'V', 'U', Ns_dp, Hp_local, Ns_dp, Mp_local, Ns_dp, eig_val);
        else
      ...
      if (pSPARC->StandardEigenFlag == 0) {
        if (rank_kpt == 0)
          printf("DP_Solve_Generalized_EigenProblem rank 0 used %.3lf ms, LAPACKE_dsygvd used %.3lf ms\n",
                 1000.0 * (et1 - st), 1000.0 * (et0 - st));
      } else {
        if (rank_kpt == 0)
          printf("DP_Solve_Generalized_EigenProblem rank 0 used %.3lf ms, LAPACKE_dsyevd used %.3lf ms\n",
                 1000.0 * (et1 - st), 1000.0 * (et0 - st));
      }
#endif
    } else {
      ...
      if (rank_dmcomm == 0) {
        ...
        pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, DP_CheFSI->Hp_local, &ONE, &ONE, DP_CheFSI->desc_Hp_local,
                  pSPARC->Hp, &ONE, &ONE, pSPARC->desc_Hp_BLCYC, &pSPARC->ictxt_blacs_topo);
        pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, DP_CheFSI->Mp_local, &ONE, &ONE, DP_CheFSI->desc_Mp_local,
                  pSPARC->Mp, &ONE, &ONE, pSPARC->desc_Mp_BLCYC, &pSPARC->ictxt_blacs_topo);
        ...
        pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, pSPARC->Q, &ONE, &ONE, pSPARC->desc_Q_BLCYC, DP_CheFSI->eig_vecs,
                  &ONE, &ONE, DP_CheFSI->desc_eig_vecs, &pSPARC->ictxt_blacs_topo);
      }
      int Ns_dp = DP_CheFSI->Ns_dp;
      ...
      int rank_kpt = DP_CheFSI->rank_kpt;
      if (rank_kpt == 0)
        printf("[FATAL] Subspace eigenproblem should be solved using ScaLAPACK but ScaLAPACK is not compiled\n");
      exit(255);
#endif // #if defined(USE_MKL) || defined(USE_SCALAPACK)
    }
  }
...
#ifdef SPARCX_ACCEL
  if (pSPARC->useACCEL == 1) {
    ACCEL_DGEMM(CblasColMajor, CblasNoTrans, CblasNoTrans, Nd_dp, Ns_dp, Ns_dp, 1.0, Y_dp, Nd_dp, eig_vecs, Ns_dp, 0.0,
                YQ_dp, Nd_dp);
  } else
#endif // SPARCX_ACCEL
  {    // SPARCX_ACCEL_NOTE Brackets now needed to enclose the original CPU-only cblas call
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Nd_dp, Ns_dp, Ns_dp, 1.0, Y_dp, Nd_dp, eig_vecs, Ns_dp, 0.0,
                YQ_dp, Nd_dp);
...
#ifdef DEBUG
  if (rank_kpt == 0)
    printf("DP_Subspace_Rotation rank 0 used %.3lf ms, redist PsiQ used %.3lf ms\n\n", 1000.0 * (et1 - st),
           1000.0 * (et1 - et0));
#endif
}
 ...
 *          Mp = Y' * Y.
 */
void Project_Hamiltonian(SPARC_OBJ *pSPARC, int *DMVertices, double *Y, double *Hp, double *Mp, int k, int spn_i,
                         MPI_Comm comm) {
#if defined(USE_MKL) || defined(USE_SCALAPACK)
  if (pSPARC->bandcomm_index < 0 || comm == MPI_COMM_NULL)
    return;
  ...
  if (pSPARC->npband > 1) {
    ...
    pdgemr2d_(&Nd_blacscomm, &pSPARC->Nstates, Y, &ONE, &ONE, pSPARC->desc_orbitals, pSPARC->Yorb_BLCYC, &ONE, &ONE,
              pSPARC->desc_orb_BLCYC, &pSPARC->ictxt_blacs);
  } else {
    pSPARC->Yorb_BLCYC = Y;
  }
  t2 = MPI_Wtime();
...
#ifdef DEBUG
    if (!rank && spn_i == 0)
      printf("rank = %d, STARTING PDGEMM ...\n", rank);
#endif
    // perform matrix multiplication using ScaLAPACK routines
    pdsyrk_("U", "T", &pSPARC->Nstates, &Nd_blacscomm, &alpha, pSPARC->Yorb_BLCYC, &ONE, &ONE, pSPARC->desc_orb_BLCYC,
            &beta, Mp, &ONE, &ONE, pSPARC->desc_Mp_BLCYC);
  } else {
#ifdef DEBUG
    if (!rank && spn_i == 0)
      printf("rank = %d, STARTING DGEMM ...\n", rank);
#endif
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->Nstates, pSPARC->Nstates, Nd_blacscomm, 1.0,
                pSPARC->Yorb_BLCYC, Nd_blacscomm, pSPARC->Yorb_BLCYC, Nd_blacscomm, 0.0, Mp, pSPARC->Nstates);
  }
  t2 = MPI_Wtime();
#ifdef DEBUG
  if (!rank && spn_i == 0)
  ...
  if (nproc_dmcomm > 1 && !pSPARC->is_domain_uniform) {
    ...
    MPI_Allreduce(MPI_IN_PLACE, Mp, pSPARC->nr_Mp_BLCYC * pSPARC->nc_Mp_BLCYC, MPI_DOUBLE, MPI_SUM, pSPARC->dmcomm);
  }
  t2 = MPI_Wtime();
  t4 = MPI_Wtime();
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("rank = %2d, Allreduce to sum Psi'*Psi over dmcomm took %.3f ms\n", rank, (t2 - t1) * 1e3);
  if (!rank && spn_i == 0)
    printf("rank = %2d, Distribute data + matrix mult took %.3f ms\n", rank, (t4 - t3) * 1e3);
#endif
  ...
  if (pSPARC->StandardEigenFlag == 1) {
    ...
    t2 = MPI_Wtime();
    // update Yorb
    pdgemr2d_(&Nd_blacscomm, &pSPARC->Nstates, pSPARC->Yorb_BLCYC, &ONE, &ONE, pSPARC->desc_orb_BLCYC, Y, &ONE, &ONE,
              pSPARC->desc_orbitals, &pSPARC->ictxt_blacs);
    t3 = MPI_Wtime();
#ifdef DEBUG
    if (!rank && spn_i == 0)
      printf("Orthogonalization of orbitals took: %.3f ms\n", (t2 - t1) * 1e3);
    if (!rank && spn_i == 0)
      printf("Updating orbitals took: %.3f ms\n", (t3 - t2) * 1e3);
#endif
  }
  ...
  t1 = MPI_Wtime();
  // save HY in Xorb
  Hamiltonian_vectors_mult(pSPARC, pSPARC->Nd_d_dmcomm, DMVertices, pSPARC->Veff_loc_dmcomm + sg * pSPARC->Nd_d_dmcomm,
                           pSPARC->Atom_Influence_nloc, pSPARC->nlocProj, pSPARC->Nband_bandcomm, 0.0, Y,
                           pSPARC->Xorb + spn_i * size_s, spn_i, pSPARC->dmcomm);
  t2 = MPI_Wtime();
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("rank = %2d, finding HY took %.3f ms\n", rank, (t2 - t1) * 1e3);
#endif
  ...
  if (pSPARC->npband > 1) {
    ...
    pdgemr2d_(&Nd_blacscomm, &pSPARC->Nstates, pSPARC->Xorb + spn_i * size_s, &ONE, &ONE, pSPARC->desc_orbitals,
              HY_BLCYC, &ONE, &ONE, pSPARC->desc_orb_BLCYC, &pSPARC->ictxt_blacs);
  } else {
...
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("rank = %2d, distributing HY into block cyclic form took %.3f ms\n", rank, (t2 - t1) * 1e3);
#endif

  t1 = MPI_Wtime();
  if (pSPARC->npband > 1) {
    // perform matrix multiplication Y' * HY using ScaLAPACK routines
    pdgemm_("T", "N", &pSPARC->Nstates, &pSPARC->Nstates, &Nd_blacscomm, &alpha, pSPARC->Yorb_BLCYC, &ONE, &ONE,
            pSPARC->desc_orb_BLCYC, HY_BLCYC, &ONE, &ONE, pSPARC->desc_orb_BLCYC, &beta, Hp, &ONE, &ONE,
            pSPARC->desc_Hp_BLCYC);
  } else {
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->Nstates, pSPARC->Nstates, Nd_blacscomm, 1.0,
                pSPARC->Yorb_BLCYC, Nd_blacscomm, HY_BLCYC, Nd_blacscomm, 0.0, Hp, pSPARC->Nstates);
  }

  if (nproc_dmcomm > 1 && !pSPARC->is_domain_uniform) {
    // sum over all processors in dmcomm
    MPI_Allreduce(MPI_IN_PLACE, Hp, pSPARC->nr_Hp_BLCYC * pSPARC->nc_Hp_BLCYC, MPI_DOUBLE, MPI_SUM, pSPARC->dmcomm);
  }

  t2 = MPI_Wtime();
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("rank = %2d, finding Y'*HY took %.3f ms\n", rank, (t2 - t1) * 1e3);
#endif
  if (pSPARC->npband > 1) {
    free(HY_BLCYC);
  }
  ...
  et = MPI_Wtime();
  if (rank == 0 && spn_i == 0)
    printf("Rank 0, Project_Hamiltonian used %.3lf ms\n", 1000.0 * (et - st));
#endif
#endif // #if defined(USE_MKL) || defined(USE_SCALAPACK)
}
...
void Solve_Generalized_EigenProblem(SPARC_OBJ *pSPARC, int k, int spn_i) {
...
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("Start solving generalized eigenvalue problem ...\n");
#endif
  if (pSPARC->bandcomm_index < 0 || pSPARC->dmcomm == MPI_COMM_NULL)
    return;
  ...
  if (pSPARC->useACCEL == 1) {
    ...
    if ((!pSPARC->is_domain_uniform && !pSPARC->bandcomm_index) || (pSPARC->is_domain_uniform && !rank_kptcomm)) {
      if (pSPARC->StandardEigenFlag == 0)
        info = DSYGV(LAPACK_COL_MAJOR, 1, 'V', 'U', pSPARC->Nstates, pSPARC->Hp, pSPARC->Nstates, pSPARC->Mp,
  ...
  {    // SPARCX_ACCEL_NOTE Enclose the whole IF+ELSE statement from CPU-only and place it at the bottom.
    if (pSPARC->useLAPACK == 1) {
      ...
      if ((!pSPARC->is_domain_uniform && !pSPARC->bandcomm_index) || (pSPARC->is_domain_uniform && !rank_kptcomm)) {
        ...
        if (pSPARC->StandardEigenFlag == 0)
          info = LAPACKE_dsygvd(LAPACK_COL_MAJOR, 1, 'V', 'U', pSPARC->Nstates, pSPARC->Hp, pSPARC->Nstates, pSPARC->Mp,
                                pSPARC->Nstates, pSPARC->lambda + spn_i * pSPARC->Nstates);
        else
          info = LAPACKE_dsyevd(LAPACK_COL_MAJOR, 'V', 'U', pSPARC->Nstates, pSPARC->Hp, pSPARC->Nstates,
                                pSPARC->lambda + spn_i * pSPARC->Nstates);
      ...
      t1 = MPI_Wtime();
      if (pSPARC->StandardEigenFlag == 0)
        pdsygvx_subcomm_(&ONE, "V", "A", "U", &N, pSPARC->Hp, &ONE, &ONE, pSPARC->desc_Hp_BLCYC, pSPARC->Mp, &ONE, &ONE,
                         pSPARC->desc_Mp_BLCYC, &vl, &vu, &il, &iu, &abstol, &M, &NZ, pSPARC->lambda + spn_i * N,
                         &orfac, pSPARC->Q, &ONE, &ONE, pSPARC->desc_Q_BLCYC, ifail, &info, pSPARC->blacscomm,
                         pSPARC->eig_paral_subdims, pSPARC->eig_paral_blksz);
      else
        pdsyevx_subcomm_("V", "A", "U", &N, pSPARC->Hp, &ONE, &ONE, pSPARC->desc_Hp_BLCYC, &vl, &vu, &il, &iu, &abstol,
                         &M, &NZ, pSPARC->lambda + spn_i * N, &orfac, pSPARC->Q, &ONE, &ONE, pSPARC->desc_Q_BLCYC,
                         ifail, &info, pSPARC->blacscomm, pSPARC->eig_paral_subdims, pSPARC->eig_paral_blksz);
  ...
  double et = MPI_Wtime();
  if (rank == 0)
    printf("rank = %d, Solve_Generalized_EigenProblem used %.3lf ms\n", rank, 1000.0 * (et - st));
#endif
...
void Subspace_Rotation(SPARC_OBJ *pSPARC, double *Psi, double *Q, double *PsiQ, double *Psi_rot, int k, int spn_i) {
  ...
  if (pSPARC->npband > 1) {
    ...
    pdgemm_("N", "N", &Nd_blacscomm, &pSPARC->Nstates, &pSPARC->Nstates, &alpha, Psi, &ONE, &ONE,
            pSPARC->desc_orb_BLCYC, Q, &ONE, &ONE, pSPARC->desc_Q_BLCYC, &beta, PsiQ, &ONE, &ONE,
            pSPARC->desc_orb_BLCYC);
  } else {
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Nd_blacscomm, pSPARC->Nstates, pSPARC->Nstates, 1.0, Psi,
                Nd_blacscomm, Q, pSPARC->Nstates, 0.0, PsiQ, Nd_blacscomm);
  }
  t2 = MPI_Wtime();
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("rank = %2d, subspace rotation using ScaLAPACK took %.3f ms\n", rank, (t2 - t1) * 1e3);
#endif
  t1 = MPI_Wtime();
  if (pSPARC->npband > 1) {
    // distribute rotated orbitals from block cyclic format back into
    // original format (band + domain)
    pdgemr2d_(&Nd_blacscomm, &pSPARC->Nstates, PsiQ, &ONE, &ONE, pSPARC->desc_orb_BLCYC, Psi_rot, &ONE, &ONE,
              pSPARC->desc_orbitals, &pSPARC->ictxt_blacs);
  }
  t2 = MPI_Wtime();
#ifdef DEBUG
  ...
  double et = MPI_Wtime();
  if (rank == 0)
    printf("rank = %d, Subspace_Rotation used %.3lf ms\n\n", rank, 1000.0 * (et - st));
#endif
#endif // #if defined(USE_MKL) || defined(USE_SCALAPACK)
}
 ...
 */
void Lanczos(const SPARC_OBJ *pSPARC, int *DMVertices, double *Veff_loc, ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_nloc,
             NLOC_PROJ_OBJ *nlocProj, double *eigmin, double *eigmax, double *x0, double TOL_min, double TOL_max,
...
#ifdef DEBUG
    if (!rank && spn_i == 0)
      printf("rank = %d, inter-communicator Bcast took %.3f ms\n", rank, (t2 - t1) * 1e3);
#endif
    return;
  }
...
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("Wait for veff to be bcasted took %.3f ms\n", (t2 - t1) * 1e3);
#endif
  t1 = MPI_Wtime();
  Hamiltonian_vectors_mult(pSPARC, DMnd, DMVertices, Veff_loc, Atom_Influence_nloc, nlocProj, 1, 0.0, V_jm1, V_j, spn_i,
                           comm);
  t2 = MPI_Wtime();
#ifdef DEBUG
  if (!rank && spn_i == 0)
    printf("rank = %2d, One H*x took %.3f ms\n", rank, (t2 - t1) * 1e3);
#endif
  ...
  while ((err_eigmin > TOL_min || err_eigmax > TOL_max) && j < MAXIT) {
    // t1 = MPI_Wtime();
    // V_{j+1} = H * V_j
    Hamiltonian_vectors_mult(pSPARC, DMnd, DMVertices, Veff_loc, Atom_Influence_nloc, nlocProj, 1, 0.0, V_j, V_jp1,
                             spn_i, comm);
    ...
    if (!LAPACKE_dsterf(j + 2, d, e)) {
      ...
      *eigmax = d[j + 1];
      ...
      if (rank == 0) {
        printf("WARNING: Tridiagonal matrix eigensolver (?sterf) failed!\n");
      }
      break;
    }
  ...
  if (rank == 0 && spn_i == 0) {
    printf(
        "    Lanczos iter %d, eigmin  = %.9f, eigmax = %.9f, err_eigmin = %.3e, err_eigmax = %.3e, taking %.3f ms.\n",
 ...
 *          for the Lap.
 */
void Lanczos_laplacian(const SPARC_OBJ *pSPARC, const int *DMVertices, double *eigmin, double *eigmax, double *x0,
                       const double TOL_min, const double TOL_max, const int MAXIT, int k, int spn_i, MPI_Comm comm) {
  double t1, t2, ts, te;
