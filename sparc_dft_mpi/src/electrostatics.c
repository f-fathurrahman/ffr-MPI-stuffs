...
// Info: Working in the four upper quadrants only (from symmetry) for nonorthogonal and 1 quadrant for orthogonal
// systems
...
#define RB_Y(y, L, typ) (typ) < 20 ? (y) : acos(1 - (y) * (y) / (2 * (L) * (L))) + pSPARC->twist *y
...
#ifdef DEBUG
  if (rank == 0)
    printf("Rbmax_x = %f, Rbmax_y = %f, Rbmax_z = %f\n", Rbmax_x, Rbmax_y, Rbmax_z);
#endif
  ...
  t3 = MPI_Wtime();
  if (rank == 0)
    printf("time spent on qsort: %.3f ms.\n", (t3 - t31) * 1000);
#endif
  ...
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    ...
    if (pSPARC->psd[ityp].is_r_uniform == 1) {
      SplineInterpUniform(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rVloc, pSPARC->psd[ityp].size, R, temp3,
                          len_interp, pSPARC->psd[ityp].SplinerVlocD);
    } else {
      SplineInterpNonuniform(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rVloc, pSPARC->psd[ityp].size, R, temp3,
                             len_interp, pSPARC->psd[ityp].SplinerVlocD);
    }
    ...
    while (count < 20) {
      if (error_cur < pSPARC->TOL_PSEUDOCHARGE) {
        if (error_prev >= pSPARC->TOL_PSEUDOCHARGE) {
          if (fabs(rb_cur_x - rb_prev_x) < rtol_x && fabs(rb_cur_y - rb_prev_y) < rtol_y &&
          ...
          if (fabs(rb_cur_x - lowerbd_x) < rtol_x && fabs(rb_cur_y - lowerbd_y) < rtol_y &&
        ...
        if (error_prev >= pSPARC->TOL_PSEUDOCHARGE) {
          if (fabs(upperbd_x - rb_cur_x) < rtol_x && fabs(upperbd_y - rb_cur_y) < rtol_y &&
          ...
          if (fabs(rb_prev_x - rb_cur_x) < rtol_x && fabs(rb_prev_y - rb_cur_y) < rtol_y &&
        ...
        for (k = 0; k <= Ncube_z; k++) // for(k = nz - Ncube_z; k <= nz + Ncube_z; k++)
          for (j = ny - Ncube_y; j <= ny + Ncube_y; j++)
            for (i = nx - Ncube_x; i <= nx + Ncube_x; i++) {
              val = b(i, j, k);
              if (k == 0)
                val *= 0.5; // if (k == nz - Ncube_z) val *= 0.5;
              if (k == Ncube_z)
                val *= 0.5; // if (k == nz + Ncube_z) val *= 0.5;
              if (j == ny - Ncube_y)
                val *= 0.5;
...
#ifdef DEBUG
      if (rank == 0) {
        printf("Z = %d,rb_x = %f,rb_y = %f,rb_z = %f,error = %.3E,Bint = %.13f\n", pSPARC->Znucl[ityp], rb_cur_x,
               rb_cur_y, rb_cur_z, error_cur, Bint);
      }
#endif
    }
    ...
    if (rb_cur_x == Rbmax_x || rb_cur_y == Rbmax_y || rb_cur_z == Rbmax_z) {
      if (rank == 0 && error_cur > pSPARC->TOL_PSEUDOCHARGE) {
        ...
        printf("\nWARNING: upperbond for pseudocharge radius (Rbmax_?) is not big enough! Rbmax_x = %.1f, Rbmax_y = "
               "%.1f, Rbmax_z = %.1f\n\n",
               Rbmax_x, Rbmax_y, Rbmax_z);
      }
    }
...
#ifdef DEBUG
    if (rank == 0) {
      printf("dx = %f, dy = %f, dz = %f, Ncube_x = %d, Ncube_y = %d, Ncube_z = %d\n", dx, dy, dz, Ncube_x, Ncube_y,
             Ncube_z);
      printf("ityp = %d, converged in %d iters, err_cur = %.2E, TOL = %.2E, rb = {%f, %f, %f}, after proj to grid rb = "
             "{%f, %f, %f}\n",
             ityp, count, error_cur, pSPARC->TOL_PSEUDOCHARGE, rb_cur_x, rb_cur_y, rb_cur_z, pSPARC->CUTOFF_x[ityp],
             pSPARC->CUTOFF_y[ityp], pSPARC->CUTOFF_z[ityp]);
    }
#endif
    ...
    t5 = MPI_Wtime();
    if (rank == 0)
      printf("time for finding rb using bisection: %.3f ms.\n", (t5 - t4) * 1000);
#endif
    // deallocate memory
    free(b);
    b = NULL;
...
#ifdef DEBUG
    if (!rank)
      printf("Is Rb region out? isRbOut = [%d, %d, %d]\n", isRbOut[0], isRbOut[1], isRbOut[2]);
#endif
  }
}
...
#define rho_c_J(i, j, k) rho_c_J[(k)*nxp * nyp + (j)*nxp + (i)]
  int nproc, rank, ityp, iat, i, j, k, ip, jp, kp, i_global, j_global, k_global, i_DM, j_DM, k_DM, dI, dJ, dK, FDn,
      count, count_interp, DMnx, DMny, DMnz, DMnd, nx, ny, nz, nd, nxp, nyp, nzp, nd_ex, icor, jcor, kcor, len_interp;
  int spn_i;
  double x0_i, y0_i, z0_i, x0_i_shift, y0_i_shift, z0_i_shift, x, y, z, *R, *VJ, *VJ_ref, Esc, *rho_J;
  ...
  FDn = pSPARC->order / 2;
  w2_diag =
      (pSPARC->D2_stencil_coeffs_x[0] + pSPARC->D2_stencil_coeffs_y[0] + pSPARC->D2_stencil_coeffs_z[0]) * -inv_4PI;
  if (pSPARC->cell_typ == 0) {
    Lap_wt = (double *)malloc((3 * (FDn + 1)) * sizeof(double));
    Lap_stencil = Lap_wt;
  } else {
  ...
  for (i = 0; i < DMnd; i++) {
    pSPARC->electronDens_at[i] = pSPARC->electronDens_core[i] = pSPARC->psdChrgDens[i] = pSPARC->psdChrgDens_ref[i] =
        pSPARC->Vc[i] = 0.0;
  }
        ...
        for (k = 0; k < nzp; k++) {
          ...
          for (j = 0; j < nyp; j++) {
            ...
            for (i = 0; i < nxp; i++) {
              x = i * pSPARC->delta_x - x0_i_shift;
              R[count] =
                  sqrt(pSPARC->metricT[0] * (x * x) + pSPARC->metricT[1] * (x * y) + pSPARC->metricT[2] * (x * z) +
...
// Calc_dist(pSPARC, nxp, nyp, nzp, x0_i_shift, y0_i_shift, z0_i_shift, R, rchrg, &count_interp);
      ...
      len_interp = nd_ex;
      if (pSPARC->psd[ityp].is_r_uniform == 1) {
        SplineInterpUniform(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rVloc, pSPARC->psd[ityp].size, R, VJ,
                            len_interp, pSPARC->psd[ityp].SplinerVlocD);
      } else {
        SplineInterpNonuniform(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rVloc, pSPARC->psd[ityp].size, R, VJ,
                               len_interp, pSPARC->psd[ityp].SplinerVlocD);
      }
      ...
      len_interp = nd_ex;
      if (pSPARC->psd[ityp].is_r_uniform == 1) {
        SplineInterpUniform(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rhoIsoAtom, pSPARC->psd[ityp].size, R,
                            rho_J, len_interp, pSPARC->psd[ityp].SplineFitIsoAtomDen);
      } else {
        SplineInterpNonuniform(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rhoIsoAtom, pSPARC->psd[ityp].size, R,
                               rho_J, len_interp, pSPARC->psd[ityp].SplineFitIsoAtomDen);
      }

      double *rho_c_J = (double *)malloc(nd_ex * sizeof(double));
      assert(rho_c_J != NULL);
      SplineInterpMain(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rho_c_table, pSPARC->psd[ityp].size, R, rho_c_J,
                       len_interp, pSPARC->psd[ityp].SplineRhocD, pSPARC->psd[ityp].is_r_uniform);
      ...
      for (k = 0; k < nz; k++) {
        ...
        for (j = 0; j < ny; j++) {
          ...
          for (i = 0; i < nx; i++) {
            ...
            for (spn_i = 1; spn_i < 2 * pSPARC->Nspin - 1; spn_i++) {
              pSPARC->electronDens_at(spn_i * DMnd, i_DM, j_DM, k_DM) +=
                  (0.5 + (1.5 - spn_i) * spin_frac) * rho_J(ip, jp, kp);
  ...
  t2 = MPI_Wtime();
  if (rank == 0)
    printf("the global sum of int_b = %.13f, sum_int_rho = %.13f\n", -pSPARC->PosCharge, -pSPARC->NegCharge);
#endif
  ...
  if (pSPARC->spin_typ != 0 && pSPARC->elecgs_Count == 0) {
    ...
    spn_int[0] = int_rhoup;
    spn_int[1] = int_rhodn;
    MPI_Allreduce(spn_int, spn_sum, 2, MPI_DOUBLE, MPI_SUM, pSPARC->dmcomm_phi);
...
#ifdef DEBUG
  if (rank == 0)
    printf("After scaling, int_rho = %.13f, PosCharge + NegCharge - NetCharge = %.3e\n", -pSPARC->NegCharge,
           -pSPARC->NetCharge + pSPARC->PosCharge + pSPARC->NegCharge);
#endif
  ...
  if (rank == 0) {
    printf("\n integral of b = %.13f,\n int{b} + Nelectron + NetCharge = %.3e,\n Esc = %.13f,\n MPI_Allreduce took "
           "%.3f ms\n",
           -pSPARC->PosCharge, -pSPARC->PosCharge + pSPARC->Nelectron + pSPARC->NetCharge, pSPARC->Esc,
           (t2 - t1) * 1e3);
  }
#endif
...
*/
void Calc_lapV(const SPARC_OBJ *pSPARC, const double *VJ, const double FDn, const int nxp, const int nyp, const int nzp,
               const int nx, const int ny, const int nz, const double *Lap_wt, const double w2_diag, double xin,
               double coef, double *bJ) {
  int nxny = nx * ny;
  int nxny_ex = nxp * nyp;
  int nxexny = nxp * ny;
  int nxnyex = nx * nyp;
  ...
  case 0:
    stencil_3axis_thread_v2(VJ, FDn, nx, nxp, nxny, nxny_ex, 0, nx, 0, ny, 0, nz, FDn, FDn, FDn, Lap_wt, w2_diag, 0.0,
                            VJ, bJ);
    break; /* optional */
  ...
  case 11:
    ...
    Calc_DX(VJ, dV1, FDn, nxp, nxp, nxp, nxny_ex, nxexny, 0, nxp, 0, ny, 0, nz, 0, FDn, FDn,
            pSPARC->D1_stencil_coeffs_y, 0.0);

    stencil_4comp(VJ, dV1, FDn, 1, nx, nxp, nxp, nxny, nxny_ex, nxexny, 0, nx, 0, ny, 0, nz, FDn, FDn, FDn, FDn, 0, 0,
  ...
  case 12:
    ...
    Calc_DX(VJ, dV1, FDn, nxny_ex, nxp, nxp, nxny_ex, nxexny, 0, nxp, 0, ny, 0, nz, 0, FDn, FDn,
  ...
  case 13:
    ...
    Calc_DX(VJ, dV1, FDn, nxny_ex, nxp, nx, nxny_ex, nxnyex, 0, nx, 0, nyp, 0, nz, FDn, 0, FDn,
            pSPARC->D1_stencil_coeffs_z, 0.0);

    stencil_4comp(VJ, dV1, FDn, nx, nx, nxp, nx, nxny, nxny_ex, nxnyex, 0, nx, 0, ny, 0, nz, FDn, FDn, FDn, 0, FDn, 0,
                  Lap_wt, w2_diag, 0.0, VJ, bJ);
  ...
  case 14:
    ...
    Calc_DX1_DX2(VJ, dV1, FDn, nxp, nxny_ex, nxp, nxp, nxny_ex, nxexny, 0, nxp, 0, ny, 0, nz, 0, FDn, FDn,
  ...
  case 15:
    ...
    Calc_DX1_DX2(VJ, dV1, FDn, 1, nxp, nxp, nx, nxny_ex, nxny, 0, nx, 0, ny, 0, nzp, FDn, FDn, 0,
    ...
    stencil_4comp(VJ, dV1, FDn, nxny, nx, nxp, nx, nxny, nxny_ex, nxny, 0, nx, 0, ny, 0, nz, FDn, FDn, FDn, 0, 0, FDn,
                  Lap_wt, w2_diag, 0.0, VJ, bJ);
  ...
  case 16:
    ...
    Calc_DX1_DX2(VJ, dV1, FDn, 1, nxny_ex, nxp, nx, nxny_ex, nxnyex, 0, nx, 0, nyp, 0, nz, FDn, 0, FDn,
  ...
  case 17:
    ...
    Calc_DX(VJ, dV2, FDn, nxny_ex, nxp, nx, nxny_ex, nxnyex, 0, nx, 0, nyp, 0, nz, FDn, 0, FDn,
            pSPARC->D1_stencil_coeffs_z, 0.0);

    stencil_5comp(VJ, dV1, dV2, FDn, 1, nx, nx, nxp, nxp, nx, nxny, nxny_ex, nxexny, nxnyex, 0, nx, 0, ny, 0, nz, FDn,
                  FDn, FDn, FDn, 0, 0, 0, FDn, 0, Lap_wt, w2_diag, 0.0, VJ, bJ);
...
void Calculate_Pseudopot_Ref(double *R, int len, double rcut, int Znucl, double *Vref) {
  ...
  for (i = 0; i < len; i++) {
    if (R[i] <= rcut) {
      ...
      // use horner's rule
      Vref[i] =
          R2 * (R2 * R[i] * (5.6 / C6 + R[i] * (1.8 * R[i] / (C6 * rcut * rcut) - 6.0 / (C6 * rcut))) - 2.8 / C3) +
          2.4 / rcut;
      // Vref[i] = pow(R[i],2) * ( pow(R[i],3) * (5.6 / pow(rcut,6) + R[i]*(1.8*R[i]/pow(rcut,8) - 6/pow(rcut,7) ) )
      // - 2.8/pow(rcut,3)) + 2.4/rcut;
      // // calculate directly
      // Vref[i] = (9.0*pow(R[i],7) - 30.0*pow(R[i],6)*rcut + 28.0*pow(R[i],5)*rcut*rcut - 14.0*pow(R[i],2)*pow(rcut,5)
      // + 12.0*pow(rcut,7))/(5.0*pow(rcut,8));
      Vref[i] *= Znucl;
    } else {
      Vref[i] = Znucl / R[i];
    }
...
void poisson_RHS(SPARC_OBJ *pSPARC, double *rhs) {
  ...
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  if (rank == 0)
    printf("Start calculating rhs of the poisson problem\n");
#endif
  ...
  if (n_periodic == 0) {
    ...
    t1 = MPI_Wtime();
    if (rank == 0)
      printf("Start calculating Multipole Expansion for Dirichlet BC\n");
#endif
    ...
    t1 = MPI_Wtime();
    if (rank == 0)
      printf("Start calculating Surface BC\n");
#endif
    ...
    t1 = MPI_Wtime();
    if (rank == 0)
      printf("Start calculating Wire BC\n");
#endif
...
#define electronDens(i, j, k) electronDens[(k)*DMnx * DMny + (j)*DMnx + (i)]
  ...
  void (*residule_fptr)(SPARC_OBJ *, int, double, double *, double *, double *, MPI_Comm, double *) =
  ...
  if (!rank)
    printf("rank = %d, int_b = %18.14f, int_rho = %18.14f, int_b + int_rho = %.3e, checking this took %.3f ms\n", rank,
           int_b * pSPARC->dV, int_rho * pSPARC->dV, (int_b + int_rho) * pSPARC->dV, (t2 - t1) * 1e3);
  t1 = MPI_Wtime();
#endif
  ...
  if (pSPARC->POISSON_SOLVER == 0) {
    ...
    AAR(pSPARC, residule_fptr, Jacobi_fptr, 0.0, DMnd, pSPARC->elecstPotential, rhs, omega, beta, m, p,
        pSPARC->TOL_POISSON, pSPARC->MAXIT_POISSON, pSPARC->dmcomm_phi);
  } else {
    if (rank == 0)
      printf("Please provide a valid poisson solver!\n");
    exit(EXIT_FAILURE);
    // CG(pSPARC, Lap_vec_mult, pSPARC->Nd, DMnd, pSPARC->elecstPotential, rhs, pSPARC->TOL_POISSON,
    // pSPARC->MAXIT_POISSON, pSPARC->dmcomm_phi);
  }

#ifdef DEBUG
  t2 = MPI_Wtime();
  if (rank == 0)
    printf("Solving Poisson took %.3f ms\n", (t2 - t1) * 1e3);
#endif
...
void Jacobi_preconditioner(SPARC_OBJ *pSPARC, int N, double c, double *r, double *f, MPI_Comm comm) {
  ...
  m_inv = pSPARC->D2_stencil_coeffs_x[0] + pSPARC->D2_stencil_coeffs_y[0] + pSPARC->D2_stencil_coeffs_z[0] + c;
  if (fabs(m_inv) < 1e-14) {
    m_inv = 1.0;
  }
  m_inv = -1.0 / m_inv;
...
#define phi(i, j, k) phi[(k)*nx_phi * ny_phi + (j)*nx_phi + (i)]
  ...
  int LMAX = 6, l, m, i, j, k, p, DMnx, DMny, DMnz, DMnd, count, index, Q_len, FDn, Nx, Ny, Nz, i_global, j_global,
      k_global, nbr_i, is, ie, js, je, ks, ke, nx_cor, ny_cor, nz_cor, nd_cor, is_phi, ie_phi, js_phi, je_phi, ks_phi,
      ke_phi, nx_phi, ny_phi, nz_phi, nd_phi, i_phi, j_phi, k_phi, i_DM, j_DM, k_DM, DMCorVert[6][6];
  double *Qlm, Lx, Ly, Lz, *r_pos_x, *r_pos_y, *r_pos_z, *r_pos_r, *r_pow_l, *Ylm, *phi, x, y, z, r, x2, y2, z2;
  ...
  DMCorVert[5][2] = 0;
  DMCorVert[5][3] = Ny - 1;
  DMCorVert[5][4] = Nz - FDn;
  DMCorVert[5][5] = Nz - 1;

  for (i = 0; i < DMnd; i++)
    d_cor[i] = 0.0; // init correction to 0
  ...
  for (nbr_i = 0; nbr_i < 6; nbr_i++) {
    ...
    for (i = 0; i < nd_phi; i++)
      r_pow_l[i] = 1.0; // init r_pow_l to 1
    index = 0;
    for (l = 0; l <= LMAX; l++) {
      // find r^(l+1)
      for (i = 0; i < nd_phi; i++)
    ...
    for (k = ks; k <= ke; k++) {
      k_phi = k - ks_phi;
      k_DM = k - pSPARC->DMVertices[4];
      for (j = js; j <= je; j++) {
        j_phi = j - js_phi;
        j_DM = j - pSPARC->DMVertices[2];
        for (i = is; i <= ie; i++) {
          i_phi = i - is_phi;
          i_DM = i - pSPARC->DMVertices[0];
          for (p = 1; p <= FDn; p++) {
            switch (nbr_i) {
            case 0:
              if ((i - p) < 0)
...
#define phi(i, j, k) phi[(k)*nx_phi * ny_phi + (j)*nx_phi + (i)]
  ...
  for (int nbr_i = 0; nbr_i < 6; nbr_i++) {
    ...
    switch (nbr_i) {
    case 0:
      DMVert_phi[nbr_i][0] -= FDn;
      DMVert_phi[nbr_i][1] -= FDn;
      break;
    case 1:
      DMVert_phi[nbr_i][0] += FDn;
      DMVert_phi[nbr_i][1] += FDn;
      break;
    case 2:
      DMVert_phi[nbr_i][2] -= FDn;
      DMVert_phi[nbr_i][3] -= FDn;
      break;
    case 3:
      DMVert_phi[nbr_i][2] += FDn;
      DMVert_phi[nbr_i][3] += FDn;
      break;
    case 4:
      DMVert_phi[nbr_i][4] -= FDn;
      DMVert_phi[nbr_i][5] -= FDn;
      break;
    case 5:
      DMVert_phi[nbr_i][4] += FDn;
      DMVert_phi[nbr_i][5] += FDn;
  ...
  // sum over processors in the sub-slices in the X-Y plane
  MPI_Allreduce(MPI_IN_PLACE, V_av, 2 * FDn * (NX + NY), MPI_DOUBLE, MPI_SUM, XY_comm);
