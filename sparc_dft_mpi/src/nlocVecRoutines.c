 ...
 */
void GetInfluencingAtoms_nloc(SPARC_OBJ *pSPARC,
...
#ifdef DEBUG
  if (rank == 0)
    printf("Finding atoms that has nonlocal influence on the local process "
           "domain ... \n");
#endif
  // processors that are not in the dmcomm will remain idle
  // on input make comm of processes with bandcomm_index or kptcomm_index
  // MPI_COMM_NULL!
  ...
  double Lx, Ly, Lz, rc, x0, y0, z0, x0_i, y0_i, z0_i, x2, y2, z2, r2, rc2,
      rcbox_x, rcbox_y, rcbox_z, x, y, z;
  int count_overlap_nloc, count_overlap_nloc_sphere, ityp, i, j, k, count, i_DM,
      j_DM, k_DM, iat, atmcount, atmcount2, DMnx, DMny;
  ...
  // TODO: notice the difference here from rb-domain, since nonlocal projectors
  // decay drastically, using the fd-nodes as edges are more appropriate
  DMxs = pSPARC->xin + DMVertices[0] * pSPARC->delta_x;
  DMxe = pSPARC->xin +
         (DMVertices[1]) *
             pSPARC->delta_x; // note that this is not the actual edge, add BCx
                              // to get actual domain edge
  DMys = DMVertices[2] * pSPARC->delta_y;
  DMye = (DMVertices[3]) *
         pSPARC->delta_y; // note that this is not the actual edge, add BCx to
                          // get actual domain edge
  DMzs = DMVertices[4] * pSPARC->delta_z;
  DMze = (DMVertices[5]) *
         pSPARC->delta_z; // note that this is not the actual edge, add BCx to
                          // get actual domain edge
  ...
  // TODO: in the future, it's better to save only the atom types that have
  // influence on the local domain
  *Atom_Influence_nloc = (ATOM_NLOC_INFLUENCE_OBJ *)malloc(
      sizeof(ATOM_NLOC_INFLUENCE_OBJ) * pSPARC->Ntypes);

  ATOM_NLOC_INFLUENCE_OBJ Atom_Influence_nloc_temp;

  // find which atoms have nonlocal influence on the distributed domain owned by
  // current process
  atmcount = 0;
  atmcount2 = 0;
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    ...
    // first loop over all atoms of each type to find number of influencing
    // atoms
    ...
    for (i = 0; i < pSPARC->nAtomv[ityp]; i++) {
      ...
      // check how many of it's images interacts with the local distributed
      // domain
      for (rr = rrmin; rr <= rrmax; rr++) {
        z0_i = z0 + Lz * rr; // z coord of image atom
        if ((z0_i < DMzs - rcbox_z) || (z0_i >= DMze + rcbox_z))
          continue;
        for (qq = qqmin; qq <= qqmax; qq++) {
          y0_i = y0 + Ly * qq; // y coord of image atom
          if ((y0_i < DMys - rcbox_y) || (y0_i >= DMye + rcbox_y))
            continue;
          for (pp = ppmin; pp <= ppmax; pp++) {
            x0_i = x0 + Lx * pp; // x coord of image atom
            if ((x0_i < DMxs - rcbox_x) || (x0_i >= DMxe + rcbox_x))
              continue;
            count_overlap_nloc++;
          }
        }
    ...
    Atom_Influence_nloc_temp.coords =
        (double *)malloc(sizeof(double) * count_overlap_nloc * 3);
    Atom_Influence_nloc_temp.atom_index =
        (int *)malloc(sizeof(int) * count_overlap_nloc);
    Atom_Influence_nloc_temp.xs =
        (int *)malloc(sizeof(int) * count_overlap_nloc);
    Atom_Influence_nloc_temp.ys =
        (int *)malloc(sizeof(int) * count_overlap_nloc);
    Atom_Influence_nloc_temp.zs =
        (int *)malloc(sizeof(int) * count_overlap_nloc);
    Atom_Influence_nloc_temp.xe =
        (int *)malloc(sizeof(int) * count_overlap_nloc);
    Atom_Influence_nloc_temp.ye =
        (int *)malloc(sizeof(int) * count_overlap_nloc);
    Atom_Influence_nloc_temp.ze =
        (int *)malloc(sizeof(int) * count_overlap_nloc);
    Atom_Influence_nloc_temp.ndc =
        (int *)malloc(sizeof(int) * count_overlap_nloc);
    Atom_Influence_nloc_temp.grid_pos =
        (int **)malloc(sizeof(int *) * count_overlap_nloc);

    // when there's no atom of this type that have influence, go to next type of
    // atom
    ...
    // loop over atoms of this type again to find overlapping region and atom
    // info
    count_overlap_nloc = 0;
      ...
      for (rr = rrmin; rr <= rrmax; rr++) {
        ...
        for (qq = qqmin; qq <= qqmax; qq++) {
          ...
          for (pp = ppmin; pp <= ppmax; pp++) {
            ...
            Atom_Influence_nloc_temp.atom_index[count_overlap_nloc] =
                atmcount2 - 1;
            ...
            // TODO: check if rc-region is out of fundamental domain for BC ==
            // 1! find overlap of rc-region of the image and the local dist.
            // domain
            Atom_Influence_nloc_temp.xs[count_overlap_nloc] =
                max(DMVertices[0], rc_xl);
            Atom_Influence_nloc_temp.xe[count_overlap_nloc] =
                min(DMVertices[1], rc_xr);
            Atom_Influence_nloc_temp.ys[count_overlap_nloc] =
                max(DMVertices[2], rc_yl);
            Atom_Influence_nloc_temp.ye[count_overlap_nloc] =
                min(DMVertices[3], rc_yr);
            Atom_Influence_nloc_temp.zs[count_overlap_nloc] =
                max(DMVertices[4], rc_zl);
            Atom_Influence_nloc_temp.ze[count_overlap_nloc] =
                min(DMVertices[5], rc_zr);

            // find the spherical rc-region
            ndc = (Atom_Influence_nloc_temp.xe[count_overlap_nloc] -
                   Atom_Influence_nloc_temp.xs[count_overlap_nloc] + 1) *
                  (Atom_Influence_nloc_temp.ye[count_overlap_nloc] -
                   Atom_Influence_nloc_temp.ys[count_overlap_nloc] + 1) *
                  (Atom_Influence_nloc_temp.ze[count_overlap_nloc] -
                   Atom_Influence_nloc_temp.zs[count_overlap_nloc] + 1);

            // first allocate memory for the rectangular rc-region, resize later
            // to the spherical rc-region
            Atom_Influence_nloc_temp.grid_pos[count_overlap_nloc] =
                (int *)malloc(sizeof(int) * ndc);
            if (pSPARC->cell_typ == 0) {
              count = 0;
              for (k = Atom_Influence_nloc_temp.zs[count_overlap_nloc];
                   k <= Atom_Influence_nloc_temp.ze[count_overlap_nloc]; k++) {
                k_DM = k - DMVertices[4];
                z2 = k * pSPARC->delta_z - z0_i;
                z2 *= z2;
                for (j = Atom_Influence_nloc_temp.ys[count_overlap_nloc];
                     j <= Atom_Influence_nloc_temp.ye[count_overlap_nloc];
                     j++) {
                  j_DM = j - DMVertices[2];
                  y2 = j * pSPARC->delta_y - y0_i;
                  y2 *= y2;
                  for (i = Atom_Influence_nloc_temp.xs[count_overlap_nloc];
                       i <= Atom_Influence_nloc_temp.xe[count_overlap_nloc];
                       i++) {
                    i_DM = i - DMVertices[0];
                    ...
                    r2 = x2 + y2 + z2;
                    if (r2 <= rc2) {
                      Atom_Influence_nloc_temp
                          .grid_pos[count_overlap_nloc][count] =
                          k_DM * (DMnx * DMny) + j_DM * DMnx + i_DM;
                      count++;
                    }
                  }
                }
              ...
              for (k = Atom_Influence_nloc_temp.zs[count_overlap_nloc];
                ...
                for (j = Atom_Influence_nloc_temp.ys[count_overlap_nloc];
                  ...
                  for (i = Atom_Influence_nloc_temp.xs[count_overlap_nloc];
                    ...
                    r2 = pSPARC->metricT[0] * (x * x) +
                         pSPARC->metricT[1] * (x * y) +
                         pSPARC->metricT[2] * (x * z) +
                         pSPARC->metricT[4] * (y * y) +
                         pSPARC->metricT[5] * (y * z) +
                         pSPARC->metricT[8] * (z * z);
                    if (r2 <= rc2) {
                      Atom_Influence_nloc_temp
                          .grid_pos[count_overlap_nloc][count] =
                          k_DM * (DMnx * DMny) + j_DM * DMnx + i_DM;
            ...
            // TODO: in some cases count is 0! check if ndc is 0 and remove
            // those!
    ...
    (*Atom_Influence_nloc)[ityp].coords =
        (double *)malloc(sizeof(double) * count_overlap_nloc_sphere * 3);
    (*Atom_Influence_nloc)[ityp].atom_index =
        (int *)malloc(sizeof(int) * count_overlap_nloc_sphere);
    (*Atom_Influence_nloc)[ityp].xs =
        (int *)malloc(sizeof(int) * count_overlap_nloc_sphere);
    (*Atom_Influence_nloc)[ityp].ys =
        (int *)malloc(sizeof(int) * count_overlap_nloc_sphere);
    (*Atom_Influence_nloc)[ityp].zs =
        (int *)malloc(sizeof(int) * count_overlap_nloc_sphere);
    (*Atom_Influence_nloc)[ityp].xe =
        (int *)malloc(sizeof(int) * count_overlap_nloc_sphere);
    (*Atom_Influence_nloc)[ityp].ye =
        (int *)malloc(sizeof(int) * count_overlap_nloc_sphere);
    (*Atom_Influence_nloc)[ityp].ze =
        (int *)malloc(sizeof(int) * count_overlap_nloc_sphere);
    (*Atom_Influence_nloc)[ityp].ndc =
        (int *)malloc(sizeof(int) * count_overlap_nloc_sphere);
    (*Atom_Influence_nloc)[ityp].grid_pos =
        (int **)malloc(sizeof(int *) * count_overlap_nloc_sphere);
    ...
    for (i = 0; i < count_overlap_nloc; i++) {
      if (Atom_Influence_nloc_temp.ndc[i] > 0) {
        ...
        (*Atom_Influence_nloc)[ityp].coords[count * 3] =
            Atom_Influence_nloc_temp.coords[i * 3];
        (*Atom_Influence_nloc)[ityp].coords[count * 3 + 1] =
            Atom_Influence_nloc_temp.coords[i * 3 + 1];
        (*Atom_Influence_nloc)[ityp].coords[count * 3 + 2] =
            Atom_Influence_nloc_temp.coords[i * 3 + 2];
        (*Atom_Influence_nloc)[ityp].atom_index[count] =
            Atom_Influence_nloc_temp.atom_index[i];
        ...
        (*Atom_Influence_nloc)[ityp].ndc[count] =
            Atom_Influence_nloc_temp.ndc[i];
        (*Atom_Influence_nloc)[ityp].grid_pos[count] =
            (int *)malloc(sizeof(int) * ndc);
        for (j = 0; j < ndc; j++) {
          (*Atom_Influence_nloc)[ityp].grid_pos[count][j] =
              Atom_Influence_nloc_temp.grid_pos[i][j];
        }
        count++;
      }
      free(Atom_Influence_nloc_temp.grid_pos[i]);
  ...
  if (!rank)
    printf(GRN
           "rank = %d, time for nonlocal influencing atoms: %.3f ms\n" RESET,
           rank, (t2 - t1) * 1e3);
#endif
}
...
void CalculateNonlocalProjectors(SPARC_OBJ *pSPARC, NLOC_PROJ_OBJ **nlocProj,
  ...
  if (comm == MPI_COMM_NULL) {
    return; // upon input, make sure process with bandcomm_index < 0 provides
            // MPI_COMM_NULL
  }
...
#ifdef DEBUG
  if (rank == 0)
    printf("Calculate nonlocal projectors ... \n");
#endif
  int l, np, lcount, lcount2, m, psd_len, col_count, indx, ityp, iat, ipos, ndc,
      lloc, lmax, DMnx, DMny;
  int i_DM, j_DM, k_DM;
  double x0_i, y0_i, z0_i, *rc_pos_x, *rc_pos_y, *rc_pos_z, *rc_pos_r, *Ylm,
      *UdV_sort, x2, y2, z2, x, y, z;

  // number of nodes in the local distributed domain
  DMnx = DMVertices[1] - DMVertices[0] + 1;
  DMny = DMVertices[3] - DMVertices[2] + 1;

  (*nlocProj) = (NLOC_PROJ_OBJ *)malloc(sizeof(NLOC_PROJ_OBJ) *
                                        pSPARC->Ntypes); // TODO: deallocate!!
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    // allocate memory for projectors
    (*nlocProj)[ityp].Chi =
        (double **)malloc(sizeof(double *) * Atom_Influence_nloc[ityp].n_atom);
    ...
    for (iat = 0; iat < Atom_Influence_nloc[ityp].n_atom; iat++) {
      ...
      (*nlocProj)[ityp].Chi[iat] =
          (double *)malloc(sizeof(double) * ndc * (*nlocProj)[ityp].nproj);
      ...
      for (l = 0; l <= lmax; l++) {
        ...
        for (np = 0; np < pSPARC->psd[ityp].ppl[l]; np++) {
          ...
          if (pSPARC->psd[ityp].is_r_uniform == 1) {
            SplineInterpUniform(pSPARC->psd[ityp].RadialGrid,
                                pSPARC->psd[ityp].UdV + lcount2 * psd_len,
                                psd_len, rc_pos_r, UdV_sort, ndc,
                                pSPARC->psd[ityp].SplineFitUdV +
                                    lcount * psd_len);
          } else {
            SplineInterpNonuniform(pSPARC->psd[ityp].RadialGrid,
                                   pSPARC->psd[ityp].UdV + lcount2 * psd_len,
                                   psd_len, rc_pos_r, UdV_sort, ndc,
                                   pSPARC->psd[ityp].SplineFitUdV +
                                       lcount * psd_len);
          ...
          for (m = -l; m <= l; m++) {
            ...
            RealSphericalHarmonic(ndc, rc_pos_x, rc_pos_y, rc_pos_z, rc_pos_r,
                                  l, m, Ylm);
            t2 = MPI_Wtime();
            t_tot += t2 - t1;
            ...
            for (ipos = 0; ipos < ndc; ipos++) {
              (*nlocProj)[ityp].Chi[iat][col_count * ndc + ipos] =
                  Ylm[ipos] * UdV_sort[ipos];
            }
            col_count++;
          }
          lcount++;
 ...
 * @brief   Calculate nonlocal projectors.
 */
void CalculateNonlocalProjectors_kpt(
    SPARC_OBJ *pSPARC, NLOC_PROJ_OBJ **nlocProj,
    ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_nloc, int *DMVertices,
    MPI_Comm comm) {
  // processors that are not in the dmcomm will continue
  if (comm == MPI_COMM_NULL) {
    return; // upon input, make sure process with bandcomm_index < 0 provides
  ...
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    // allocate memory for projectors
    (*nlocProj)[ityp].Chi_c = (double _Complex **)malloc(
        sizeof(double _Complex *) * Atom_Influence_nloc[ityp].n_atom);
    lloc = pSPARC->localPsd[ityp]; // local projector index
    lmax = pSPARC->psd[ityp].lmax;
    psd_len = pSPARC->psd[ityp].size;
    // number of projectors per atom
    ...
    for (iat = 0; iat < Atom_Influence_nloc[ityp].n_atom; iat++) {
      ...
      // grid nodes in (spherical) rc-domain
      ndc = Atom_Influence_nloc[ityp].ndc[iat];
      (*nlocProj)[ityp].Chi_c[iat] = (double _Complex *)malloc(
          sizeof(double _Complex) * ndc * (*nlocProj)[ityp].nproj);
      rc_pos_x = (double *)malloc(sizeof(double) * ndc);
      rc_pos_y = (double *)malloc(sizeof(double) * ndc);
      rc_pos_z = (double *)malloc(sizeof(double) * ndc);
      rc_pos_r = (double *)malloc(sizeof(double) * ndc);
      ...
      for (l = 0; l <= lmax; l++) {
        ...
        for (np = 0; np < pSPARC->psd[ityp].ppl[l]; np++) {
          ...
          for (m = -l; m <= l; m++) {
            ...
            // calculate Chi = UdV * Ylm
            for (ipos = 0; ipos < ndc; ipos++) {
              (*nlocProj)[ityp].Chi_c[iat][col_count * ndc + ipos] =
                  Ylm[ipos] * UdV_sort[ipos];
            }
            col_count++;
          }
 ...
 *
 *          We will store the inner product < Chi_Jlm, x_n > in a continuous
 * array "alpha", the dimensions are in the order: <lm>, n, J. Here we find out
 * the sizes of the inner product corresponding to atom J, and the total number
 * of inner products corresponding to each vector x_n.
 */
 ...
 */
void Vnl_vec_mult(const SPARC_OBJ *pSPARC, int DMnd,
                  ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_nloc,
                  NLOC_PROJ_OBJ *nlocProj, int ncol, double *x, double *Hx,
                  MPI_Comm comm) {
  int i, n, np, count;
  ...
  alpha =
      (double *)calloc(pSPARC->IP_displ[pSPARC->n_atom] * ncol, sizeof(double));
  // first find inner product
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    // int lloc = pSPARC->localPsd[ityp];
    // lmax = pSPARC->psd[ityp].lmax;
    if (!nlocProj[ityp].nproj)
      continue; // this is typical for hydrogen
    for (iat = 0; iat < Atom_Influence_nloc[ityp].n_atom; iat++) {
      ndc = Atom_Influence_nloc[ityp].ndc[iat];
      ...
      for (n = 0; n < ncol; n++) {
        for (i = 0; i < ndc; i++) {
          x_rc[n * ndc + i] =
              x[n * DMnd + Atom_Influence_nloc[ityp].grid_pos[iat][i]];
        }
      }
      cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, nlocProj[ityp].nproj,
                  ncol, ndc, pSPARC->dV, nlocProj[ityp].Chi[iat], ndc, x_rc,
                  ndc, 1.0, alpha + pSPARC->IP_displ[atom_index] * ncol,
                  nlocProj[ityp].nproj);
      free(x_rc);
    }
  }

  // if there are domain parallelization over each band, we need to sum over all
  // processes over domain comm
  int commsize;
  MPI_Comm_size(comm, &commsize);
  if (commsize > 1) {
    MPI_Allreduce(MPI_IN_PLACE, alpha, pSPARC->IP_displ[pSPARC->n_atom] * ncol,
  ...
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    ...
    for (iat = 0; iat < Atom_Influence_nloc[ityp].n_atom; iat++) {
      ...
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, ndc, ncol,
                  nlocProj[ityp].nproj, 1.0, nlocProj[ityp].Chi[iat], ndc,
                  alpha + pSPARC->IP_displ[atom_index] * ncol,
                  nlocProj[ityp].nproj, 0.0, Vnlx, ndc);
      for (n = 0; n < ncol; n++) {
        for (i = 0; i < ndc; i++) {
          Hx[n * DMnd + Atom_Influence_nloc[ityp].grid_pos[iat][i]] +=
              Vnlx[n * ndc + i];
        }
      }
      free(Vnlx);
 ...
 */
void Vnl_vec_mult_kpt(const SPARC_OBJ *pSPARC, int DMnd,
                      ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_nloc,
                      NLOC_PROJ_OBJ *nlocProj, int ncol, double _Complex *x,
                      double _Complex *Hx, int kpt, MPI_Comm comm) {
  int i, n, np, count;
  /* compute nonlocal operator times vector(s) */
  int ityp, iat, l, m, ldispl, lmax, ndc, atom_index;
  double x0_i, y0_i, z0_i;
  double _Complex *alpha, *x_rc, *Vnlx;
  alpha = (double _Complex *)calloc(pSPARC->IP_displ[pSPARC->n_atom] * ncol,
  ...
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    ...
    for (iat = 0; iat < Atom_Influence_nloc[ityp].n_atom; iat++) {
      ...
      y0_i = Atom_Influence_nloc[ityp].coords[iat * 3 + 1];
      z0_i = Atom_Influence_nloc[ityp].coords[iat * 3 + 2];
      theta = -k1 * (floor(x0_i / Lx) * Lx) - k2 * (floor(y0_i / Ly) * Ly) -
              k3 * (floor(z0_i / Lz) * Lz);
      ...
      cblas_zgemm(CblasColMajor, CblasTrans, CblasNoTrans, nlocProj[ityp].nproj,
                  ncol, ndc, &a, nlocProj[ityp].Chi_c[iat], ndc, x_rc, ndc, &b,
                  alpha + pSPARC->IP_displ[atom_index] * ncol,
                  nlocProj[ityp].nproj);
      free(x_rc);
    }
  ...
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    ...
    for (iat = 0; iat < Atom_Influence_nloc[ityp].n_atom; iat++) {
      ...
      cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, ndc, ncol,
                  nlocProj[ityp].nproj, &bloch_fac, nlocProj[ityp].Chi_c[iat],
                  ndc, alpha + pSPARC->IP_displ[atom_index] * ncol,
                  nlocProj[ityp].nproj, &b, Vnlx, ndc);
      for (n = 0; n < ncol; n++) {
        for (i = 0; i < ndc; i++) {
          Hx[n * DMnd + Atom_Influence_nloc[ityp].grid_pos[iat][i]] +=
              Vnlx[n * ndc + i];
