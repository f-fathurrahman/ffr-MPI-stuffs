...
void Free_basic(SPARC_OBJ *pSPARC) {
  ...
  if (pSPARC->dmcomm_phi != MPI_COMM_NULL) {
    ...
    free(pSPARC->e_xc);
    if (strcmpi(pSPARC->XC, "GGA_PBE") == 0 || strcmpi(pSPARC->XC, "GGA_RPBE") == 0 ||
        strcmpi(pSPARC->XC, "GGA_PBEsol") == 0 || strcmpi(pSPARC->XC, "PBE0") == 0 || strcmpi(pSPARC->XC, "HF") == 0 ||
        strcmpi(pSPARC->XC, "HSE") == 0 || strcmpi(pSPARC->XC, "SCAN") == 0 || strcmpi(pSPARC->XC, "vdWDF1") == 0 ||
...
void Free_SPARC(SPARC_OBJ *pSPARC) {
  ...
  if (((pSPARC->chefsibound_flag == 0 || pSPARC->chefsibound_flag == 1) && pSPARC->spincomm_index >= 0 &&
       pSPARC->kptcomm_index >= 0 && (pSPARC->spin_typ != 0 || !pSPARC->is_phi_eq_kpt_topo || !pSPARC->isGammaPoint)) ||
      (pSPARC->usefock > 0)) {
  ...
  if (pSPARC->isGammaPoint) {
    ...
    if (pSPARC->dmcomm != MPI_COMM_NULL && pSPARC->bandcomm_index >= 0) {
      ...
      if (pSPARC->SOC_Flag == 1) {
        for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
          // if (! pSPARC->nlocProj[ityp].nproj) continue;
          if (!pSPARC->nlocProj[ityp].nprojso)
            continue;
          for (iat = 0; iat < pSPARC->Atom_Influence_nloc[ityp].n_atom; iat++) {
        ...
        for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
          // if (! pSPARC->nlocProj[ityp].nproj) continue;
          if (!pSPARC->nlocProj[ityp].nprojso_ext)
            continue;
          for (iat = 0; iat < pSPARC->Atom_Influence_nloc[ityp].n_atom; iat++) {
    ...
    if (pSPARC->kptcomm_topo != MPI_COMM_NULL && pSPARC->kptcomm_index >= 0) {
      ...
      if (pSPARC->SOC_Flag == 1) {
        for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
          // if (! pSPARC->nlocProj[ityp].nproj) continue;
          if (!pSPARC->nlocProj_kptcomm[ityp].nprojso)
            continue;
          for (iat = 0; iat < pSPARC->Atom_Influence_nloc_kptcomm[ityp].n_atom; iat++) {
        ...
        for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
          // if (! pSPARC->nlocProj[ityp].nproj) continue;
          if (!pSPARC->nlocProj_kptcomm[ityp].nprojso_ext)
            continue;
          for (iat = 0; iat < pSPARC->Atom_Influence_nloc_kptcomm[ityp].n_atom; iat++) {
