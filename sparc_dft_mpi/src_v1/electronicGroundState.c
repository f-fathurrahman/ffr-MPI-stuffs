...
void Calculate_electronicGroundState(SPARC_OBJ *pSPARC) {
...
#ifdef DEBUG
  if (rank == 0)
    printf("Start ground-state calculation.\n");
#endif
  ...
  if (rank == 0) {
    ...
    if (pSPARC->REFERENCE_CUTOFF < pSPARC->delta_x || pSPARC->REFERENCE_CUTOFF < pSPARC->delta_y ||
        pSPARC->REFERENCE_CUTOFF < pSPARC->delta_z) {
      printf("\nWARNING: REFERENCE _CUFOFF (%.6f Bohr) < MESH_SPACING (dx %.6f Bohr, dy %.6f Bohr, dz %.6f Bohr) in "
             "SCF#%d\n",
  ...
  if (!rank && pSPARC->Verbosity) {
    ...
    for (i = 0; i < pSPARC->n_atom; i++) {
      temp = fabs(sqrt(pow(pSPARC->forces[3 * i], 2.0) + pow(pSPARC->forces[3 * i + 1], 2.0) +
  ...
  if (pSPARC->Calc_stress == 1) {
    ...
    // move this function into Calculate_electronic_pressure?
    t2 = MPI_Wtime();
    if (!rank && pSPARC->Verbosity) {
      output_fp = fopen(pSPARC->OutFilename, "a");
      if (output_fp == NULL) {
  ...
  if (pSPARC->MDFlag == 1 || pSPARC->RelaxFlag == 1) {
    ...
    if (pSPARC->cell_typ != 0) {
      for (i = 0; i < pSPARC->n_atom; i++)
        nonCart2Cart_coord(pSPARC, &pSPARC->atom_pos[3 * i], &pSPARC->atom_pos[3 * i + 1],
  ...
  if (pSPARC->PrintElecDensFlag == 1) {
    ...
    t2 = MPI_Wtime();
    if (rank == 0)
      printf("Time for printing density: %.3f ms\n", (t2 - t1) * 1e3);
#endif
  }
  ...
  if (pSPARC->PrintEigenFlag == 1) {
    ...
    t2 = MPI_Wtime();
    if (rank == 0)
      printf("Time for printing eigenvalues: %.3f ms\n", (t2 - t1) * 1e3);
#endif
  }
  ...
  if (pSPARC->PrintPsiFlag[0] == 1) {
    ...
    t2 = MPI_Wtime();
    if (rank == 0)
      printf("Time for printing orbitals: %.3f ms\n", (t2 - t1) * 1e3);
#endif
  }
  ...
  if (pSPARC->PrintEnergyDensFlag == 1) {
    ...
    t2 = MPI_Wtime();
    if (rank == 0)
      printf("Time for printing energy density: %.3f ms\n", (t2 - t1) * 1e3);
#endif
  }
}
...
void Calculate_EGS_elecDensEnergy(SPARC_OBJ *pSPARC) {
  ...
  if (pSPARC->SQFlag == 1) {
    ...
    GetInfluencingAtoms_nloc(pSPARC, &pSPARC->Atom_Influence_nloc_kptcomm, pSQ->DMVertices_PR, pSQ->dmcomm_SQ);
    ...
    if (pSPARC->isGammaPoint)
      CalculateNonlocalProjectors(pSPARC, &pSPARC->nlocProj, pSPARC->Atom_Influence_nloc, pSPARC->DMVertices_dmcomm,
                                  pSPARC->bandcomm_index < 0 ? MPI_COMM_NULL : pSPARC->dmcomm);
    else
      CalculateNonlocalProjectors_kpt(pSPARC, &pSPARC->nlocProj, pSPARC->Atom_Influence_nloc, pSPARC->DMVertices_dmcomm,
                                      pSPARC->bandcomm_index < 0 ? MPI_COMM_NULL : pSPARC->dmcomm);

    if (pSPARC->SOC_Flag) {
      CalculateNonlocalProjectors_SOC(pSPARC, pSPARC->nlocProj, pSPARC->Atom_Influence_nloc, pSPARC->DMVertices_dmcomm,
                                      pSPARC->bandcomm_index < 0 ? MPI_COMM_NULL : pSPARC->dmcomm);
  ...
  if (pSPARC->d3Flag == 1) {
    ...
    t2 = MPI_Wtime();
    if (rank == 0)
      printf("Time for D3 calculation:    %.3f (sec)\n", t2 - t1);
#endif
  }
}
...
void scf(SPARC_OBJ *pSPARC) {
  ...
  t2 = MPI_Wtime();
  if (rank == 0)
    printf("rank = %d, XC calculation took %.3f ms\n", rank, (t2 - t1) * 1e3);
  t1 = MPI_Wtime();
#endif
  ...
  if (pSPARC->SQFlag == 1) {
    ...
    for (i = 0; i < pSPARC->Nspin; i++)
      Transfer_Veff_loc(pSPARC, pSPARC->Veff_loc_dmcomm_phi + i * DMnd,
...
void scf_loop(SPARC_OBJ *pSPARC) {
  ...
  while (SCFcount < pSPARC->MAXIT_SCF) {
    ...
    if (pSPARC->MixingVariable == 1) { // potential mixing
      ...
      t2 = MPI_Wtime();
      if (!rank)
        printf("rank = %d, Solving poisson equation took %.3f ms\n", rank, (t2 - t1) * 1e3);
      t1 = MPI_Wtime();
#endif
      ...
      t2 = MPI_Wtime();
      if (!rank)
        printf("rank = %d, Calculating Vxc took %.3f ms\n", rank, (t2 - t1) * 1e3);
#endif
    ...
    if (pSPARC->MixingVariable == 1) { // potential mixing
      ...
      Escc = Calculate_Escc(pSPARC, NspinDMnd, pSPARC->Veff_loc_dmcomm_phi, pSPARC->Veff_loc_dmcomm_phi_in,
                            pSPARC->electronDens + sindx_rho, pSPARC->dmcomm_phi);
      pSPARC->Escc = Escc;
...
// Apply mixing and preconditioner, if required
    ...
    t2 = MPI_Wtime();
    if (!rank)
      printf("rank = %d, Mixing (+ precond) took %.3f ms\n", rank, (t2 - t1) * 1e3);
#endif
    ...
    if (!rank) {
      ...
      if (pSPARC->spin_typ == 0)
        ...
        fprintf(output_fp, "%-6d      %18.10E        %11.4E        %.3E        %.3f\n", SCFcount,
                pSPARC->Etot / pSPARC->n_atom, pSPARC->netM, error, t_cum_scf);
      fclose(output_fp);
      t_cum_scf = 0.0;
    }

    // STOP scf if scf error is less than the given tolerance
    if (scf_conv && SCFcount >= pSPARC->MINIT_SCF)
      break;
  }
  ...
  if (pSPARC->SQFlag == 1) {
...
#ifdef DEBUG
      if (!rank)
        printf("Max eigenvalue and corresponding occupation number: %.15f, %.15f\n", maxeig, occ_maxeig);
#endif

      if (occ_maxeig > pSPARC->SQ_tol_occ) {
        if (!rank)
          printf("WARNING: Occupation number corresponding to maximum "
        ...
        if (!rank) {
          ...
          for (i = 0; i < nocc_print; i++) {
            printf("lambda[%4d] = %18.14f, occ[%4d] = %18.14f\n", Ns - nocc_print + i + 1,
                   pSPARC->lambda_sorted[spn_i * Ns * Nk + k * Ns + Ns - nocc_print + i], Ns - nocc_print + i + 1,
                   (3.0 - pSPARC->Nspin) / pSPARC->Nspinor *
                       pSPARC->occ_sorted[spn_i * Ns * Nk + k * Ns + Ns - nocc_print + i]);
          }
        }
      }
    }
    ...
    if (!rank) {
      ...
      printf("Occupation of state %d (90%%) = %.15f.\n"
             ...
             ind_90percent + 1, (3.0 - pSPARC->Nspin) / pSPARC->Nspinor * g_ind_90percent, ind_100percent + 1,
             (3.0 - pSPARC->Nspin) / pSPARC->Nspinor * g_ind_100percent);
    }
#endif
  }
  ...
  if (error > TOL) {
    if (!rank) {
      printf("WARNING: SCF#%d did not converge to desired accuracy!\n",
             pSPARC->MDFlag ? pSPARC->MDCount + pSPARC->restartCount + (pSPARC->RestartFlag == 0)
                            : pSPARC->RelaxCount + pSPARC->restartCount + (pSPARC->RestartFlag == 0));
      // write to .out file
      output_fp = fopen(pSPARC->OutFilename, "a");
      if (output_fp == NULL) {
...
void Evaluate_scf_error(SPARC_OBJ *pSPARC, double *scf_error, int *scf_conv) {
  ...
  // output
  *scf_error = error;
  *scf_conv = (pSPARC->usefock % 2 == 1) ? ((int)(error < pSPARC->TOL_SCF_INIT)) : ((int)(error < pSPARC->TOL_SCF));
}
...
void Evaluate_QE_scf_error(SPARC_OBJ *pSPARC, double *scf_error, int *scf_conv) {
  ...
  if (pSPARC->MixingVariable == 0) { // desity mixing
    ...
    t2 = MPI_Wtime();
    if (!rank)
      printf("QE scf error: update phi_out took %.3f ms\n", (t2 - t1) * 1e3);
...
void Update_mixing_hist_xk(SPARC_OBJ *pSPARC, double veff_mean) {
  ...
  if (pSPARC->spin_typ == 0) {
    if (pSPARC->MixingVariable == 0) { // density mixing
      for (i = 0; i < pSPARC->Nd_d; i++) {
        pSPARC->mixing_hist_xk[i] = pSPARC->mixing_hist_xkm1[i] = pSPARC->electronDens[i];
      }
    } else if (pSPARC->MixingVariable == 1) { // potential mixing
      for (i = 0; i < pSPARC->Nd_d; i++) {
        pSPARC->mixing_hist_xk[i] = pSPARC->mixing_hist_xkm1[i] = pSPARC->Veff_loc_dmcomm_phi[i] - veff_mean;
      }
    }
  } else {
    if (pSPARC->MixingVariable == 0) { // density mixing
      for (i = 0; i < 2 * pSPARC->Nd_d; i++) {
        pSPARC->mixing_hist_xk[i] = pSPARC->mixing_hist_xkm1[i] = pSPARC->electronDens[pSPARC->Nd_d + i];
      }
    } else if (pSPARC->MixingVariable == 1) { // potential mixing
      for (i = 0; i < 2 * pSPARC->Nd_d; i++) {
        pSPARC->mixing_hist_xk[i] = pSPARC->mixing_hist_xkm1[i] = pSPARC->Veff_loc_dmcomm_phi[i] - veff_mean;
...
void Transfer_Veff_loc(SPARC_OBJ *pSPARC, double *Veff_phi_domain, double *Veff_psi_domain) {
...
#ifdef DEBUG
  if (rank == 0)
    printf("Transmitting Veff_loc from phi-domain to psi-domain (LOCAL) ...\n");
#endif
  ...
  D2D(&pSPARC->d2d_dmcomm_phi, &pSPARC->d2d_dmcomm, gridsizes, pSPARC->DMVertices, Veff_phi_domain,
      ...
      (pSPARC->spincomm_index == 0 && pSPARC->kptcomm_index == 0 && pSPARC->bandcomm_index == 0) ? pSPARC->dmcomm
                                                                                                 : MPI_COMM_NULL,
      rdims, MPI_COMM_WORLD);
#ifdef DEBUG
  t2 = MPI_Wtime();
  if (rank == 0)
    printf("---Transfer Veff_loc: D2D took %.3f ms\n", (t2 - t1) * 1e3);
  t1 = MPI_Wtime();
#endif
  ...
  t2 = MPI_Wtime();
  if (rank == 0)
    printf("---Transfer Veff_loc: bcast btw/ spincomms of 1st kptcomm took %.3f ms\n", (t2 - t1) * 1e3);
  t1 = MPI_Wtime();
#endif

  // Broadcast phi from the dmcomm that contain root process to all dmcomms of the first bandcomms in each kptcomm
  if (pSPARC->spincomm_index >= 0 && pSPARC->npkpt > 1 && pSPARC->kptcomm_index >= 0 && pSPARC->bandcomm_index == 0 &&
  ...
  t2 = MPI_Wtime();
  if (rank == 0)
    printf("---Transfer Veff_loc: bcast btw/ kptcomms of 1st bandcomm took %.3f ms\n", (t2 - t1) * 1e3);
#endif

  MPI_Barrier(pSPARC->blacscomm); // experienced severe slowdown of MPI_Bcast below on Quartz cluster, this Barrier
                                  // fixed the issue (why?)
  ...
  MPI_Barrier(pSPARC->blacscomm); // experienced severe slowdown of MPI_Bcast above on Quartz cluster, this Barrier
                                  // fixed the issue (why?)

#ifdef DEBUG
  t2 = MPI_Wtime();
  if (rank == 0)
    printf("---Transfer Veff_loc: mpi_bcast (count = %d) to all bandcomms took %.3f ms\n", pSPARC->Nd_d_dmcomm,
           (t2 - t1) * 1e3);
#endif
}
...
void TransferDensity(SPARC_OBJ *pSPARC, double *rho_send, double *rho_recv) {
  ...
  t1 = MPI_Wtime();
#endif
  D2D(&pSPARC->d2d_dmcomm, &pSPARC->d2d_dmcomm_phi, gridsizes, pSPARC->DMVertices_dmcomm, rho_send, pSPARC->DMVertices,
      rho_recv,
      (pSPARC->spincomm_index == 0 && pSPARC->kptcomm_index == 0 && pSPARC->bandcomm_index == 0) ? pSPARC->dmcomm
                                                                                                 : MPI_COMM_NULL,
