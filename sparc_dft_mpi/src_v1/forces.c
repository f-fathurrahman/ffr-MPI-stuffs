...
void Calculate_EGS_Forces(SPARC_OBJ *pSPARC) {
  ...
  t2 = MPI_Wtime();
  if (!rank)
    printf("Time for calculating local force components: %.3f ms\n", (t2 - t1) * 1e3);
  t1 = MPI_Wtime();
#endif
  ...
  t2 = MPI_Wtime();
  if (!rank)
    printf("Time for calculating nonlocal force components: %.3f ms\n", (t2 - t1) * 1e3);
#endif
  ...
  if (pSPARC->cell_typ > 10 && pSPARC->cell_typ < 20) {
    for (i = 0; i < pSPARC->n_atom; i++) {
      nonCart2Cart_grad(pSPARC, pSPARC->forces + 3 * i, pSPARC->forces + 3 * i + 1,
                        pSPARC->forces + 3 * i + 2);
    }
  }
  ...
  if (pSPARC->NLCC_flag) {
    ...
    if (rank == 0) {
      ...
      for (i = 0; i < pSPARC->n_atom; i++) {
        printf(GRN "%18.14f %18.14f %18.14f\n" RESET, forces_xc[3 * i], forces_xc[3 * i + 1],
               forces_xc[3 * i + 2]);
      }
    }
    t2 = MPI_Wtime();
    if (!rank)
      printf("Time for calculating XC forces components: %.3f ms\n", (t2 - t1) * 1e3);
#endif
    free(forces_xc);
  }
  ...
  if (!rank) {
    ...
    for (i = 0; i < pSPARC->n_atom; i++) {
      printf("%18.14f %18.14f %18.14f\n", pSPARC->forces[i * 3], pSPARC->forces[i * 3 + 1],
             pSPARC->forces[i * 3 + 2]);
    }
  }
#endif
...
void Calculate_nonlocal_forces_linear(SPARC_OBJ *pSPARC) {
  ...
  int i, n, np, ldispl, ndc, ityp, iat, ncol, DMnd, dim, atom_index, count, l, m, lmax, spn_i,
      nspin, size_s;
...
#ifdef DEBUG
  if (!rank)
    printf("Start Calculating nonlocal forces\n");
#endif
  count = 0;
    ...
    for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
      ...
      for (iat = 0; iat < pSPARC->Atom_Influence_nloc[ityp].n_atom; iat++) {
        ...
        for (n = 0; n < ncol; n++) {
          ...
          for (i = 0; i < ndc; i++) {
            // x_rc[n*ndc+i] =
            // pSPARC->Xorb[n*DMnd+pSPARC->Atom_Influence_nloc[ityp].grid_pos[iat][i]];
            *(x_rc_ptr + i) = *(x_ptr + pSPARC->Atom_Influence_nloc[ityp].grid_pos[iat][i]);
          }
        }
        cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->nlocProj[ityp].nproj, ncol,
                    ndc, pSPARC->dV, pSPARC->nlocProj[ityp].Chi[iat], ndc, x_rc, ndc, 1.0,
                    beta + pSPARC->IP_displ[atom_index] * ncol,
                    pSPARC->nlocProj[ityp].nproj); // multiply dV to get inner-product
        // cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, ncol, pSPARC->nlocProj[ityp].nproj,
        // ndc, pSPARC->dV, x_rc, ndc,
        //            pSPARC->nlocProj[ityp].Chi[iat], ndc, 1.0,
        //            alpha+pSPARC->IP_displ[atom_index]*ncol, ncol); // this calculates <Psi_n,
        //            Chi_Jlm>
        free(x_rc);
        ...
        //     pSPARC->Xorb, pSPARC->Yorb, dim, pSPARC->dmcomm); beta = alpha +
        //     pSPARC->IP_displ[pSPARC->n_atom]*ncol*(dim+1); atom_index =
        //     pSPARC->Atom_Influence_nloc[ityp].atom_index[iat]; for (n = 0; n < ncol; n++) {
        //         dx_ptr = pSPARC->Yorb + n * DMnd;
        //         dx_rc_ptr = dx_rc + n * ndc;
        //         for (i = 0; i < ndc; i++) {
        //             // dx_rc[n*ndc+i] =
        //             pSPARC->Yorb[n*DMnd+pSPARC->Atom_Influence_nloc[ityp].grid_pos[iat][i]];
        //             *(dx_rc_ptr + i) = *(dx_ptr +
        //             pSPARC->Atom_Influence_nloc[ityp].grid_pos[iat][i]);
        //         }
        //     }
        //     /* Note: in principle we need to multiply dV to get inner-product, however, since Psi
        //     is normalized
        //     *       in the l2-norm instead of L2-norm, each psi value has to be multiplied by
        //     1/sqrt(dV) to
        //     *       recover the actual value. Considering this, we only multiply dV in one of the
        //     inner product
        //     *       and the other dV is canceled by the product of two scaling factors,
        //     1/sqrt(dV) and 1/sqrt(dV).
        //     */
        //     cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->nlocProj[ityp].nproj,
        //     ncol, ndc, 1.0, pSPARC->nlocProj[ityp].Chi[iat], ndc, dx_rc, ndc, 1.0,
        //     beta+pSPARC->IP_displ[atom_index]*ncol, pSPARC->nlocProj[ityp].nproj);
        // }
        // free(dx_rc);
      }
    }
  ...
  for (dim = 0; dim < 3; dim++) {
    ...
    for (spn_i = 0; spn_i < nspin; spn_i++) {
      // find dPsi in direction dim
      Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, ncol, 0.0,
                           pSPARC->Xorb + spn_i * size_s, pSPARC->Yorb, dim, pSPARC->dmcomm);
      ...
      for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
        ...
        for (iat = 0; iat < pSPARC->Atom_Influence_nloc[ityp].n_atom; iat++) {
          ...
          /* Note: in principle we need to multiply dV to get inner-product, however, since Psi is
           * normalized in the l2-norm instead of L2-norm, each psi value has to be multiplied by
           * 1/sqrt(dV) to recover the actual value. Considering this, we only multiply dV in one of
           * the inner product and the other dV is canceled by the product of two scaling factors,
           * 1/sqrt(dV) and 1/sqrt(dV).
           */
          cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->nlocProj[ityp].nproj, ncol,
                      ndc, 1.0, pSPARC->nlocProj[ityp].Chi[iat], ndc, dx_rc, ndc, 1.0,
                      beta + pSPARC->IP_displ[atom_index] * ncol, pSPARC->nlocProj[ityp].nproj);
          free(dx_rc);
        }
      }
  ...
  if (pSPARC->npNd > 1) {
    MPI_Allreduce(MPI_IN_PLACE, alpha, pSPARC->IP_displ[pSPARC->n_atom] * ncol * nspin * 4,
  ...
  // sum over all spin
  if (pSPARC->npspin > 1) {
    MPI_Allreduce(MPI_IN_PLACE, force_nloc, 3 * pSPARC->n_atom, MPI_DOUBLE, MPI_SUM,
  ...
  if (!rank) {
    printf("force_nloc = \n");
    for (i = 0; i < pSPARC->n_atom; i++) {
      printf("%18.14f %18.14f %18.14f\n", force_nloc[i * 3], force_nloc[i * 3 + 1],
             force_nloc[i * 3 + 2]);
    }
  }
  if (!rank) {
...
void Calculate_nonlocal_forces_kpt_linear(SPARC_OBJ *pSPARC) {
  ...
  int i, k, n, np, ldispl, ndc, ityp, iat, ncol, DMnd, dim, atom_index, count, l, m, lmax, kpt, Nk,
      size_k, spn_i, nspin, size_s;
  ncol = pSPARC->Nband_bandcomm; // number of bands assigned
  DMnd = pSPARC->Nd_d_dmcomm;
  ...
  size_k = DMnd * ncol;
  size_s = size_k * Nk;
  double _Complex *alpha, *beta, *x_ptr, *dx_ptr, *x_rc, *dx_rc, *x_rc_ptr, *dx_rc_ptr, *beta_x,
      *beta_y, *beta_z;
  double *force_nloc, fJ_x, fJ_y, fJ_z, val_x, val_y, val_z, val2_x, val2_y, val2_z, g_nk;

  force_nloc = (double *)calloc(3 * pSPARC->n_atom, sizeof(double));
  alpha = (double _Complex *)calloc(pSPARC->IP_displ[pSPARC->n_atom] * ncol * Nk * nspin * 4,
    ...
    for (kpt = 0; kpt < Nk; kpt++) {
      ...
      for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
        ...
        for (iat = 0; iat < pSPARC->Atom_Influence_nloc[ityp].n_atom; iat++) {
          ...
          cblas_zgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->nlocProj[ityp].nproj, ncol,
                      ndc, &a, pSPARC->nlocProj[ityp].Chi_c[iat], ndc, x_rc, ndc, &b,
                      beta + pSPARC->IP_displ[atom_index] * ncol,
                      pSPARC->nlocProj[ityp].nproj); // multiply dV to get inner-product
          // cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, ncol,
          // pSPARC->nlocProj[ityp].nproj, ndc, pSPARC->dV, x_rc, ndc,
          //            pSPARC->nlocProj[ityp].Chi[iat], ndc, 1.0,
          //            alpha+pSPARC->IP_displ[atom_index]*ncol, ncol); // this calculates <Psi_n,
          //            Chi_Jlm>
          free(x_rc);
    ...
    for (spn_i = 0; spn_i < nspin; spn_i++) {
      for (kpt = 0; kpt < Nk; kpt++) {
        ...
        Gradient_vectors_dir_kpt(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, ncol, 0.0,
                                 pSPARC->Xorb_kpt + spn_i * size_s + kpt * size_k, pSPARC->Yorb_kpt,
                                 dim, kpt_vec, pSPARC->dmcomm);
        ...
        for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
          ...
          for (iat = 0; iat < pSPARC->Atom_Influence_nloc[ityp].n_atom; iat++) {
            ...
            /* Note: in principle we need to multiply dV to get inner-product, however, since Psi is
             * normalized in the l2-norm instead of L2-norm, each psi value has to be multiplied by
             * 1/sqrt(dV) to recover the actual value. Considering this, we only multiply dV in one
             * of the inner product and the other dV is canceled by the product of two scaling
             * factors, 1/sqrt(dV) and 1/sqrt(dV).
             */
            cblas_zgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->nlocProj[ityp].nproj, ncol,
                        ndc, &bloch_fac, pSPARC->nlocProj[ityp].Chi_c[iat], ndc, dx_rc, ndc, &b,
                        beta + pSPARC->IP_displ[atom_index] * ncol, pSPARC->nlocProj[ityp].nproj);
  ...
  if (pSPARC->npNd > 1) {
    MPI_Allreduce(MPI_IN_PLACE, alpha, pSPARC->IP_displ[pSPARC->n_atom] * ncol * Nk * nspin * 4,
          ...
          for (n = pSPARC->band_start_indx; n <= pSPARC->band_end_indx; n++) {
            ...
            for (l = 0; l <= lmax; l++) {
              ...
              for (np = 0; np < pSPARC->psd[ityp].ppl[l]; np++) {
                val_x = val_y = val_z = 0.0;
                for (m = -l; m <= l; m++) {
                  val_x += creal(alpha[count]) * creal(beta_x[count]) -
                           cimag(alpha[count]) * cimag(beta_x[count]);
                  val_y += creal(alpha[count]) * creal(beta_y[count]) -
                           cimag(alpha[count]) * cimag(beta_y[count]);
                  val_z += creal(alpha[count]) * creal(beta_z[count]) -
                           cimag(alpha[count]) * cimag(beta_z[count]);
                  count++;
                }
                val2_x += val_x * pSPARC->psd[ityp].Gamma[ldispl + np];
                val2_y += val_y * pSPARC->psd[ityp].Gamma[ldispl + np];
 ...
 */
void Calculate_local_forces(SPARC_OBJ *pSPARC) { Calculate_local_forces_linear(pSPARC); }
...
void Calculate_local_forces_linear(SPARC_OBJ *pSPARC) {
  if (pSPARC->dmcomm_phi == MPI_COMM_NULL)
    return; // consider broadcasting the force components or force residual

  int ityp, iat, i, j, k, p, ip, jp, kp, i_DM, j_DM, k_DM, FDn, count, count_interp, DMnx, DMny,
      DMnd, nx, ny, nz, nd, nxp, nyp, nzp, nd_ex, icor, jcor, kcor, atom_index, *ind_interp, dK, dJ,
      dI;
  double x0_i, y0_i, z0_i, x0_i_shift, y0_i_shift, z0_i_shift, x, y, z, *R, *VJ, *VJ_ref, *VcJ,
      DVcJ_x_val, DVcJ_y_val, DVcJ_z_val, force_x, force_y, force_z, force_corr_x, force_corr_y,
      force_corr_z, *R_interp, *VJ_interp;
...
#ifdef DEBUG
  if (!rank)
    printf("Start calculating local components of forces ...\n");
#endif
  double t1, t2, t_sort = 0.0;
  double *Lap_wt, *Lap_stencil;

  FDn = pSPARC->order / 2;
  w2_diag = (pSPARC->D2_stencil_coeffs_x[0] + pSPARC->D2_stencil_coeffs_y[0] +
  ...
  DMnx = pSPARC->Nx_d;
  DMny = pSPARC->Ny_d; // DMnz = pSPARC->Nz_d;
  DMnd = pSPARC->Nd_d;
  ...
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, pSPARC->elecstPotential, Dphi_x, 0,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, pSPARC->elecstPotential, Dphi_y, 1,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, pSPARC->elecstPotential, Dphi_z, 2,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, pSPARC->Vc, DVc_x, 0,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, pSPARC->Vc, DVc_y, 1,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, pSPARC->Vc, DVc_z, 2,
                       pSPARC->dmcomm_phi);
  ...
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    ...
    for (iat = 0; iat < pSPARC->Atom_Influence_local[ityp].n_atom; iat++) {
      ...
      nx = pSPARC->Atom_Influence_local[ityp].xe[iat] - pSPARC->Atom_Influence_local[ityp].xs[iat] +
           1;
      ny = pSPARC->Atom_Influence_local[ityp].ye[iat] - pSPARC->Atom_Influence_local[ityp].ys[iat] +
           1;
      nz = pSPARC->Atom_Influence_local[ityp].ze[iat] - pSPARC->Atom_Influence_local[ityp].zs[iat] +
           1;
      nd = nx * ny * nz;
      ...
      if (pSPARC->cell_typ == 0) {
        ...
        for (k = 0; k < nzp; k++) {
          ...
          for (j = 0; j < nyp; j++) {
            ...
            for (i = 0; i < nxp; i++) {
              x = i * pSPARC->delta_x - x0_i_shift;
              R[count] = sqrt(pSPARC->metricT[0] * (x * x) + pSPARC->metricT[1] * (x * y) +
      ...
      if (pSPARC->psd[ityp].is_r_uniform == 1) {
        SplineInterpUniform(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rVloc,
                            pSPARC->psd[ityp].size, R_interp, VJ_interp, count_interp,
                            pSPARC->psd[ityp].SplinerVlocD);
      } else {
        // SortSplineInterp(pSPARC->psd[ityp].RadialGrid,pSPARC->psd[ityp].rVloc,
        // pSPARC->psd[ityp].size,
        //                  R_interp, VJ_interp, count_interp, pSPARC->psd[ityp].SplinerVlocD);
        SplineInterpNonuniform(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rVloc,
                               pSPARC->psd[ityp].size, R_interp, VJ_interp, count_interp,
                               pSPARC->psd[ityp].SplinerVlocD);
      }
      ...
      Calc_lapV(pSPARC, VJ_ref, FDn, nxp, nyp, nzp, nx, ny, nz, Lap_wt, w2_diag, 0.0, -inv_4PI,
                bJ_ref);
      ...
      for (k = 0; k < nz; k++) {
        ...
        for (j = 0; j < ny; j++) {
          ...
          for (i = 0; i < nx; i++) {
            ...
            for (p = 1; p <= FDn; p++) {
              DVcJ_x_val +=
                  (VcJ[ishift_p + p] - VcJ[ishift_p - p]) * pSPARC->D1_stencil_coeffs_x[p];
              DVcJ_y_val += (VcJ[ishift_p + pshifty_ex[p]] - VcJ[ishift_p - pshifty_ex[p]]) *
                            pSPARC->D1_stencil_coeffs_y[p];
              DVcJ_z_val += (VcJ[ishift_p + pshiftz_ex[p]] - VcJ[ishift_p - pshiftz_ex[p]]) *
                            pSPARC->D1_stencil_coeffs_z[p];
            }
            // find integrals in the force expression
            double b_plus_b_ref =
                pSPARC->psdChrgDens[ishift_DM] + pSPARC->psdChrgDens_ref[ishift_DM];
  ...
  t1 = MPI_Wtime();
  // do Allreduce/Reduce to find total integral // TODO: check if there's only 1 process, then skip
  // this
  MPI_Allreduce(MPI_IN_PLACE, pSPARC->forces, 3 * pSPARC->n_atom, MPI_DOUBLE, MPI_SUM,
                pSPARC->dmcomm_phi);
  t2 = MPI_Wtime();
#ifdef DEBUG
  if (!rank)
    printf("time for sorting and interpolate pseudopotential: %.3f ms, time for Allreduce/Reduce: "
           "%.3f ms \n",
...
void Calculate_forces_xc(SPARC_OBJ *pSPARC, double *forces_xc) {
  ...
  int ityp, iat, i, j, k, p, ip, jp, kp, di, dj, dk, i_DM, j_DM, k_DM, FDn, count, count_interp,
      DMnx, DMny, nx, ny, nz, nxp, nyp, nzp, nd_ex, nx2p, ny2p, nz2p, nd_2ex, icor, jcor, kcor,
      *pshifty, *pshiftz, *pshifty_ex, *pshiftz_ex, *ind_interp;
  double x0_i, y0_i, z0_i, x0_i_shift, y0_i_shift, z0_i_shift, x, y, z, *R, *R_interp, *VJ_interp;
  double rchrg;
  double t1, t2, t_sort = 0.0;
  ...
  for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
    ...
    for (iat = 0; iat < pSPARC->Atom_Influence_local[ityp].n_atom; iat++) {
      ...
      nzp = nz + pSPARC->order;
      nd_ex = nxp * nyp * nzp; // total number of nodes
      // number of finite-difference nodes in each direction of extended_extended_rb (rb + order)
      // region
      nx2p = nxp + pSPARC->order;
      ny2p = nyp + pSPARC->order;
      nz2p = nzp + pSPARC->order;
      ...
      SplineInterpMain(pSPARC->psd[ityp].RadialGrid, pSPARC->psd[ityp].rho_c_table,
                       pSPARC->psd[ityp].size, R_interp, rhocJ_interp, count_interp,
                       pSPARC->psd[ityp].SplineRhocD, pSPARC->psd[ityp].is_r_uniform);
      t2 = MPI_Wtime();
      t_sort += t2 - t1;
      ...
      for (int k2p = FDn, kp = 0; k2p < nz2p - FDn; k2p++, kp++) {
        ...
        for (int j2p = FDn, jp = 0; j2p < ny2p - FDn; j2p++, jp++) {
          ...
          for (int i2p = FDn, ip = 0; i2p < nx2p - FDn; i2p++, ip++) {
            ...
            for (p = 1; p <= FDn; p++) {
              drhocJ_x_val +=
                  (rhocJ[ishift_2p + p] - rhocJ[ishift_2p - p]) * pSPARC->D1_stencil_coeffs_x[p];
              drhocJ_y_val +=
                  (rhocJ[ishift_2p + pshifty_ex[p]] - rhocJ[ishift_2p - pshifty_ex[p]]) *
                  pSPARC->D1_stencil_coeffs_y[p];
              drhocJ_z_val +=
                  (rhocJ[ishift_2p + pshiftz_ex[p]] - rhocJ[ishift_2p - pshiftz_ex[p]]) *
                  pSPARC->D1_stencil_coeffs_z[p];
            }
  ...
  MPI_Allreduce(MPI_IN_PLACE, forces_xc, 3 * pSPARC->n_atom, MPI_DOUBLE, MPI_SUM,
                pSPARC->dmcomm_phi);
