...
void simplifyPath(char *path, char *newpath, size_t newlen) {
  ...
  *tail = '\0';
  ...
  if (*head == '/')
    while (*(head + 1) == '/')
      head++;
  if (strlen(prefix) > 0 && prefix[strlen(prefix) - 1] == '/' && *head == '/')
    head++;
...
/**
 * @brief   The following code for factorizing an integer is copied from Rosetta
 * Code (https://rosettacode.org/wiki/Factors_of_an_integer#C).
 */
...
/**
 * @brief   Calculates derivatives of a tabulated function required for spline
 * interpolation.
 */
 ...
 */
void SplineInterp(double *X1, double *Y1, int len1, double *X2, double *Y2,
                  int len2, double *YD) {
  if (len2 <= 0)
    return;
  int i, j;
 ...
 */
void SortSplineInterp(double *X1, double *Y1, int len1, double *X2, double *Y2,
                      int len2, double *YD) {
  int i, *Ind_sort, len_interp;
  double *Y2_sort;

  // first sort X2 in ascending order
  // NOTE: we're using the memory of Y2 to store sorted X2 temporarily to save
  // memory!
  ...
  len_interp = len2;
  while (len_interp >= 1 && Y2[len_interp - 1] > X1[len1 - 1])
    len_interp--;

  // apply cubic spline interpolation to find Y2_sort, note here Y2 stores
  // sorted values of X2
 ...
 */
void SplineInterpUniform(double *X1, double *Y1, int len1, double *X2,
                         double *Y2, int len2, double *YD) {
  if (len2 <= 0 || len1 < 2)
    return;
  int i, j;
  double A0, A1, A2, A3, x, dx, dy, p1, p2, p3;
 ...
 */
void SplineInterpNonuniform(double *X1, double *Y1, int len1, double *X2,
                            double *Y2, int len2, double *YD) {
 ...
 *          routines.
 */
void SplineInterpMain(double *X1, double *Y1, int len1, double *X2, double *Y2,
                      int len2, double *YD, int isUniform) {
  if (isUniform)
    SplineInterpUniform(X1, Y1, len1, X2, Y2, len2, YD);
  else
    SplineInterpNonuniform(X1, Y1, len1, X2, Y2, len2, YD);
}

/**
 * @brief   Cubic spline coefficients.
 */
// void SplineCoeff(double *X,double *Y,double *YD,int len,double *A3,double
// *A2,double *A1,double *A0) { 	int j; 	double x,dx,dy; 	for (j = 0; j < len-1;
// j++) { 		dx = 1.0 / (X[j+1] - X[j]); 		dy = (Y[j+1] - Y[j]) * dx; 		A0[j] = Y[j];
// 		A1[j] = YD[j];
...
/**
 * @brief   Cubic spline coefficients, but without copying Y and YD into A0 and
 * A1, respectively.
 */
void SplineCoeff(double *X, double *Y, double *YD, int len, double *A3,
                 double *A2) {
  int j;
  double dx, dy;
  for (j = 0; j < len - 1; j++) {
 ...
 *
 */
void Vector2Norm(const double *Vec, const int len, double *vec_2norm,
                 MPI_Comm comm) {
  int k;
  double sqsum = 0.0;
  for (k = 0; k < len; k++)
 ...
 * @brief   Calculate global 2-norm of a vector among the given communicator.
 */
void Vector2Norm_complex(const double _Complex *Vec, const int len,
                         double *vec_2norm, MPI_Comm comm) {
  int k;
  double sqsum = 0.0;
  for (k = 0; k < len; k++)
 ...
 * @brief   Calculate global 2-norm of a vector among the given communicator.
 */
void VectorDotProduct(const double *Vec1, const double *Vec2, const int len,
                      double *vec_dot, MPI_Comm comm) {
  int i;
...
/**
 * @brief   Calculate global 2-norm of a vector with integration weight among
 * the given communicator.
 */
void VectorDotProduct_wt(const double *Vec1, const double *Vec2,
                         const double *Intgwt, const int len, double *vec_dot,
                         MPI_Comm comm) {
  int i;
  double vec_dot_loc = 0.0;
  for (i = 0; i < len; i++)
 ...
 * @brief   Calculate global 2-norm of a vector among the given communicator.
 */
void VectorDotProduct_complex(const double _Complex *Vec1,
                              const double _Complex *Vec2, const int len,
                              double *vec_dot, MPI_Comm comm) {
  int i;
  double vec_dot_loc = 0.0;
  for (i = 0; i < len; i++)
 ...
 */
void VectorSum(const double *Vec, const int len, double *vec_sum,
               MPI_Comm comm) {
...
/**
 * @brief   Create a random matrix with each entry a random number within given
 * range.
 *
 *          Note that each process within comm will have different random
 * entries.
 *
 ...
 * @param n     Number of columns of the local part of the matrix.
 */
void SetRandMat_complex(double _Complex *Mat, int m, int n, double rand_min,
                        double rand_max, MPI_Comm comm) {
  int rank, i, len_tot;
  MPI_Comm_rank(comm, &rank);
...
/**
 * @brief   Create a random vector with fixed seeds corresponding to it's global
 *index.
 *
 *			This function is to fix the random number seeds so that all
 *random numbers generated in parallel under MPI are the same as those generated
 *in sequential execution.
 */
void SeededRandVec(double *Vec, const int DMVert[6], const int gridsizes[3],
  ...
  for (k = 0, kg = DMVert[4]; k < DMnz; k++, kg++) {
    for (j = 0, jg = DMVert[2]; j < DMny; j++, jg++) {
      for (i = 0, ig = DMVert[0]; i < DMnx; i++, ig++) {
        ...
        Vec[index] =
            rand_min + (rand_max - rand_min) * ((double)rand() / RAND_MAX);
      }
    }
  }
}
 ...
 *in sequential execution.
 */
void SeededRandVec_complex(double _Complex *Vec, const int DMVert[6],
                           const int gridsizes[3], const double rand_min,
                           const double rand_max, const int seed_offset) {
  int shift_gz = gridsizes[0] * gridsizes[1];
  int shift_gy = gridsizes[0];
  int DMnx = DMVert[1] - DMVert[0] + 1;
  int DMny = DMVert[3] - DMVert[2] + 1;
...
/**
 * @brief   Create a perturbed unit constant vector distributed among the given
 * communicator.
 */
void SetUnitOnesVector(double *Vec, int n_loc, int n_global, double pert_fac,
                       MPI_Comm comm) {
  int i;
  double vscal;
  if (pert_fac == 0.0) {
 ...
 * @param origin_shift_i Shift of the origin of the above local index in the 1st
 * dir.
 * @param origin_shift_j Shift of the origin of the above local index in the 2nd
 * dir.
 * @param origin_shift_k Shift of the origin of the above local index in the 3rd
 * dir.
...
int is_grid_outside(const int ip, const int jp, const int kp,
                    const int origin_shift_i, const int origin_shift_j,
                    const int origin_shift_k, const int DMVerts[6],
                    const int gridsizes[3]) {
  int i_global = ip + origin_shift_i + DMVerts[0];
  int j_global = jp + origin_shift_j + DMVerts[2];
  int k_global = kp + origin_shift_k + DMVerts[4];
  int is_out = (i_global < 0) || (i_global >= gridsizes[0]) || (j_global < 0) ||
               (j_global >= gridsizes[1]) || (k_global < 0) ||
               (k_global >= gridsizes[2]);
  return is_out;
}

/**
 * @brief Check if a grid lies outside the global domain and returns which
 * region it lies in (indexed from 0 to 25, center region is indexed -1).
 *
 * @param ip Grid local coordinate index in the 1st dir.
 * @param jp Grid local coordinate index in the 2nd dir.
 * @param kp Grid local coordinate index in the 3rd dir.
 ...
 * @param DMVerts Local domain vertices.
 * @param gridsizes Global grid sizes.
 * @return int The index of region the grid lies in. The index goes from bottom
 * left corner (0-based) to upper right corner (25). We skip the center region,
 * which is inside the domain, and continues after that. The index goes from 0
 * to 25. If grid is at the center region (i.e., within the domain), -1 is
 * returned.
 */
int grid_outside_region(const int ip, const int jp, const int kp,
  ...
  int j_global = jp + origin_shift_j + DMVerts[2];
  int k_global = kp + origin_shift_k + DMVerts[4];
  int i_ind = (0 <= i_global) -
              (i_global < gridsizes[0]); // -1 - left, 0 - center, 1 - right
  int j_ind = (0 <= j_global) -
              (j_global < gridsizes[1]); // -1 - left, 0 - center, 1 - right
  int k_ind = (0 <= k_global) -
              (k_global < gridsizes[2]); // -1 - left, 0 - center, 1 - right
  int region_ind = (k_ind + 1) * 9 + (j_ind + 1) * 3 + (i_ind + 1);
  if (region_ind == 13)
    return -1; // grid is not outside the domain
  else if (region_ind < 13)
    return region_ind; // region before the center (original domain)
  else
    return region_ind -
           1; // region after the center, we skip the center index 13
}
 ...
 *          MATLAB(TM) function ndgrid. The result X is a
 * (ncopy*nnodes)-by-ndims matrix that contains all the coordinates in the n-d
 * space. Note that X is column major.
 *
 ...
 *                  the n-d space.
 */
void c_ndgrid_recursion(int ndims, int ncopy, int nnodes, int *xs, int *xe,
                        int *X) {
  if (ndims < 1)
    return;
...
/**
 * @brief   Calculate real spherical harmonics for given positions and given l
 * and m.
 *
 *          Only for l = 0, 1, ..., 6.
 */
void RealSphericalHarmonic(const int len, double *x, double *y, double *z,
                           double *r, const int l, const int m, double *Ylm) {
  ...
  case 1:
    ...
    /* incorrect m */
    default:
      printf("<m> must be an integer between %d and %d!\n", -l, l);
      break;
    }
    break;
  ...
  case 2:
    ...
    case 0: /* m = 0 */
      for (i = 0; i < len; i++)
        Ylm[i] = C20 * (-x[i] * x[i] - y[i] * y[i] + 2.0 * z[i] * z[i]) /
                 (r[i] * r[i]);
      break;
  ...
  case 3:
    ...
    case -3: /* m = -3 */
      for (i = 0; i < len; i++)
        Ylm[i] = C3m3 * (3 * x[i] * x[i] - y[i] * y[i]) * y[i] /
                 (r[i] * r[i] * r[i]);
      break;
    ...
    case -1: /* m = -1 */
      for (i = 0; i < len; i++)
        Ylm[i] = C3m1 * y[i] * (4 * z[i] * z[i] - x[i] * x[i] - y[i] * y[i]) /
                 (r[i] * r[i] * r[i]);
      break;
    ...
    case 0: /* m = 0 */
      for (i = 0; i < len; i++)
        Ylm[i] = C30 * z[i] *
                 (2 * z[i] * z[i] - 3 * x[i] * x[i] - 3 * y[i] * y[i]) /
                 (r[i] * r[i] * r[i]);
      break;

    case 1: /* m = 1 */
      for (i = 0; i < len; i++)
        Ylm[i] = C3p1 * x[i] * (4 * z[i] * z[i] - x[i] * x[i] - y[i] * y[i]) /
                 (r[i] * r[i] * r[i]);
      break;
    ...
    case 2: /* m = 2 */
      for (i = 0; i < len; i++)
        Ylm[i] =
            C3p2 * z[i] * (x[i] * x[i] - y[i] * y[i]) / (r[i] * r[i] * r[i]);
      break;

    case 3: /* m = 3 */
      for (i = 0; i < len; i++)
        Ylm[i] = C3p3 * x[i] * (x[i] * x[i] - 3 * y[i] * y[i]) /
                 (r[i] * r[i] * r[i]);
      break;
  ...
  case 4:
    ...
    case -4: /* m = -4 */
      for (i = 0; i < len; i++)
        Ylm[i] = C4m4 * (x[i] * y[i] * (x[i] * x[i] - y[i] * y[i])) /
                 (r[i] * r[i] * r[i] * r[i]);
      break;

    case -3: /* m = -3 */
      for (i = 0; i < len; i++)
        Ylm[i] = C4m3 * (3.0 * x[i] * x[i] - y[i] * y[i]) * y[i] * z[i] /
                 (r[i] * r[i] * r[i] * r[i]);
      break;

    case -2: /* m = -2 */
      for (i = 0; i < len; i++)
        Ylm[i] = C4m2 * x[i] * y[i] * (7.0 * z[i] * z[i] - r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i]);
      break;

    case -1: /* m = -1 */
      for (i = 0; i < len; i++)
        Ylm[i] = C4m1 * y[i] * z[i] * (7.0 * z[i] * z[i] - 3.0 * r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i]);
      break;
    ...
    case 0: /* m = 0 */
      for (i = 0; i < len; i++)
        Ylm[i] = C40 *
                 (35.0 * z[i] * z[i] * z[i] * z[i] -
                  30.0 * z[i] * z[i] * r[i] * r[i] +
                  3.0 * r[i] * r[i] * r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i]);
      break;
    ...
    case 1: /* m = 1 */
      for (i = 0; i < len; i++)
        Ylm[i] = C4p1 * x[i] * z[i] * (7.0 * z[i] * z[i] - 3.0 * r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i]);
      break;

    case 2: /* m = 2 */
      for (i = 0; i < len; i++)
        Ylm[i] = C4p2 * (x[i] * x[i] - y[i] * y[i]) *
                 (7.0 * z[i] * z[i] - r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i]);
      break;
    ...
    case 3: /* m = 3 */
      for (i = 0; i < len; i++)
        Ylm[i] = C4p3 * (x[i] * x[i] - 3.0 * y[i] * y[i]) * x[i] * z[i] /
                 (r[i] * r[i] * r[i] * r[i]);
      break;
    ...
    case 4: /* m = 4 */
      for (i = 0; i < len; i++)
        Ylm[i] = C4p4 *
                 (x[i] * x[i] * (x[i] * x[i] - 3.0 * y[i] * y[i]) -
                  y[i] * y[i] * (3.0 * x[i] * x[i] - y[i] * y[i])) /
                 (r[i] * r[i] * r[i] * r[i]);
      break;
  ...
  case 5:
    ...
    case -5: /* m = -5 */
      for (i = 0; i < len; i++) {
        p = sqrt(x[i] * x[i] + y[i] * y[i]);
        Ylm[i] = C5m5 *
                 (8.0 * x[i] * x[i] * x[i] * x[i] * y[i] -
                  4.0 * x[i] * x[i] * y[i] * y[i] * y[i] + 4.0 * pow(y[i], 5) -
                  3.0 * y[i] * p * p * p * p) /
                 (r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case -3: /* m = -3 */
      for (i = 0; i < len; i++) {
        p = sqrt(x[i] * x[i] + y[i] * y[i]);
        Ylm[i] = C5m3 * (3.0 * y[i] * p * p - 4.0 * y[i] * y[i] * y[i]) *
                 (9.0 * z[i] * z[i] - r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case -1: /* m = -1 */
      for (i = 0; i < len; i++) {
        ...
        Ylm[i] =
            C5m1 * y[i] *
            (21.0 * z[i] * z[i] * z[i] * z[i] -
             14.0 * r[i] * r[i] * z[i] * z[i] + r[i] * r[i] * r[i] * r[i]) /
            (r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 0: /* m = 0 */
      for (i = 0; i < len; i++) {
        // p = sqrt(x[i]*x[i]+y[i]*y[i]);
        Ylm[i] = C50 *
                 (63.0 * z[i] * z[i] * z[i] * z[i] * z[i] -
                  70.0 * z[i] * z[i] * z[i] * r[i] * r[i] +
                  15.0 * z[i] * r[i] * r[i] * r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 2: /* m = 2 */
      for (i = 0; i < len; i++) {
        // p = sqrt(x[i]*x[i]+y[i]*y[i]);
        Ylm[i] = C5p2 * (x[i] * x[i] - y[i] * y[i]) *
                 (3.0 * z[i] * z[i] * z[i] - r[i] * r[i] * z[i]) /
                 (r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 3: /* m = 3 */
      for (i = 0; i < len; i++) {
        p = sqrt(x[i] * x[i] + y[i] * y[i]);
        Ylm[i] = C5p3 * (4.0 * x[i] * x[i] * x[i] - 3.0 * p * p * x[i]) *
                 (9.0 * z[i] * z[i] - r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 4: /* m = 4 */
      for (i = 0; i < len; i++) {
        ...
        Ylm[i] =
            C5p4 *
            (4.0 * (x[i] * x[i] * x[i] * x[i] + y[i] * y[i] * y[i] * y[i]) -
             3.0 * p * p * p * p) *
            z[i] / (r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 5: /* m = 5 */
      for (i = 0; i < len; i++) {
        p = sqrt(x[i] * x[i] + y[i] * y[i]);
        Ylm[i] = C5p5 *
                 (4.0 * x[i] * x[i] * x[i] * x[i] * x[i] +
                  8.0 * x[i] * y[i] * y[i] * y[i] * y[i] -
                  4.0 * x[i] * x[i] * x[i] * y[i] * y[i] -
                  3.0 * x[i] * p * p * p * p) /
                 (r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
  ...
  case 6:
    ...
    case -6: /* m = -6 */
      for (i = 0; i < len; i++) {
        p = sqrt(x[i] * x[i] + y[i] * y[i]);
        Ylm[i] = C6m6 *
                 (12.0 * pow(x[i], 5) * y[i] + 12.0 * x[i] * pow(y[i], 5) -
                  8.0 * x[i] * x[i] * x[i] * y[i] * y[i] * y[i] -
                  6.0 * x[i] * y[i] * pow(p, 4)) /
    ...
    case -3: /* m = -3 */
      for (i = 0; i < len; i++) {
        p = sqrt(x[i] * x[i] + y[i] * y[i]);
        Ylm[i] = C6m3 * (-4.0 * y[i] * y[i] * y[i] + 3.0 * y[i] * p * p) *
                 (11.0 * z[i] * z[i] * z[i] - 3.0 * z[i] * r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case -2: /* m = -2 */
      for (i = 0; i < len; i++) {
        // p = sqrt(x[i]*x[i]+y[i]*y[i]);
        Ylm[i] = C6m2 * (2.0 * x[i] * y[i]) *
                 (33.0 * pow(z[i], 4) - 18.0 * z[i] * z[i] * r[i] * r[i] +
                  pow(r[i], 4)) /
                 (r[i] * r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case -1: /* m = -1 */
      for (i = 0; i < len; i++)
        Ylm[i] =
            C6m1 * y[i] *
            (33.0 * pow(z[i], 5) - 30.0 * z[i] * z[i] * z[i] * r[i] * r[i] +
             5.0 * z[i] * pow(r[i], 4)) /
            (r[i] * r[i] * r[i] * r[i] * r[i] * r[i]);
      break;
    ...
    case 0: /* m = 0 */
      for (i = 0; i < len; i++) {
        // p = sqrt(x[i]*x[i]+y[i]*y[i]);
        Ylm[i] = C60 *
                 (231.0 * pow(z[i], 6) - 315 * pow(z[i], 4) * r[i] * r[i] +
                  105.0 * z[i] * z[i] * pow(r[i], 4) - 5.0 * pow(r[i], 6)) /
                 (r[i] * r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 3: /* m = 3 */
      for (i = 0; i < len; i++) {
        p = sqrt(x[i] * x[i] + y[i] * y[i]);
        Ylm[i] = C6p3 * (4.0 * x[i] * x[i] * x[i] - 3.0 * x[i] * p * p) *
                 (11.0 * z[i] * z[i] * z[i] - 3.0 * z[i] * r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 4: /* m = 4 */
      for (i = 0; i < len; i++) {
        p = sqrt(x[i] * x[i] + y[i] * y[i]);
        Ylm[i] = C6p4 *
                 (4.0 * pow(x[i], 4) + 4.0 * pow(y[i], 4) - 3.0 * pow(p, 4)) *
                 (11.0 * z[i] * z[i] - r[i] * r[i]) /
                 (r[i] * r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 5: /* m = 5 */
      for (i = 0; i < len; i++) {
        ...
        Ylm[i] =
            C6p5 *
            (4.0 * pow(x[i], 5) + 8.0 * x[i] * pow(y[i], 4) -
             4.0 * x[i] * x[i] * x[i] * y[i] * y[i] - 3.0 * x[i] * pow(p, 4)) *
            z[i] / (r[i] * r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
    ...
    case 6: /* m = 6 */
      for (i = 0; i < len; i++) {
        ...
        Ylm[i] =
            C6p6 *
            (4.0 * pow(x[i], 6) - 4.0 * pow(y[i], 6) +
             12.0 * x[i] * x[i] * pow(y[i], 4) -
             12.0 * pow(x[i], 4) * y[i] * y[i] + 3.0 * y[i] * y[i] * pow(p, 4) -
             3.0 * x[i] * x[i] * pow(p, 4)) /
            (r[i] * r[i] * r[i] * r[i] * r[i] * r[i]);
      }
      break;
  ...
  default:
    printf("<l> must be an integer between 0 and 6!\n");
    break;
  }
...
/**
 * @brief   Calculate Complex spherical harmonics for given positions and given
 * l and m.
 *
 *          Only for l = 0, 1, ..., 6.
 */
void ComplexSphericalHarmonic(const int len, double *x, double *y, double *z,
                              double *r, const int l, const int m,
                              double _Complex *Ylm) {
  // only l=0,1,2,3,4,5,6 implemented for now
  ...
  case 2:
    ...
    case 0: /* m = 0 */
      for (i = 0; i < len; i++)
        Ylm[i] =
            C20 * (2 * z[i] * z[i] - x[i] * x[i] - y[i] * y[i]) / (r[i] * r[i]);
      break;
  ...
  case 3:
    ...
    case -3: /* m = -3 */
      for (i = 0; i < len; i++)
        Ylm[i] = C33 *
                 ((x[i] - I * y[i]) * (x[i] - I * y[i]) * (x[i] - I * y[i])) /
                 (r[i] * r[i] * r[i]);
      break;

    case -2: /* m = -2 */
      for (i = 0; i < len; i++)
        Ylm[i] = C32 * ((x[i] - I * y[i]) * ((x[i] - I * y[i])) * z[i]) /
                 (r[i] * r[i] * r[i]);
      break;
  ...
  case 4:
    ...
    case -1: /* m = -1 */
      for (i = 0; i < len; i++)
        Ylm[i] =
            C41 *
            ((x[i] - I * y[i]) * z[i] * (7 * z[i] * z[i] - 3 * r[i] * r[i])) /
            pow(r[i], 4);
      break;
  ...
  case 5:
    ...
    case -1: /* m = -1 */
      for (i = 0; i < len; i++) {
        Ylm[i] = C51 *
                 ((x[i] - I * y[i]) *
                  (21 * pow(z[i], 4) - 14 * z[i] * z[i] * r[i] * r[i] +
                   pow(r[i], 4))) /
                 pow(r[i], 5);
      }
      break;
    ...
    case 0: /* m = 0 */
      for (i = 0; i < len; i++) {
        Ylm[i] =
            C50 *
            (63 * pow(z[i], 5) - 70 * (z[i] * z[i] * z[i]) * (r[i] * r[i]) +
             15 * z[i] * pow(r[i], 4)) /
            pow(r[i], 5);
      }
      break;
  ...
  case 6:
    ...
    case -4: /* m = -4 */
      for (i = 0; i < len; i++)
        Ylm[i] = C64 *
                 (cpow(x[i] - I * y[i], 4) * (11 * z[i] * z[i] - r[i] * r[i])) /
                 pow(r[i], 6);
      break;
    ...
    case -2: /* m = -2 */
      for (i = 0; i < len; i++) {
        Ylm[i] = C62 *
                 (((x[i] - I * y[i]) * (x[i] - I * y[i])) *
                  (33 * pow(z[i], 4) - 18 * (z[i] * z[i]) * (r[i] * r[i]) +
                   pow(r[i], 4))) /
                 pow(r[i], 6);
      }
      break;
    ...
    case -1: /* m = -1 */
      for (i = 0; i < len; i++)
        Ylm[i] =
            C61 *
            ((x[i] - I * y[i]) *
             (33 * pow(z[i], 5) - 30 * (z[i] * z[i] * z[i]) * (r[i] * r[i]) +
              5 * z[i] * pow(r[i], 4))) /
            pow(r[i], 6);
      break;
    ...
    case 0: /* m = 0 */
      for (i = 0; i < len; i++) {
        Ylm[i] = C60 *
                 (231 * pow(z[i], 6) - 315 * pow(z[i], 4) * (r[i] * r[i]) +
                  105 * (z[i] * z[i]) * pow(r[i], 4) - 5 * pow(r[i], 6)) /
                 pow(r[i], 6);
      }
      break;
...
*/
void Calc_dist(SPARC_OBJ *pSPARC, int nxp, int nyp, int nzp, double x0_i_shift,
               double y0_i_shift, double z0_i_shift, double *R, double rchrg,
               int *count_interp) {
    ...
    for (k = 0; k < nzp; k++) {
      ...
      for (j = 0; j < nyp; j++) {
        ...
        for (i = 0; i < nxp; i++) {
          x = i * pSPARC->delta_x - x0_i_shift;
          R[count] =
              sqrt(pSPARC->metricT[0] * (x * x) + pSPARC->metricT[1] * (x * y) +
                   pSPARC->metricT[2] * (x * z) + pSPARC->metricT[4] * (y * y) +
                   pSPARC->metricT[5] * (y * z) + pSPARC->metricT[8] * (z * z));
          if (R[count] <= rchrg)
            (*count_interp)++;
          (count)++;
        }
 ...
 * @param DMVertices Array of length 6, domain vertices of the local pieces of
 * x.
 * @param fname      The name of the file to which the vector will be written.
 * @param comm       MPI communicator.
 */
void print_vec(double *x, int *gridsizes, int *DMVertices, char *fname,
               MPI_Comm comm) {
  if (comm == MPI_COMM_NULL)
    return;
 ...
 * @param comm       MPI communicator.
 */
void read_vec(double *x, int *gridsizes, int *DMVertices, char *fname,
              MPI_Comm comm) {
  if (comm == MPI_COMM_NULL)
    return;
  ...
  if (nproc_comm >
      1) { // if there's more than one process, need to distribute x
    ...
    // number of processes is large (> 1000), in that case, do
    // this step only once and keep the d2d target objects
    Set_D2D_Target(&d2d_sender, &d2d_recvr, gridsizes, sDMVert, DMVertices,
                   send_comm, sdims, comm, rdims, comm);

    // collect vector to one process
    D2D(&d2d_sender, &d2d_recvr, gridsizes, sDMVert, x_global, DMVertices, x,
        send_comm, sdims, comm, rdims, comm);
...
void check_below_entries(FILE *ion_fp, char *tag) {
  ...
  for (i = 0; i < strlen(str); i++) {
    if (isdigit(str[i])) {
      printf(
          RED
          "ERROR: Please remove the data in the same line as the %s tag in the "
          "ION file. All entries should be strictly below the %s tag.\n" RESET,
          tag, tag);
      exit(EXIT_FAILURE);
    }
  }
...
int check_num_input(FILE *fp, void *array, char TYPE) {
  ...
  if (TYPE == 'I') {
    while ((nums_now = sscanf(str + bytes_consumed, "%d%n",
                              (int *)array + nums_read, &bytes_now)) > 0 &&
           nums_read < 10) {
      bytes_consumed += bytes_now;
      nums_read += nums_now;
    }
  } else if (TYPE == 'D') {
    while ((nums_now = sscanf(str + bytes_consumed, "%lf%n",
                              (double *)array + nums_read, &bytes_now)) > 0 &&
           nums_read < 10) {
      bytes_consumed += bytes_now;
      nums_read += nums_now;
    }
...
/*
 @ brief: function to calculate a 3x3 matrix times a vector
*/
void matrixTimesVec_3d_complex(double *A, double _Complex *b,
                               double _Complex *c) {
#define A(i, j) A[i + j * 3]
  int i, j;
  for (i = 0; i < 3; i++) {
...
#if defined(USE_MKL)
/**
 * @brief   MKL multi-dimension FFT interface, real to complex, following
 * conjugate even distribution.
 */
void MKL_MDFFT_real(double *r2c_3dinput, MKL_LONG *dim_sizes,
                    MKL_LONG *strides_out, double _Complex *r2c_3doutput) {
  DFTI_DESCRIPTOR_HANDLE my_desc_handle = NULL;
  MKL_LONG status;
  /********************************************************************/

  status = DftiCreateDescriptor(&my_desc_handle, DFTI_DOUBLE, DFTI_REAL, 3,
                                dim_sizes);
 ...
 */
void MKL_MDFFT(double _Complex *c2c_3dinput, MKL_LONG *dim_sizes,
               MKL_LONG *strides_out, double _Complex *c2c_3doutput) {
  DFTI_DESCRIPTOR_HANDLE my_desc_handle = NULL;
  MKL_LONG status;
  /********************************************************************/

  status = DftiCreateDescriptor(&my_desc_handle, DFTI_DOUBLE, DFTI_COMPLEX, 3,
                                dim_sizes);
  status =
      DftiSetValue(my_desc_handle, DFTI_COMPLEX_COMPLEX, DFTI_COMPLEX_COMPLEX);
  status = DftiSetValue(my_desc_handle, DFTI_PLACEMENT, DFTI_NOT_INPLACE);
  status = DftiSetValue(my_desc_handle, DFTI_OUTPUT_STRIDES, strides_out);
...
/**
 * @brief   MKL multi-dimension iFFT interface, complex to real, following
 * conjugate even distribution.
 */
void MKL_MDiFFT_real(double _Complex *c2r_3dinput, MKL_LONG *dim_sizes,
                     MKL_LONG *strides_in, double *c2r_3doutput) {
  DFTI_DESCRIPTOR_HANDLE my_desc_handle = NULL;
  MKL_LONG status;
  /********************************************************************/
 ...
 */
void MKL_MDiFFT(double _Complex *c2c_3dinput, MKL_LONG *dim_sizes,
                MKL_LONG *strides_out, double _Complex *c2c_3doutput) {
  DFTI_DESCRIPTOR_HANDLE my_desc_handle = NULL;
  MKL_LONG status;
  /********************************************************************/
 ...
 */
void FFTW_MDFFT(int *dim_sizes, double _Complex *c2c_3dinput,
                double _Complex *c2c_3doutput) {
  fftw_complex *in, *out;
  fftw_plan p;
  int N = dim_sizes[0] * dim_sizes[1] * dim_sizes[2];
  p = fftw_plan_dft(3, dim_sizes, c2c_3dinput, c2c_3doutput, FFTW_FORWARD,
                    FFTW_ESTIMATE);
  fftw_execute(p);
  fftw_destroy_plan(p);
}
 ...
 */
void FFTW_MDiFFT(int *dim_sizes, double _Complex *c2c_3dinput,
                 double _Complex *c2c_3doutput) {
  fftw_complex *in, *out;
  fftw_plan p;
  int N = dim_sizes[0] * dim_sizes[1] * dim_sizes[2], i;
  p = fftw_plan_dft(3, dim_sizes, c2c_3dinput, c2c_3doutput, FFTW_BACKWARD,
                    FFTW_ESTIMATE);
  fftw_execute(p);
  fftw_destroy_plan(p);
  for (i = 0; i < N; i++)
 ...
 */
void FFTW_MDFFT_real(int *dim_sizes, double *r2c_3dinput,
                     double _Complex *r2c_3doutput) {
  fftw_complex *in, *out;
  fftw_plan p;
  int N = dim_sizes[0] * dim_sizes[1] * dim_sizes[2];
 ...
 */
void FFTW_MDiFFT_real(int *dim_sizes, double _Complex *c2r_3dinput,
                      double *c2r_3doutput) {
  fftw_complex *in, *out;
  fftw_plan p;
  int N = dim_sizes[0] * dim_sizes[1] * dim_sizes[2], i;
 ...
 *
 */
void restrict_to_subgrid(const double *v_i, double *v_o, const int stride_y_o,
                         const int stride_y_i, const int stride_z_o,
                         const int stride_z_i, const int x_o_spos,
                         const int x_o_epos, const int y_o_spos,
                         const int y_o_epos, const int z_o_spos,
                         const int z_o_epos, const int x_i_spos,
                         const int y_i_spos, const int z_i_spos) {
