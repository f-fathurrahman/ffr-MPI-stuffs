...
void Setup_Comms(SPARC_OBJ *pSPARC) {
...
#ifdef DEBUG
  if (rank == 0)
    printf("Set up communicators.\n");
#endif

  // in the case user doesn't set any parallelization parameters, search for a
  // good combination in advance
  ...
  if (pSPARC->npspin == 0 && pSPARC->npkpt == 0 && pSPARC->npband == 0 &&
      npNd == 0) {
    dims_divide_skbd(pSPARC->Nspin, pSPARC->Nkpts_sym, pSPARC->Nstates,
                     gridsizes, nproc, &pSPARC->npspin, &pSPARC->npkpt,
                     &pSPARC->npband, &npNd, minsize, pSPARC->usefock);
  ...
  // Allocate number of processors and their local indices in a spin
  // communicator
  ...
  MPI_Comm_split(MPI_COMM_WORLD, color, pSPARC->spincomm_index,
  ...
  if (pSPARC->npkpt == 0) {
    pSPARC->npkpt =
        min(size_spincomm,
            pSPARC->Nkpts_sym); // paral over k-points as much as possible
  } else if (pSPARC->npkpt > pSPARC->Nkpts_sym ||
             pSPARC->npkpt > size_spincomm) {
    pSPARC->npkpt = min(size_spincomm, pSPARC->Nkpts_sym);
    if (rank == 0) {
      printf("WARNING: npkpt is larger than number of k-points after symmetry "
             "reduction or size_spincomm!\n"
  ...
  if (pSPARC->spincomm_index != -1 &&
      rank_spincomm < (size_spincomm - size_spincomm % pSPARC->npkpt))
  ...
  if (rank_spincomm < (size_spincomm - size_spincomm % pSPARC->npkpt))
    pSPARC->Nkpts_kptcomm =
        pSPARC->Nkpts_sym / pSPARC->npkpt +
        (int)(pSPARC->kptcomm_index < (pSPARC->Nkpts_sym % pSPARC->npkpt));
  else
  ...
  t1 = MPI_Wtime();
#endif
  // split the pSPARC->spincomm into several kptcomms using color =
  // kptcomm_index
  ...
  if (pSPARC->Nkpts >= 1 && pSPARC->kptcomm_index != -1) {
    ...
    // if (pSPARC->BC != 1) {
    pSPARC->kptWts_loc =
        (double *)malloc(pSPARC->Nkpts_kptcomm * sizeof(double));
  ...
  if (pSPARC->kptcomm_index < 0)
    nproc_kptcomm = size_kptcomm; // let all proc know the size
...
#ifdef DEBUG
  if (!rank)
    printf("\n kpt_topo #%d, kptcomm topology dims = {%d, %d, %d}, nodes/proc "
           "= {%.2f,%.2f,%.2f}\n",
           pSPARC->kptcomm_index, dims[0], dims[1], dims[2],
           (double)gridsizes[0] / dims[0], (double)gridsizes[1] / dims[1],
           (double)gridsizes[2] / dims[2]);
#endif

  if (pSPARC->kptcomm_index >= 0) {
    // Processes in kptcomm with rank >= dims[0]*dims[1]*…*dims[d-1] will return
    // MPI_COMM_NULL
    MPI_Cart_create(pSPARC->kptcomm, 3, dims, periods, 1,
                    &pSPARC->kptcomm_topo); // 1 is to reorder rank
  } else {
  ...
  if (pSPARC->kptcomm_topo != MPI_COMM_NULL) {
    ...
    pSPARC->Nx_d_kptcomm =
        block_decompose(gridsizes[0], dims[0], coord_kpt_topo[0]);
    pSPARC->Ny_d_kptcomm =
        block_decompose(gridsizes[1], dims[1], coord_kpt_topo[1]);
    pSPARC->Nz_d_kptcomm =
        block_decompose(gridsizes[2], dims[2], coord_kpt_topo[2]);
    pSPARC->Nd_d_kptcomm =
        pSPARC->Nx_d_kptcomm * pSPARC->Ny_d_kptcomm * pSPARC->Nz_d_kptcomm;

    // find corners of the distributed domain in kptcomm_topo
    pSPARC->DMVertices_kptcomm[0] =
        block_decompose_nstart(gridsizes[0], dims[0], coord_kpt_topo[0]);
    pSPARC->DMVertices_kptcomm[1] =
        pSPARC->DMVertices_kptcomm[0] + pSPARC->Nx_d_kptcomm - 1;
    pSPARC->DMVertices_kptcomm[2] =
        block_decompose_nstart(gridsizes[1], dims[1], coord_kpt_topo[1]);
    pSPARC->DMVertices_kptcomm[3] =
        pSPARC->DMVertices_kptcomm[2] + pSPARC->Ny_d_kptcomm - 1;
    pSPARC->DMVertices_kptcomm[4] =
        block_decompose_nstart(gridsizes[2], dims[2], coord_kpt_topo[2]);
    pSPARC->DMVertices_kptcomm[5] =
        pSPARC->DMVertices_kptcomm[4] + pSPARC->Nz_d_kptcomm - 1;
  } else {
    rank_kpt_topo = -1;
    coord_kpt_topo[0] = -1;
    coord_kpt_topo[1] = -1;
  ...
  if (pSPARC->cell_typ != 0) {
    if (pSPARC->kptcomm_topo != MPI_COMM_NULL) {
      ...
      int nneighb =
          pow(2 * nnproc + 1, 3) - 1; // 6 faces + 12 edges + 8 corners
      int *neighb;
        ...
        for (j = -nnproc; j <= nnproc; j++) {
          for (i = -nnproc; i <= nnproc; i++) {
            ...
            if (i == 0 && j == 0 && k == 0) {
              ...
              // TODO: For dirchlet give rank = MPI_PROC_NULL for out of bounds
              // coordinates
              if (tmp == 1)
                MPI_Cart_rank(
                    pSPARC->kptcomm_topo, ncoords,
                    &rank_chk); // proc rank corresponding to ncoords_mapped
      ...
      MPI_Dist_graph_create_adjacent(pSPARC->kptcomm_topo, nneighb, neighb,
                                     (int *)MPI_UNWEIGHTED, nneighb, neighb,
                                     (int *)MPI_UNWEIGHTED, MPI_INFO_NULL, 0,
                                     &pSPARC->kptcomm_topo_dist_graph);
      free(neighb);
    }
  }

  //------------------------------------------------------------------------------------//
  //    set up inter-communicators between the Cart topology and the rest in
  //    kptcomm    //
  //------------------------------------------------------------------------------------//
  int nproc_kptcomm_topo;
  nproc_kptcomm_topo =
      pSPARC->npNdx_kptcomm * pSPARC->npNdy_kptcomm * pSPARC->npNdz_kptcomm;
  if (nproc_kptcomm_topo < nproc_kptcomm && pSPARC->kptcomm_index >= 0) {
#ifdef DEBUG
    t1 = MPI_Wtime();
#endif
    // first create a comm that includes all the processes that are excluded
    // from the Cart topology
    MPI_Group kptgroup, kptgroup_excl;
    MPI_Comm_group(pSPARC->kptcomm, &kptgroup);
    int *incl_ranks, count;
    incl_ranks =
        (int *)malloc((nproc_kptcomm - nproc_kptcomm_topo) * sizeof(int));
    count = 0;
    ...
    MPI_Comm_create_group(pSPARC->kptcomm, kptgroup_excl, 110,
                          &pSPARC->kptcomm_topo_excl);

    // now create an inter-comm between kptcomm_topo and kptcomm_topo_excl
    if (pSPARC->kptcomm_topo != MPI_COMM_NULL) {
      MPI_Intercomm_create(pSPARC->kptcomm_topo, 0, pSPARC->kptcomm,
                           nproc_kptcomm_topo, 111, &pSPARC->kptcomm_inter);
    } else {
      MPI_Intercomm_create(pSPARC->kptcomm_topo_excl, 0, pSPARC->kptcomm, 0,
                           111, &pSPARC->kptcomm_inter);
    }
  ...
  MPI_Comm_split(pSPARC->spincomm, color, pSPARC->kptcomm_index,
                 &pSPARC->kpt_bridge_comm); // TODO: exclude null kptcomms

  //------------------------------------------------//
  //               set up bandcomm                  //
  //------------------------------------------------//
  // in each kptcomm, create sub-communicators over bands
  // note: different from kptcomms, we require the bandcomms in the same kptcomm
  // to be of the same size
  if (pSPARC->npband == 0) {
    pSPARC->npband = min(
        nproc_kptcomm, pSPARC->Nstates); // paral over band as much as possible
  ...
  NP_BANDCOMM =
      pSPARC->npband * size_bandcomm; // number of processors that belong to a
                                      // bandcomm, others are excluded
  // calculate which bandcomm processor with RANK = rank_kptcomm belongs to
  if (rank_kptcomm < NP_BANDCOMM && pSPARC->kptcomm_index != -1) {
    // pSPARC->bandcomm_index = rank_kptcomm / size_bandcomm; // assign
    // processes column-wisely
    pSPARC->bandcomm_index =
        rank_kptcomm % pSPARC->npband; // assign processes row-wisely
  } else {
    pSPARC->bandcomm_index =
        -1; // these processors won't be used to do calculations
  }
  // calculate number of bands assigned to each bandcomm, this is a special case
  // of block-cyclic distribution (*,block)
  if (pSPARC->bandcomm_index == -1) {
    pSPARC->Nband_bandcomm = 0;
  } else {
    NB = (pSPARC->Nstates - 1) / pSPARC->npband +
         1; // this is equal to ceil(Nstates/npband), for int inputs only
    pSPARC->Nband_bandcomm =
        pSPARC->bandcomm_index < (pSPARC->Nstates / NB)
            ? NB
            : (pSPARC->bandcomm_index == (pSPARC->Nstates / NB)
                   ? (pSPARC->Nstates % NB)
                   : 0);
  ...
  if (pSPARC->bandcomm_index == -1) {
    ...
    pSPARC->band_start_indx =
        pSPARC->Nstates; // TODO: this might be dangerous, consider using 0,
                         // instead of Ns here
  }
  ...
  if (npNd == 0) {
    ...
    pSPARC->npNdy = dims[1];
    pSPARC->npNdz = dims[2];
  } else if (npNd < 0 || npNd > nproc_bandcomm ||
             pSPARC->Nx / pSPARC->npNdx < minsize ||
             pSPARC->Ny / pSPARC->npNdy < minsize ||
             pSPARC->Nz / pSPARC->npNdz < minsize) {
...
#ifdef DEBUG
  if (!rank)
    printf("rank = %d, dmcomm dims = {%d, %d, %d}\n", rank, pSPARC->npNdx,
           pSPARC->npNdy, pSPARC->npNdz);
  t1 = MPI_Wtime();
#endif

  // Processes in bandcomm with rank >= dims[0]*dims[1]*…*dims[d-1] will return
  // MPI_COMM_NULL
  if (pSPARC->bandcomm_index != -1) {
    MPI_Cart_create(pSPARC->bandcomm, 3, dims, periods, 1,
                    &pSPARC->dmcomm); // 1 is to reorder rank
  } else {
    pSPARC->dmcomm = MPI_COMM_NULL;
  }

  if (gridsizes[0] % dims[0] || gridsizes[1] % dims[1] ||
      gridsizes[2] % dims[2]) {
    pSPARC->is_domain_uniform = 0; // not uniform
  } else {
    pSPARC->is_domain_uniform = 1; // uniform
  }
  pSPARC->is_domain_uniform =
      0; // turn off uniform, this feature will not be used anymore
#ifdef DEBUG
  if (!rank)
    printf(
        "gridsizes = [%d, %d, %d], Nstates = %d, dmcomm dims = [%d, %d, %d]\n",
        gridsizes[0], gridsizes[1], gridsizes[2], pSPARC->Nstates, dims[0],
        dims[1], dims[2]);
#endif
  ...
  if (pSPARC->dmcomm != MPI_COMM_NULL && pSPARC->bandcomm_index != -1) {
    ...
    pSPARC->Nx_d_dmcomm =
        block_decompose(gridsizes[0], dims[0], coord_dmcomm[0]);
    pSPARC->Ny_d_dmcomm =
        block_decompose(gridsizes[1], dims[1], coord_dmcomm[1]);
    pSPARC->Nz_d_dmcomm =
        block_decompose(gridsizes[2], dims[2], coord_dmcomm[2]);
    pSPARC->Nd_d_dmcomm =
        pSPARC->Nx_d_dmcomm * pSPARC->Ny_d_dmcomm * pSPARC->Nz_d_dmcomm;

    // find corners of the distributed domain in dmcomm
    pSPARC->DMVertices_dmcomm[0] =
        block_decompose_nstart(gridsizes[0], dims[0], coord_dmcomm[0]);
    pSPARC->DMVertices_dmcomm[1] =
        pSPARC->DMVertices_dmcomm[0] + pSPARC->Nx_d_dmcomm - 1;
    pSPARC->DMVertices_dmcomm[2] =
        block_decompose_nstart(gridsizes[1], dims[1], coord_dmcomm[1]);
    pSPARC->DMVertices_dmcomm[3] =
        pSPARC->DMVertices_dmcomm[2] + pSPARC->Ny_d_dmcomm - 1;
    pSPARC->DMVertices_dmcomm[4] =
        block_decompose_nstart(gridsizes[2], dims[2], coord_dmcomm[2]);
    pSPARC->DMVertices_dmcomm[5] =
        pSPARC->DMVertices_dmcomm[4] + pSPARC->Nz_d_dmcomm - 1;
  ...
  // TODO: Modify the communicator based on number of non zero enteries in off
  // diagonal of pSPARC->lapcT
  ...
  if (pSPARC->cell_typ != 0) {
    if (pSPARC->dmcomm != MPI_COMM_NULL) {
      ...
      MPI_Dist_graph_create_adjacent(
          pSPARC->dmcomm, nneighb, neighb, (int *)MPI_UNWEIGHTED, nneighb,
          neighb, (int *)MPI_UNWEIGHTED, MPI_INFO_NULL, 0,
          &pSPARC->comm_dist_graph_psi); // creates a distributed graph topology
                                         // (adjacent, cartesian cubical)
  ...
  // 1. Let the group of all processors dealing with the same local domain (but
  // different
  //    bands) be a blacscomm. We will find Psi_i' * Psi_i in each blacscomm,
  //    and sum over all local domains (dmcomm) to find Psi' * Psi. This way,
  //    however, doesn't seem to be efficient since the sum (Allreduce) part
  //    will take a lot of time. In some cases even more than the computation
  //    time.
  // 2. In stead of having many parallel blacscomm in each kptcomm, we create
  // only one
  //    blacscomm, containing all the parallel blacscomm described above. This
  //    way uses a little trick of linear algebra: since the rows are reordered
  //    when we do domain parallelization, the matrix Psi is interpretated as P
  //    * Psi by the ScaLAPACK routines distributed block-wisely, where P is a
  //    permutation matrix (with 0's and 1's). But, since P is orthogonal, we
  //    could still preceed and find (P * Psi)' * (P * Psi) = Psi' * (P'*P) *
  //    Psi = Psi' * Psi, and the result will still be the same as Psi'*Psi.
  //    P.S., however, this requires the domain partition to be UNIFORM!
  ...
  MPI_Comm_split(pSPARC->kptcomm, color, pSPARC->bandcomm_index,
                 &pSPARC->blacscomm);
#ifdef DEBUG
  t2 = MPI_Wtime();
  if (rank == 0)
  ...
  size_blacscomm = pSPARC->is_domain_uniform ? (pSPARC->npband * pSPARC->npNd)
                                             : pSPARC->npband;
  ...
  if (pSPARC->bandcomm_index != -1 && pSPARC->dmcomm != MPI_COMM_NULL) {
    ...
    if (pSPARC->is_domain_uniform) {
      // note that this usermap we created here is row-major, but Cblacs_gridmap
      // assumes usermap is column-major, so we have to change it later after we
      // decide context size
      for (i = 0; i < size_blacscomm; i++) {
        usermap[i] = i + rank - rank_kptcomm;
      }
    } else {
      // MPI_Allgather(&rank, 1, MPI_INT, usermap, 1, MPI_INT,
      // pSPARC->blacscomm);
      for (i = 0; i < size_blacscomm; i++) {
        usermap[i] = usermap_0[i] = usermap_1[i] =
            i + rank - rank_kptcomm + rank_dmcomm * pSPARC->npband;
      }
    }
...
#ifdef DEBUG
    if (!rank)
      printf("rank = %d, size_blacscomm = %d, ScaLAPACK topology Dims = (%d, "
             "%d)\n",
             rank, size_blacscomm, dims[0], dims[1]);
#endif
    // TODO: make it able to use a subgroup of the blacscomm! For now just
    // enforce it.
  ...
  if (pSPARC->bandcomm_index != -1 && pSPARC->dmcomm != MPI_COMM_NULL) {
    if (pSPARC->is_domain_uniform) {
      ...
      Cblacs_gridmap(&pSPARC->ictxt_blacs, usermap_0, pSPARC->npNd,
                     pSPARC->npNd, pSPARC->npband); // row topology
    } else {
      Cblacs_gridmap(&pSPARC->ictxt_blacs, usermap_0, 1, 1,
                     pSPARC->npband); // row topology
    }
  } else {
    Cblacs_gridmap(&pSPARC->ictxt_blacs, usermap_0, 1, 1,
                   dims[0] * dims[1]); // row topology
  }
  free(usermap_0);
  ...
  Cblacs_gridmap(&pSPARC->ictxt_blacs_topo, usermap_1, dims[0], dims[0],
                 dims[1]); // Cart topology
  free(usermap_1);
  free(usermap);
  ...
  nb = (pSPARC->Nstates - 1) / pSPARC->npband +
       1; // equal to ceil(Nstates/npband), for int only
  // set up descriptor for storage of orbitals in ictxt_blacs (original)
  llda = max(1, Nd_blacscomm);
  if (pSPARC->bandcomm_index != -1 && pSPARC->dmcomm != MPI_COMM_NULL) {
    descinit_(&pSPARC->desc_orbitals[0], &Nd_blacscomm, &pSPARC->Nstates, &mb,
              &nb, &ZERO, &ZERO, &pSPARC->ictxt_blacs, &llda, &info);
  } else {
    for (i = 0; i < 9; i++)
  ...
  if (!rank)
    printf("rank = %2d, my blacs rank = %d, BLCYC size (%d, %d), actual size "
           "(%d, %d)\n",
           rank, pSPARC->bandcomm_index, temp_r, temp_c, Nd_blacscomm,
           pSPARC->Nband_bandcomm);
#endif
  ...
  // set up descriptor for block-cyclic format storage of orbitals in
  // ictxt_blacs
  // TODO: make block-cyclic parameters mb and nb input variables!
  mb = max(1, Nd_blacscomm / dims[0]);    // this is only block, no cyclic! Tune
  ...
  if (pSPARC->bandcomm_index != -1 && pSPARC->dmcomm != MPI_COMM_NULL) {
    ...
    pSPARC->nc_orb_BLCYC =
        numroc_(&pSPARC->Nstates, &nb, &mycol, &ZERO, &npcol);
  } else {
  ...
  if (pSPARC->bandcomm_index != -1 && pSPARC->dmcomm != MPI_COMM_NULL) {
    descinit_(&pSPARC->desc_orb_BLCYC[0], &Nd_blacscomm, &pSPARC->Nstates, &mb,
              &nb, &ZERO, &ZERO, &pSPARC->ictxt_blacs_topo, &llda, &info);
  } else {
    for (i = 0; i < 9; i++)
      pSPARC->desc_orb_BLCYC[i] = 0;
  }

  // set up distribution of projected Hamiltonian and the corresponding overlap
  // matrix
  // TODO: Find optimal distribution of the projected Hamiltonian and mass
  // matrix!
  //       For now Hp and Mp are distributed as follows: we distribute them in
  //       the same context topology as the bands. Note that mb = nb!
  ...
  if (pSPARC->useLAPACK == 1) {
    // in this case we will call LAPACK instead to solve the subspace
    // eigenproblem
    ...
    mbQ = nbQ =
        pSPARC->eig_paral_blksz; // block size for storing subspace eigenvectors
  }
#ifdef DEBUG
  if (!rank)
    printf("rank = %d, mb = nb = %d, mbQ = nbQ = %d\n", rank, mb, mbQ);
#endif
  if (pSPARC->bandcomm_index != -1 && pSPARC->dmcomm != MPI_COMM_NULL) {
    pSPARC->nr_Hp_BLCYC = pSPARC->nr_Mp_BLCYC =
        numroc_(&pSPARC->Nstates, &mb, &myrow, &ZERO, &nprow);
    pSPARC->nr_Hp_BLCYC = pSPARC->nr_Mp_BLCYC = max(1, pSPARC->nr_Mp_BLCYC);
    pSPARC->nc_Hp_BLCYC = pSPARC->nc_Mp_BLCYC =
        numroc_(&pSPARC->Nstates, &nb, &mycol, &ZERO, &npcol);
  ...
  if (pSPARC->bandcomm_index != -1 && pSPARC->dmcomm != MPI_COMM_NULL) {
    ...
    for (i = 0; i < 9; i++) {
      // pSPARC->desc_Q_BLCYC[i] = pSPARC->desc_Mp_BLCYC[i] =
      // pSPARC->desc_Hp_BLCYC[i];
      pSPARC->desc_Mp_BLCYC[i] = pSPARC->desc_Hp_BLCYC[i];
    }
    descinit_(&pSPARC->desc_Q_BLCYC[0], &pSPARC->Nstates, &pSPARC->Nstates,
              &mbQ, &nbQ, &ZERO, &ZERO, &pSPARC->ictxt_blacs_topo, &lldaQ,
              &info);
  } else {
    for (i = 0; i < 9; i++) {
      pSPARC->desc_Q_BLCYC[i] = pSPARC->desc_Mp_BLCYC[i] =
          pSPARC->desc_Hp_BLCYC[i] = 0;
    }
  }

#ifdef DEBUG
  if (!rank)
    printf("rank = %d, nr_Hp = %d, nc_Hp = %d\n", rank, pSPARC->nr_Hp_BLCYC,
           pSPARC->nc_Hp_BLCYC);
#endif

  // allocate memory for block cyclic distribution of projected Hamiltonian and
  // mass matrix
  if (pSPARC->isGammaPoint) {
    pSPARC->Hp = (double *)malloc(pSPARC->nr_Hp_BLCYC * pSPARC->nc_Hp_BLCYC *
                                  sizeof(double));
    pSPARC->Mp = (double *)malloc(pSPARC->nr_Mp_BLCYC * pSPARC->nc_Mp_BLCYC *
                                  sizeof(double));
    pSPARC->Q = (double *)malloc(pSPARC->nr_Q_BLCYC * pSPARC->nc_Q_BLCYC *
                                 sizeof(double));
  } else {
    pSPARC->Hp_kpt = (double _Complex *)malloc(
        pSPARC->nr_Hp_BLCYC * pSPARC->nc_Hp_BLCYC * sizeof(double _Complex));
    pSPARC->Mp_kpt = (double _Complex *)malloc(
        pSPARC->nr_Mp_BLCYC * pSPARC->nc_Mp_BLCYC * sizeof(double _Complex));
    pSPARC->Q_kpt = (double _Complex *)malloc(
        pSPARC->nr_Q_BLCYC * pSPARC->nc_Q_BLCYC * sizeof(double _Complex));
  }
  ...
  if (npNd == 0) {
    ...
    pSPARC->npNdz_phi = dims[2];
             ...
             pSPARC->Nx / pSPARC->npNdx_phi < minsize ||
             pSPARC->Ny / pSPARC->npNdy_phi < minsize ||
             pSPARC->Nz / pSPARC->npNdz_phi < minsize) {
    // when domain decomposition parameters are not reasonable
    npNd = nproc;
    SPARC_Dims_create(npNd, 3, gridsizes, minsize, dims, &ierr);
    if (ierr == 1 && rank == 0) {
  ...
  // Processes in bandcomm with rank >= dims[0]*dims[1]*…*dims[d-1] will return
  // MPI_COMM_NULL
  MPI_Cart_create(MPI_COMM_WORLD, 3, dims, periods, 1,
                  &pSPARC->dmcomm_phi); // 1 is to reorder rank
  ...
  if (rank == 0) {
    printf(
        ...
        nproc, pSPARC->Nx, pSPARC->Ny, pSPARC->Nz, dims[0] * dims[1] * dims[2],
        dims[0], dims[1], dims[2], pSPARC->Nx / (double)dims[0],
        pSPARC->Ny / (double)dims[1], pSPARC->Nz / (double)dims[2]);
  }
#endif
  ...
  if (pSPARC->cell_typ != 0) {
    if (pSPARC->dmcomm_phi != MPI_COMM_NULL) {
      ...
      MPI_Dist_graph_create_adjacent(
          pSPARC->dmcomm_phi, nneighb, neighb, (int *)MPI_UNWEIGHTED, nneighb,
          neighb, (int *)MPI_UNWEIGHTED, MPI_INFO_NULL, 0,
          &pSPARC->comm_dist_graph_phi); // creates a distributed graph topology
                                         // (adjacent, cartesian cubical)
      // pSPARC->dmcomm_phi = pSPARC->comm_dist_graph_phi;
      free(neighb);
    }
  ...
  pSPARC->lambda = (double *)calloc(pSPARC->Nstates * pSPARC->Nkpts_kptcomm *
                                        pSPARC->Nspin_spincomm,
                                    sizeof(double));
  ...
  // allocate memory for storing eigenvalues
  pSPARC->occ = (double *)calloc(pSPARC->Nstates * pSPARC->Nkpts_kptcomm *
                                     pSPARC->Nspin_spincomm,
                                 sizeof(double));
  ...
  pSPARC->eigmin = (double *)calloc(
      pSPARC->Nkpts_kptcomm * pSPARC->Nspin_spincomm, sizeof(double));
  pSPARC->eigmax = (double *)calloc(
      pSPARC->Nkpts_kptcomm * pSPARC->Nspin_spincomm, sizeof(double));
  ...
  if (pSPARC->dmcomm != MPI_COMM_NULL && pSPARC->bandcomm_index >= 0) {
    pSPARC->Veff_loc_dmcomm =
        (double *)malloc(pSPARC->Nd_d_dmcomm * pSPARC->Nspin * sizeof(double));
    assert(pSPARC->Veff_loc_dmcomm != NULL);
  }

  // if (pSPARC->npkpt >= 1 ) { //&& pSPARC->kptcomm_topo != MPI_COMM_NULL
  pSPARC->Veff_loc_kptcomm_topo =
      (double *)malloc(pSPARC->Nd_d_kptcomm * sizeof(double));
  ...
  if (pSPARC->isGammaPoint != 1 && pSPARC->kptcomm_topo != MPI_COMM_NULL) {
    pSPARC->Lanczos_x0_complex = (double _Complex *)malloc(
        pSPARC->Nd_d_kptcomm * pSPARC->Nspinor * sizeof(double _Complex));
  ...
  if (pSPARC->dmcomm_phi != MPI_COMM_NULL) {
    ...
    DMnd = DMnx * DMny * DMnz;
    // allocate memory for electron density (sum of atom potential) and charge
    // density
    pSPARC->electronDens_at =
        (double *)malloc(DMnd * (2 * pSPARC->Nspin - 1) * sizeof(double));
    pSPARC->electronDens_core =
        (double *)calloc(DMnd * (2 * pSPARC->Nspin - 1), sizeof(double));
    ...
    if (pSPARC->MDFlag == 1 || pSPARC->RelaxFlag == 1 ||
      ...
      pSPARC->atom_pos_nm =
          (double *)malloc(3 * pSPARC->n_atom * sizeof(double));
    ...
    pSPARC->XCPotential =
        (double *)malloc(DMnd * pSPARC->Nspin * sizeof(double));
    ...
    // if GGA then allocate for xc energy per particle for each grid point and
    // der. wrt. grho
    if (strcmpi(pSPARC->XC, "GGA_PBE") == 0 ||
        strcmpi(pSPARC->XC, "GGA_RPBE") == 0 ||
        strcmpi(pSPARC->XC, "GGA_PBEsol") == 0 ||
        strcmpi(pSPARC->XC, "PBE0") == 0 || strcmpi(pSPARC->XC, "HF") == 0 ||
        strcmpi(pSPARC->XC, "HSE") == 0 || strcmpi(pSPARC->XC, "SCAN") == 0 ||
        strcmpi(pSPARC->XC, "vdWDF1") == 0 ||
        strcmpi(pSPARC->XC, "vdWDF2") == 0) {
      pSPARC->Dxcdgrho =
          (double *)malloc(DMnd * (2 * pSPARC->Nspin - 1) * sizeof(double));
      assert(pSPARC->Dxcdgrho != NULL);
    }

    pSPARC->Veff_loc_dmcomm_phi =
        (double *)malloc(DMnd * pSPARC->Nspin * sizeof(double));
    pSPARC->mixing_hist_xk =
        (double *)malloc(DMnd * pSPARC->Nspin * sizeof(double));
    pSPARC->mixing_hist_fk =
        (double *)calloc(DMnd * pSPARC->Nspin, sizeof(double));
    pSPARC->mixing_hist_fkm1 =
        (double *)calloc(DMnd * pSPARC->Nspin, sizeof(double));
    pSPARC->mixing_hist_xkm1 =
        (double *)malloc(DMnd * pSPARC->Nspin * sizeof(double));
    pSPARC->mixing_hist_Xk = (double *)malloc(
        DMnd * pSPARC->Nspin * pSPARC->MixingHistory * sizeof(double));
    pSPARC->mixing_hist_Fk = (double *)malloc(
        DMnd * pSPARC->Nspin * pSPARC->MixingHistory * sizeof(double));
    assert(pSPARC->Veff_loc_dmcomm_phi != NULL &&
           pSPARC->mixing_hist_xk != NULL && pSPARC->mixing_hist_fk != NULL &&
           pSPARC->mixing_hist_fkm1 != NULL &&
           pSPARC->mixing_hist_xkm1 != NULL && pSPARC->mixing_hist_Xk != NULL &&
           pSPARC->mixing_hist_Fk != NULL);

    if (pSPARC->MixingVariable ==
        1) { // for potential mixing, the history is stored already
      pSPARC->Veff_loc_dmcomm_phi_in = pSPARC->mixing_hist_xk;
    } else { // for denstiy mixing, need extra memory to store potential history
      pSPARC->Veff_loc_dmcomm_phi_in =
          (double *)malloc(DMnd * pSPARC->Nspin * sizeof(double));
      assert(pSPARC->Veff_loc_dmcomm_phi_in != NULL);
    }

    // The following rho_in and phi_in are only used for evaluating QE scf
    // errors
    ...
    pSPARC->mixing_hist_Pfk =
        (double *)calloc(DMnd * pSPARC->Nspin, sizeof(double));

    // initialize electrostatic potential as random guess vector
    if (pSPARC->FixRandSeed == 1) {
      SeededRandVec(pSPARC->elecstPotential, pSPARC->DMVertices, gridsizes,
                    -1.0, 1.0, 0);
    } else {
      srand(rank + 1);
      double rand_min = -1.0, rand_max = 1.0;
      for (i = 0; i < DMnd; i++) {
        pSPARC->elecstPotential[i] = rand_min + (rand_max - rand_min) *
                                                    (double)rand() /
                                                    RAND_MAX; // or 1.0
      }
    }
  ...
  // Note: Set_D2D_target require gridsizes, rdims, sdims to be global in
  // union{send_comm, recv_comm},
  //       i.e., all processes know about these values and they are the same in
  //       all processes
  int sdims[3], rdims[3];
  gridsizes[0] = pSPARC->Nx;
  gridsizes[1] = pSPARC->Ny;
  ...
  Set_D2D_Target(&pSPARC->d2d_dmcomm_phi, &pSPARC->d2d_dmcomm, gridsizes,
                 pSPARC->DMVertices, pSPARC->DMVertices_dmcomm,
                 pSPARC->dmcomm_phi, sdims,
                 (pSPARC->spincomm_index == 0 && pSPARC->kptcomm_index == 0 &&
                  pSPARC->bandcomm_index == 0)
                     ? pSPARC->dmcomm
                     : MPI_COMM_NULL,
  ...
  // If found rank order in the cartesian topology is different for dmcomm_phi
  // and kptcomm_topo, consider to add MPI_Bcast for is_phi_eq_kpt_topo
  ...
  if (((pSPARC->chefsibound_flag == 0 || pSPARC->chefsibound_flag == 1) &&
       pSPARC->spincomm_index >= 0 && pSPARC->kptcomm_index >= 0 &&
       (pSPARC->spin_typ != 0 || !pSPARC->is_phi_eq_kpt_topo ||
        !pSPARC->isGammaPoint)) ||
      (pSPARC->usefock > 0)) {
    gridsizes[0] = pSPARC->Nx;
    gridsizes[1] = pSPARC->Ny;
    ...
    Set_D2D_Target(&pSPARC->d2d_dmcomm_lanczos, &pSPARC->d2d_kptcomm_topo,
                   gridsizes, pSPARC->DMVertices_dmcomm,
...
// parallelization summary
  ...
  if (rank == 0) {
    ...
    printf("# of FD-grid points per processor: %d = (%d,%d,%d)\n",
           pSPARC->Nd_d_dmcomm, pSPARC->Nx_d_dmcomm, pSPARC->Ny_d_dmcomm,
           pSPARC->Nz_d_dmcomm);
    printf("-----------------------------------------------\n");
    printf("== Phi domain ==\n");
    ...
    printf("Embeded Cartesian topology dims: (%d,%d,%d)\n", pSPARC->npNdx_phi,
           pSPARC->npNdy_phi, pSPARC->npNdz_phi);
    printf("# of FD-grid points per processor: %d = (%d,%d,%d)\n", pSPARC->Nd_d,
           pSPARC->Nx_d, pSPARC->Ny_d, pSPARC->Nz_d);
    printf("-----------------------------------------------\n");
  }
#endif
}
 ...
 */
void SPARC_Dims_create(int nproc, int ndims, int *gridsizes, int minsize,
                       int *dims, int *ierr) {
#define NPSTRIDE 5
#define NPSTRIDE3D 2
  *ierr = 0;
  ...
  if (ndims == 1) {
    ...
    *dims = max(*dims, 1);
    ...
    int i, j, count, best_ind, max_npi, tmpint1, tmpint2,
        dimx[(NPSTRIDE * 2 + 1) * 2], dimy[(NPSTRIDE * 2 + 1) * 2],
        npi[(NPSTRIDE * 2 + 1) * 2];
    ...
    for (i = 0; i < (NPSTRIDE * 2 + 1); i++) {
      ...
      dimx[count] =
          (tmpint1 > 0)
              ? (tmpint1 <= nproc ? tmpint1 : (tmpint1 - (NPSTRIDE * 2 + 1)))
              : (tmpint1 + (NPSTRIDE * 2 + 1));
      // dimx[count] > 0 ? (dimx[count] <= nproc ? : (dimx[count] = 1)) :
      // (dimx[count] = 1);
      if (dimx[count] > 0 && dimx[count] <= nproc) {
        SPARC_Dims_create(max(1, nproc / dimx[count]), 1, &gridsizes[1],
                          minsize, &dimy[count], ierr);
    ...
    for (j = 0; j < (NPSTRIDE * 2 + 1); j++) {
      ...
      dimy[count] =
          (tmpint2 > 0)
              ? (tmpint2 <= nproc ? tmpint2 : (tmpint2 - (NPSTRIDE * 2 + 1)))
              : (tmpint2 + (NPSTRIDE * 2 + 1));
      // dimy[count] > 0 ? (dimy[count] <= nproc ? : (dimy[count] = 1)) :
      // (dimy[count] = 1);
      if (dimy[count] > 0 && dimy[count] <= nproc) {
        SPARC_Dims_create(max(1, nproc / dimy[count]), 1, &gridsizes[0],
                          minsize, &dimx[count], ierr);
        npi[count] = dimx[count] * dimy[count];
        tmp1 = ((double)gridsizes[0]) / dimx[count];
        tmp2 = ((double)gridsizes[1]) / dimy[count];
        rnormi[count] = (tmp1 - tmp2) * (tmp1 - tmp2);
    ...
    for (i = 0; i < (NPSTRIDE * 2 + 1) * 2; i++) {
      ...
      if (npi[count] > max_npi && gridsizes[0] / dimx[count] >= minsize &&
          gridsizes[1] / dimy[count] >= minsize) {
        best_ind = count;
        max_npi = npi[count];
    ...
    // TODO: after first scan, perhaps we can allow np to be up to 3% smaller,
    // and choose the one with smaller rnormi, the idea is that by reducing
    // total number of process we lose speed by 3% or less, but we might gain
    // speed up in communication by more than 3%
    if (best_ind != -1) {
      dims[0] = dimx[best_ind];
      dims[1] = dimy[best_ind];
    } else {
      dims[0] = nproc;
      dims[1] = 1;
      ;
    ...
    int i, j, k, count, best_ind, max_npi, tmpint1, tmpint2, tmpint3,
        dims_temp[2], gridsizes_temp[2];
    int dimx[(NPSTRIDE3D * 2 + 1) * 3], dimy[(NPSTRIDE3D * 2 + 1) * 3],
        dimz[(NPSTRIDE3D * 2 + 1) * 3], npi[(NPSTRIDE3D * 2 + 1) * 3];
    r = cbrt((double)nproc) / cbrt((double)gridsizes[0] * (double)gridsizes[1] *
                                   (double)gridsizes[2]);
    r = min(r, 1 / (double)minsize);
    double r2 = r * r;
    ...
    for (i = 0; i < (NPSTRIDE3D * 2 + 1); i++) {
      ...
      dimx[count] =
          (tmpint1 > 0)
              ? (tmpint1 <= nproc ? tmpint1 : (tmpint1 - (NPSTRIDE3D * 2 + 1)))
              : (tmpint1 + (NPSTRIDE3D * 2 + 1));
      // dimx[count] > 0 ? (dimx[count] <= nproc ? : (dimx[count] = 1)) :
      // (dimx[count] = 1);
      if (dimx[count] > 0 && dimx[count] <= nproc) {
        SPARC_Dims_create(max(1, nproc / dimx[count]), 2, &gridsizes[1],
                          minsize, dims_temp, ierr);
    ...
    for (j = 0; j < (NPSTRIDE3D * 2 + 1); j++) {
      ...
      dimy[count] =
          (tmpint2 > 0)
              ? (tmpint2 <= nproc ? tmpint2 : (tmpint2 - (NPSTRIDE3D * 2 + 1)))
              : (tmpint2 + (NPSTRIDE3D * 2 + 1));
      // dimy[count] > 0 ? (dimy[count] <= nproc ? : (dimy[count] = 1)) :
      // (dimy[count] = 1);
      if (dimy[count] > 0 && dimy[count] <= nproc) {
        SPARC_Dims_create(max(1, nproc / dimy[count]), 2, gridsizes_temp,
                          minsize, dims_temp, ierr);
    ...
    for (k = 0; k < (NPSTRIDE3D * 2 + 1); k++) {
      ...
      dimz[count] =
          (tmpint3 > 0)
              ? (tmpint3 <= nproc ? tmpint3 : (tmpint3 - (NPSTRIDE3D * 2 + 1)))
              : (tmpint3 + (NPSTRIDE3D * 2 + 1));
      // for these cases, one can actually skip the calculation
      // dimz[count] > 0 ? (dimz[count] <= nproc ? : (dimz[count] = 1)) :
      // (dimz[count] = 1);
      if (dimz[count] > 0 && dimz[count] <= nproc) {
        SPARC_Dims_create(max(1, nproc / dimz[count]), 2, &gridsizes[0],
                          minsize, dims_temp, ierr);
        dimx[count] = dims_temp[0];
    ...
    for (i = 0; i < (NPSTRIDE3D * 2 + 1) * 3; i++) {
      ...
      if (npi[count] > max_npi && gridsizes[0] / dimx[count] >= minsize &&
          gridsizes[1] / dimy[count] >= minsize &&
          gridsizes[2] / dimz[count] >= minsize) {
        best_ind = count;
        max_npi = npi[count];
        min_rnorm = rnormi[count];
      } else if (npi[count] == max_npi && rnormi[count] < min_rnorm) {
...
/**
 * @brief   Look for a 2D decomposition of number of processors according to
 * application size. This function is adapted to optimizing 2D block cyclic
 * distribution of matrices, where the number of processors are kept fixed and
 * number of nodes/direction is not substantial regarding performance. The goal
 * is to factorize nproc = a x b (a >= b), such that a and b are as close to
...
void ScaLAPACK_Dims_2D_BLCYC(int nproc, int *gridsizes, int *dims) {
  ...
  for (i = 0; i < ftors.count / 2; i++) {
    diff1 = fabs((double)gridsizes[1] / ftors.list[2 * i] -
                 (double)gridsizes[0] / ftors.list[2 * i + 1]);
    diff2 = fabs((double)gridsizes[0] / ftors.list[2 * i] -
                 (double)gridsizes[1] / ftors.list[2 * i + 1]);
  ...
  if (ftors.count % 2 && i == ftors.count / 2) {
    diff1 = fabs((double)gridsizes[1] / ftors.list[ftors.count - 1] -
                 (double)gridsizes[0] / ftors.list[ftors.count - 1]);
    if (diff1 < diff) {
 ...
 *
 **/
int workload_assigned(const int Nk, const int Ns, const int np1,
                      const int np2) {
 ...
 *                      ~= ceil(Nk/np1)*ceil(Ns/np2)* (Nd/np3) #since Nd is
 *large
 ...
 *
 **/
double work_load_efficiency(const int Nk, const int Ns, const int np,
 ...
 *
 *         For a given number of processors, choose np1 and np2 so that the
 *maximum work assigned to each process is the smallest, i.e., choose x * y <=
 *np, s.t., ceil(N1/x) * ceil(N2/y) is minimum.
 *
 *         Here we assume the parallelization of the two properties are the
 *same, therefore there's no preference to use more or less processes in any
 *         dimension.
 *
 *         The objective function and the constraint are both quadratic, which
 *         makes this a very hard problem in general.
 *
 *         In this func, we try to find a reasonably good solution using the
 *following strategy: note that if x0 | N1 and y0 | N2 ( "|" means divides),
 *then (x0,y0) is a solution to the problem with a weaker constraint x*y <=
 *x0*y0. We can then keep reducing the constraint from np down until we find a
 *         solution for the subproblem, and choose the best of all the searched
 *combo.
 *
 * @param N1   Number of properties to parallelize on the grid in the 1st
 *dimension.
 * @param N2   Number of properties to parallelize on the grid in the 2nd
 *dimension.
 * @param np   Number of processors available.
 *
 **/
void dims_divide_2d(const int N1, const int N2, const int np, int *np1,
                    int *np2) {
  int search_count = 0;
 ...
 *         parallelization.
 *
 *         For a given number of processors, choose npkpt, npband, and npdomain
 *so that the maximum work assigned to each process is the smallest, i.e.,
 *         choose x * y * z <= np, s.t.,
 ...
 *
 *         In this func, we try to find a reasonably good solution using the
 *following strategy: since the parallelization over kpoint and band are both
 *very efficient, we try to parallel over kpoint and band first, then we
 *consider parallelization over domain. There are two cases: np <=  or > Nk *
 *Ns. Case 1: np <= Nk * Ns. Then we try to fix npdomain = 1:10, and find the
 *best parameters for the given npdomain, we always prefer to use less npdomain
 *if possible. Case 2: np > Nk * Ns. Then we try to provide Nk*Ns ./ [1:10] for
 *kpoint and band parallelization, and pick the best combination. Again we
 *prefer to use as less npdomain (more for K & B) if work load is the similar.
 *
 * @param Nk        Number of kpoints (after symmetry reduction).
 ...
 **/
void dims_divide_kbd(const int Nk, const int Ns, const int *gridsizes,
...
double skbd_band_efficiency(const int x) {
  ...
  for (int i = 0; i < len - 1; i++) {
    if (x >= x_table[i] && x < x_table[i + 1]) {
      double slope =
          (y_table[i + 1] - y_table[i]) / (x_table[i + 1] - x_table[i]);
 ...
 *
 *         Adding a side effect for tall thin local psi where applying ACE
 * operator is slow
 */
double scaling_DMndbyNband(const int Ns, const int *gridsizes, const int npb,
                           const int npd) {
...
double skbd_weighted_efficiency(const int Nspin, const int Nk, const int Ns,
                                const int *gridsizes, const int np,
                                const int nps, const int npk, const int npb,
                                const int npd, const int isfock) {
  double eff;
...
double work_load_efficiency_fock(const int Nk, const int Ns,
                                 const int *gridsizes, const int np,
                                 const int np1, const int np2, const int np3) {
  int ncols = workload_assigned(Nk, Ns, np1, np2);
...
void dims_divide_kbd_fock(const int Nk, const int Ns, const int *gridsizes,
                          const int np, int *np1, int *np2, int *np3,
                          const int minsize) {
#define LEN_NPB 20
  ...
  double best_weight =
      work_load_efficiency_fock(Nk, Ns, gridsizes, np, cur1, cur2, cur3);
  ...
  for (int i = 0; i < npb_count; i++) {
    ...
    weight =
        work_load_efficiency_fock(Nk, Ns, gridsizes, np, npkpt, npband, npdm);
    if (weight > best_weight) {
      cur1 = npkpt;
      cur2 = npband;
      cur3 = npdm;
 ...
 **/
void dims_divide_skbd(const int Nspin, const int Nk, const int Ns,
                      const int *gridsizes, const int np, int *nps, int *npk,
                      int *npb, int *npd, int minsize, int isfock) {
  if (Nspin != 1 && Nspin != 2)
    exit(1);
  ...
  weight_cur = skbd_weighted_efficiency(Nspin, Nk, Ns, gridsizes, np, nps_cur,
                                        npk_cur, npb_cur, npd_cur, isfock);
  ...
  if (!isfock) {
    ...
    dims_divide_kbd_fock(Nk, Ns, gridsizes, np / (nps_1), &npk_1, &npb_1,
                         &npd_1, minsize);
  }
  // double weight_1 =
  // work_load_efficiency(Nspin*Nk,Ns,np,nps_1*npk_1,npb_1,npd_1);
  double weight_1;
  weight_1 = skbd_weighted_efficiency(Nspin, Nk, Ns, gridsizes, np, nps_1,
                                      npk_1, npb_1, npd_1, isfock);
  ...
  if (Nspin > 1 && np > 1) {
    ...
    if (!isfock) {
      ...
      dims_divide_kbd_fock(Nk, Ns, gridsizes, np / (nps_2), &npk_2, &npb_2,
                           &npd_2, minsize);
    }
    double weight_2;
    weight_2 = skbd_weighted_efficiency(Nspin, Nk, Ns, gridsizes, np, nps_2,
                                        npk_2, npb_2, npd_2, isfock);
...
/**
 * @brief   This function sets up the input structures needed for the function
 * D2D(), which transfers data from one domain decomposition to another domain
 *          decomposition. It's required that send_comm, recv_comm and
 * union_comm overlap at the same root process.
 *
 *          One can first call this function to set up d2d_sender and d2d_recvr,
 * which contains the number of processes to communicate with along with their
 * ranks in the union_comm. Note: In principle, it is not required to call this
 * function if there're other ways to find out the senders and receivers for
 * each process.
 */
void Set_D2D_Target(D2D_OBJ *d2d_sender, D2D_OBJ *d2d_recvr, int *gridsizes,
                    int *sDMVert, int *rDMVert, MPI_Comm send_comm, int *sdims,
                    MPI_Comm recv_comm, int *rdims, MPI_Comm union_comm) {
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
#ifdef DEBUG
  double t3, t4;
  ...
  int rank_recv_comm, nproc_recv_comm, rank_send_comm, nproc_send_comm,
      nproc_union_comm, rank_union_comm, coords_send_comm[3],
      coords_recv_comm[3], send_coord_start[3], send_coord_end[3], nsend[3],
      *send_coords, *rcounts_send_comm, *scounts_send_comm, *displs_send_comm,
      *scoords_vec, *srank_vec, *srank_vec_union, n, i, j, count, coord_temp[3],
      nsend_tot, nrecv_tot, recv_coord_start[3], recv_coord_end[3], nrecv[3],
      *recv_coords, *rcounts_recv_comm, *scounts_recv_comm, *displs_recv_comm,
      *rcoords_vec, *rrank_vec, *rrank_vec_union;
  ...
  if (send_comm != MPI_COMM_NULL) {
    ...
    nsend_tot = 1;
    // find out in each dimension, how many recv processes the local domain
    // spans over
    for (n = 0; n < 3; n++) {
      send_coord_start[n] =
          block_decompose_rank(gridsizes[n], rdims[n], sDMVert[n * 2]);
      send_coord_end[n] =
          block_decompose_rank(gridsizes[n], rdims[n], sDMVert[n * 2 + 1]);
    ...
    // find out all the coordinates of the receiver processes in the recv_comm
    // Cart Topology
    ...
    t4 = MPI_Wtime();
    if (rank == 0)
      printf("======Set_D2D_Target: find receivers in each process (c_ndgrid) "
             "in send_comm took %.3f ms\n",
             (t4 - t3) * 1e3);
    t3 = MPI_Wtime();
#endif
    ...
    if (rank_send_comm == 0) { // we require both comms overlap at rank 0
      ...
      MPI_Gather(&nsend_tot, 1, MPI_INT, scounts_send_comm, 1, MPI_INT, 0,
                 send_comm);
      ...
      scoords_vec =
          (int *)malloc(displs_send_comm[nproc_send_comm] * sizeof(int));
      srank_vec =
          (int *)malloc(displs_send_comm[nproc_send_comm] / 3 * sizeof(int));
      srank_vec_union =
          (int *)malloc(displs_send_comm[nproc_send_comm] / 3 * sizeof(int));

      // gather the coordinates
      MPI_Gatherv(send_coords, nsend_tot * 3, MPI_INT, scoords_vec,
                  rcounts_send_comm, displs_send_comm, MPI_INT, 0, send_comm);

      // In root process, find out rank corresponding to each coord in
      // recv_comm, this works because root process is in both send_comm and
      // recv_comm
      count = 0;
      for (n = 0; n < nproc_send_comm; n++) {
        ...
        for (j = 0; j < scounts_send_comm[n]; j++) {
          ...
          coord_temp[1] =
              scoords_vec[displs_send_comm[n] + scounts_send_comm[n] + j];
          coord_temp[2] =
              scoords_vec[displs_send_comm[n] + 2 * scounts_send_comm[n] + j];
          // find rank corresponding to send_coords in recv_comm
          MPI_Cart_rank(recv_comm, coord_temp, &srank_vec[count]);
          count++;
        }
      }
      // Translate ranks in recv_comm to ranks in union_comm using
      // MPI_Group_translate_ranks
      MPI_Group_translate_ranks(recv_group, count, srank_vec, union_group,
                                srank_vec_union);
      ...
      MPI_Scatterv(srank_vec_union, scounts_send_comm, displs_send_comm,
                   MPI_INT, d2d_sender->target_ranks, nsend_tot, MPI_INT, 0,
                   send_comm);
    } else {
    ...
    t4 = MPI_Wtime();
    if (rank == 0)
      printf("======Set_D2D_Target: Gather and Scatter receivers in send_comm "
             "took %.3f ms\n",
             (t4 - t3) * 1e3);
#endif
  }
  ...
  if (recv_comm != MPI_COMM_NULL) {
    ...
    nrecv_tot = 1;
    // find in each dimension, how many send processes the local domain spans
    // over
    for (n = 0; n < 3; n++) {
      recv_coord_start[n] =
          block_decompose_rank(gridsizes[n], sdims[n], rDMVert[n * 2]);
      recv_coord_end[n] =
          block_decompose_rank(gridsizes[n], sdims[n], rDMVert[n * 2 + 1]);
    ...
    // Gather all the recv_coords in root process to find out their ranks in
    // send_comm
    if (rank_recv_comm == 0) {
      ...
      // first gather the number of coordinates to be recived from each process
      // in recv_comm
      MPI_Gather(&nrecv_tot, 1, MPI_INT, scounts_recv_comm, 1, MPI_INT, 0,
                 recv_comm);
      ...
      rcoords_vec =
          (int *)malloc(displs_recv_comm[nproc_recv_comm] * sizeof(int));
      rrank_vec =
          (int *)malloc(displs_recv_comm[nproc_recv_comm] / 3 * sizeof(int));
      rrank_vec_union =
          (int *)malloc(displs_recv_comm[nproc_recv_comm] / 3 * sizeof(int));

      // gather the coordinates
      MPI_Gatherv(recv_coords, nrecv_tot * 3, MPI_INT, rcoords_vec,
                  rcounts_recv_comm, displs_recv_comm, MPI_INT, 0, recv_comm);

      // In root process, find out rank corresponding to each coord in
      // send_comm, this works because root process is in both send_comm and
      // recv_comm
      count = 0;
      for (n = 0; n < nproc_recv_comm; n++) {
        ...
        for (j = 0; j < scounts_recv_comm[n]; j++) {
          ...
          coord_temp[1] =
              rcoords_vec[displs_recv_comm[n] + scounts_recv_comm[n] + j];
          coord_temp[2] =
              rcoords_vec[displs_recv_comm[n] + 2 * scounts_recv_comm[n] + j];
          // find rank corresponding to recv_coords in send_comm
          MPI_Cart_rank(send_comm, coord_temp, &rrank_vec[count++]);
        }
      }

      // Translate ranks in send_comm to ranks in union_comm using
      // MPI_Group_translate_ranks
      MPI_Group_translate_ranks(send_group, count, rrank_vec, union_group,
                                rrank_vec_union);
      ...
      MPI_Scatterv(rrank_vec_union, scounts_recv_comm, displs_recv_comm,
                   MPI_INT, d2d_recvr->target_ranks, nrecv_tot, MPI_INT, 0,
                   recv_comm);
...
/**
 * @brief   Free D2D_OBJ structure created by Set_D2D_Target or otherwise
 * created manually as input to D2D.
 */
void Free_D2D_Target(D2D_OBJ *d2d_sender, D2D_OBJ *d2d_recvr,
                     MPI_Comm send_comm, MPI_Comm recv_comm) {
  if (send_comm != MPI_COMM_NULL) {
 ...
 *
 *          This function has the same objective as DD2DD. However, the process
 * of finding sender and receiver ranks are seperated out from this function.
 * The reason for this change is that 1: the ranks remain the same for every
 * simulation, so it only has to be done once, 2: finding senders and receivers
 * can become costly for large number of processes.
 *
 *          Note: In this function we assume Domain Decomposition is
 * quasi-uniform, i.e., each process will roughly have the same number of nodes.
 * If the dim of the Cartesian in one direction does not divide the gridsize,
 * the first few processes will have one more node than the others.
 *
 * @param sdata         (INPUT) Local piece of the data owned by processes in
 * send_comm.
 * @param rdata         (OUTPUT) Re-distributed data owned by processes in
 * recv_comm.
 * @param send_comm     Original communicator in which the data was distributed.
 * Processors not part of the original DD should provide MPI_COMM_NULL for this
 * argument. send_comm is assumed to have a Cartesian topology.
 * @param sdims         Dimensions of the send_comm.
 * @param recv_comm     New Domain Decomposition communicator, processes that
 * are not part of this should provide MPI_COMM_NULL. recv_comm is assumed to
 * have a Cartesian topology.
 * @param rdims         Dimensions of the recv_comm.
 * @param union_comm    Union communicator. Union communicator can be any
 * communicator that satisfy the following conditions:
 *                          1. send_comm and recv_comm are both sub-comms of
 * union_comm.
 *                          2. it is not required in this function that
 * send_comm, recv_comm and union_comm share root process any more. As long as
 * senders and receivers are provided correctly.
 */
void D2D(D2D_OBJ *d2d_sender, D2D_OBJ *d2d_recvr, int *gridsizes, int *sDMVert,
         double *sdata, int *rDMVert, double *rdata, MPI_Comm send_comm,
         int *sdims, MPI_Comm recv_comm, int *rdims, MPI_Comm union_comm) {
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  ...
  int ndims, rank_recv_comm, rank_send_comm, coords_send_comm[3],
      coords_recv_comm[3];
  ndims = 3;
  ...
  if (send_comm != MPI_COMM_NULL) {
    ...
    t4 = MPI_Wtime();
    if (rank == 0)
      printf("======D2D: find receivers' coords in each process (c_ndgrid) in "
             "send_comm took %.3f ms\n",
             (t4 - t3) * 1e3);
#endif
    ...
    for (n = 0; n < nsend_tot; n++) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        DMVert_temp[2 * i] =
            block_decompose_nstart(gridsizes[i], rdims[i], coord_temp[i]);
        ...
        DMVert_temp[2 * i + 1] =
            min(DMVert_temp[2 * i + 1], sDMVert[2 * i + 1]);
      ...
      MPI_Isend(sendbuf[n], DMnd, MPI_DOUBLE, d2d_sender->target_ranks[n], 111,
                union_comm, &send_request[n]);
    }
  }
#ifdef DEBUG
  if (rank == 0)
    printf("======D2D: finished initiating send_comm! Start entering receivers "
           "comm\n");
#endif
  ...
  if (recv_comm != MPI_COMM_NULL) {
    ...
    t4 = MPI_Wtime();
    if (rank == 0)
      printf("======D2D: find senders' coords in each process (c_ndgrid) in "
             "recv_comm took %.3f ms\n",
             (t4 - t3) * 1e3);
#endif
    ...
    for (n = 0; n < nrecv_tot; n++) {
      ...
      for (i = 0; i < ndims; i++) {
        ...
        DMVert_temp[2 * i] =
            block_decompose_nstart(gridsizes[i], sdims[i], coord_temp[i]);
        DMnxi[i] = block_decompose(gridsizes[i], sdims[i], coord_temp[i]);
        DMVert_temp[2 * i + 1] = DMVert_temp[2 * i] + DMnxi[i] - 1;
        // find intersect of local domains in send process and receiver process
        DMVert_temp[2 * i] = max(DMVert_temp[2 * i], rDMVert[2 * i]);
        DMVert_temp[2 * i + 1] =
            min(DMVert_temp[2 * i + 1], rDMVert[2 * i + 1]);
        DMnxi[i] = DMVert_temp[2 * i + 1] - DMVert_temp[2 * i] + 1;
        DMnd *= DMnxi[i];
      }
      recvbuf[n] = (double *)malloc(DMnd * sizeof(double));
      MPI_Irecv(recvbuf[n], DMnd, MPI_DOUBLE, d2d_recvr->target_ranks[n], 111,
                union_comm, &recv_request[n]);
    }
  }
  ...
  t2 = MPI_Wtime();
  if (rank == 0)
    printf("======D2D: initiated sending and receiving took %.3f ms\n",
           (t2 - t1) * 1e3);
#endif
 ...
 */
void D2D_kpt(D2D_OBJ *d2d_sender, D2D_OBJ *d2d_recvr, int *gridsizes,
             int *sDMVert, double _Complex *sdata, int *rDMVert,
             double _Complex *rdata, MPI_Comm send_comm, int *sdims,
             MPI_Comm recv_comm, int *rdims, MPI_Comm union_comm) {
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  ...
  if (send_comm != MPI_COMM_NULL) {
    ...
    for (n = 0; n < nsend_tot; n++) {
      ...
      MPI_Isend(sendbuf[n], DMnd, MPI_DOUBLE_COMPLEX,
                d2d_sender->target_ranks[n], 111, union_comm, &send_request[n]);
    }
  }
#ifdef DEBUG
  if (rank == 0)
  ...
  if (recv_comm != MPI_COMM_NULL) {
    ...
    for (n = 0; n < nrecv_tot; n++) {
      ...
      MPI_Irecv(recvbuf[n], DMnd, MPI_DOUBLE_COMPLEX,
                d2d_recvr->target_ranks[n], 111, union_comm, &recv_request[n]);
    }
  }
...
void DD2DD(SPARC_OBJ *pSPARC, int *gridsizes, int *sDMVert, double *sdata,
           int *rDMVert, double *rdata, MPI_Comm send_comm, int *sdims,
           MPI_Comm recv_comm, int *rdims, MPI_Comm union_comm) {
  D2D_OBJ d2d_sender, d2d_recvr;
  Set_D2D_Target(&d2d_sender, &d2d_recvr, gridsizes, sDMVert, rDMVert,
                 send_comm, sdims, recv_comm, rdims, union_comm);
  D2D(&d2d_sender, &d2d_recvr, gridsizes, sDMVert, sdata, rDMVert, rdata,
      send_comm, sdims, recv_comm, rdims, union_comm);
...
void copy_mat_blk(const size_t unit_size, const void *src_, const int lds,
...
void BP2DP(const MPI_Comm comm, const int nproc, const int nrow_bp,
           const int ncol_bp, const int *row_dp_displs,
           const int *bp2dp_sendcnts, const int *bp2dp_sdispls,
           const int *dp2bp_sendcnts, const int *dp2bp_sdispls,
           const int unit_size, const void *bp_data, void *packbuf,
           void *dp_data) {
  ...
  if (unit_size == 8) {
    ...
    for (int i = 0; i < nproc; i++) {
      ...
      copy_mat_blk(unit_size, src_ptr, nrow_bp, ncol_bp, nrow_dp_i, dst_ptr,
                   nrow_dp_i);
    }
  }
  if (unit_size == 16) {
  ...
  // Do MPI_Alltoallv
  if (unit_size == 8) {
    MPI_Alltoallv(packbuf, bp2dp_sendcnts, bp2dp_sdispls, MPI_DOUBLE, dp_data,
                  dp2bp_sendcnts, dp2bp_sdispls, MPI_DOUBLE, comm);
  }
  if (unit_size == 16) {
    MPI_Alltoallv(packbuf, bp2dp_sendcnts, bp2dp_sdispls, MPI_C_DOUBLE_COMPLEX,
                  dp_data, dp2bp_sendcnts, dp2bp_sdispls, MPI_C_DOUBLE_COMPLEX,
                  comm);
  }
}

void DP2BP(const MPI_Comm comm, const int nproc, const int nrow_bp,
           const int ncol_bp, const int *row_dp_displs,
           const int *bp2dp_sendcnts, const int *bp2dp_sdispls,
           const int *dp2bp_sendcnts, const int *dp2bp_sdispls,
           const int unit_size, const void *dp_data, void *packbuf,
           void *bp_data) {
  // Do MPI_Alltoallv
  if (unit_size == 8) {
    MPI_Alltoallv(dp_data, dp2bp_sendcnts, dp2bp_sdispls, MPI_DOUBLE, packbuf,
                  bp2dp_sendcnts, bp2dp_sdispls, MPI_DOUBLE, comm);
  }
  if (unit_size == 16) {
    MPI_Alltoallv(dp_data, dp2bp_sendcnts, dp2bp_sdispls, MPI_C_DOUBLE_COMPLEX,
  ...
  if (unit_size == 8) {
    ...
    for (int i = 0; i < nproc; i++) {
      ...
      copy_mat_blk(unit_size, src_ptr, nrow_dp_i, ncol_bp, nrow_dp_i, dst_ptr,
                   nrow_bp);
    }
  }
  if (unit_size == 16) {
