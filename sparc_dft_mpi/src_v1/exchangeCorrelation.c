...
void xc_constants_init(XCCST_OBJ *xc_cst, SPARC_OBJ *pSPARC) {
  xc_cst->alpha_zeta2 = 1.0 - 1.0e-6;
  xc_cst->alpha_zeta = 1.0 - 1.0e-6; // ABINIT
  ...
  xc_cst->ec0_aa = 0.031091;
  xc_cst->ec1_aa = 0.015545;
  xc_cst->mac_aa = 0.016887; // ABINIT
  // xc_cst->ec0_aa = 0.0310907; xc_cst->ec1_aa = 0.01554535; xc_cst->mac_aa =
  // 0.0168869; // LIBXC
  xc_cst->ec0_a1 = 0.21370;
  xc_cst->ec1_a1 = 0.20548;
  xc_cst->mac_a1 = 0.11125;
  xc_cst->ec0_b1 = 7.5957;
  xc_cst->ec1_b1 = 14.1189;
  xc_cst->mac_b1 = 10.357;
  xc_cst->ec0_b2 = 3.5876;
  xc_cst->ec1_b2 = 6.1977;
  xc_cst->mac_b2 = 3.6231;
  xc_cst->ec0_b3 = 1.6382;
  xc_cst->ec1_b3 = 3.3662;
  xc_cst->mac_b3 = 0.88026;
  xc_cst->ec0_b4 = 0.49294;
  xc_cst->ec1_b4 = 0.62517;
  xc_cst->mac_b4 = 0.49671;
  ...
  xc_cst->coeff_tt =
      1.0 / (4.0 * 4.0 * xc_cst->piinv * pow(3.0 * M_PI * M_PI, xc_cst->third));
...
void Calculate_Vxc(SPARC_OBJ *pSPARC) {
  ...
  for (i = 0; i < sz_rho; i++) {
    ...
    // for non-linear core correction, use rho+rho_core to evaluate
    // Vxc[rho+rho_core]
    if (pSPARC->NLCC_flag)
...
void Calculate_Vxc_LDA_PW(SPARC_OBJ *pSPARC, double *rho) {
  ...
  int i;
  double p, A, alpha1, beta1, beta2, beta3, beta4, C3,
      C31; // parameters (constants)
  ...
  for (i = 0; i < pSPARC->Nd_d; i++) {
    ...
    rs_pow_p = rs; // rs^p, where p = 1, pow function is slow (~100x slower than
                   // add/sub)
    rs_pow_pplus1 = rs_pow_p * rs; // rs^(p+1)
    G1 = log(1.0 + 1.0 / (2.0 * A *
                          (beta1 * rs_sqrt + beta2 * rs + beta3 * rs_pow_1p5 +
                           beta4 * rs_pow_pplus1)));
    G2 = 2.0 * A *
         (beta1 * rs_sqrt + beta2 * rs + beta3 * rs_pow_1p5 +
          beta4 * rs_pow_pplus1);

    pSPARC->XCPotential[i] =
        -2.0 * A * (1.0 + alpha1 * rs) * G1 -
        (rs / 3.0) *
            (-2.0 * A * alpha1 * G1 +
             (2.0 * A * (1.0 + alpha1 * rs) *
              (A * (beta1 / rs_sqrt + 2.0 * beta2 + 3.0 * beta3 * rs_sqrt +
                    2.0 * (p + 1.0) * beta4 * rs_pow_p))) /
                 (G2 * (G2 + 1.0)));
...
void Calculate_Vxc_LSDA_PW(SPARC_OBJ *pSPARC, XCCST_OBJ *xc_cst, double *rho) {
  ...
  int DMnd, i, spn_i;
  double rho_updn, rho_updnm1_3, rhom1_3, rhotot_inv, zeta, zetp, zetm,
      zetpm1_3, zetmm1_3, rhotmo6, rhoto6;
  double exc, rhomot, ex_lsd, rs, sqr_rs, rsm1_2;
  double ec0_q0, ec0_q1, ec0_q1p, ec0_den, ec0_log, ecrs0, decrs0_drs, mac_q0,
      mac_q1, mac_q1p, mac_den, mac_log, macrs, dmacrs_drs;
  double ec1_q0, ec1_q1, ec1_q1p, ec1_den, ec1_log, ecrs1, decrs1_drs, zetp_1_3,
      zetm_1_3, f_zeta, fp_zeta, zeta4;
  ...
  for (i = 0; i < DMnd; i++) {
    ...
    for (spn_i = 0; spn_i < 2; spn_i++) {
      ...
      rhomot = rho_updnm1_3;
      ex_lsd = -xc_cst->threefourth_divpi * xc_cst->sixpi2_1_3 *
               (rhomot * rhomot * rho_updn);
    ...
    ec0_q1 = 2.0 * xc_cst->ec0_aa *
             (xc_cst->ec0_b1 * sqr_rs + xc_cst->ec0_b2 * rs +
              xc_cst->ec0_b3 * rs * sqr_rs + xc_cst->ec0_b4 * rs * rs);
    ec0_q1p = xc_cst->ec0_aa *
              (xc_cst->ec0_b1 * rsm1_2 + 2.0 * xc_cst->ec0_b2 +
               3.0 * xc_cst->ec0_b3 * sqr_rs + 4.0 * xc_cst->ec0_b4 * rs);
    ec0_den = 1.0 / (ec0_q1 * ec0_q1 + ec0_q1);
    ec0_log = -log(ec0_q1 * ec0_q1 * ec0_den);
    ecrs0 = ec0_q0 * ec0_log;
    decrs0_drs = -2.0 * xc_cst->ec0_aa * xc_cst->ec0_a1 * ec0_log -
                 ec0_q0 * ec0_q1p * ec0_den;

    mac_q0 = -2.0 * xc_cst->mac_aa * (1.0 + xc_cst->mac_a1 * rs);
    mac_q1 = 2.0 * xc_cst->mac_aa *
             (xc_cst->mac_b1 * sqr_rs + xc_cst->mac_b2 * rs +
              xc_cst->mac_b3 * rs * sqr_rs + xc_cst->mac_b4 * rs * rs);
    mac_q1p = xc_cst->mac_aa *
              (xc_cst->mac_b1 * rsm1_2 + 2.0 * xc_cst->mac_b2 +
               3.0 * xc_cst->mac_b3 * sqr_rs + 4.0 * xc_cst->mac_b4 * rs);
    mac_den = 1.0 / (mac_q1 * mac_q1 + mac_q1);
    mac_log = -log(mac_q1 * mac_q1 * mac_den);
    macrs = mac_q0 * mac_log;
    dmacrs_drs = -2.0 * xc_cst->mac_aa * xc_cst->mac_a1 * mac_log -
                 mac_q0 * mac_q1p * mac_den;

    ec1_q0 = -2.0 * xc_cst->ec1_aa * (1.0 + xc_cst->ec1_a1 * rs);
    ec1_q1 = 2.0 * xc_cst->ec1_aa *
             (xc_cst->ec1_b1 * sqr_rs + xc_cst->ec1_b2 * rs +
              xc_cst->ec1_b3 * rs * sqr_rs + xc_cst->ec1_b4 * rs * rs);
    ec1_q1p = xc_cst->ec1_aa *
              (xc_cst->ec1_b1 * rsm1_2 + 2.0 * xc_cst->ec1_b2 +
               3.0 * xc_cst->ec1_b3 * sqr_rs + 4.0 * xc_cst->ec1_b4 * rs);
    ec1_den = 1.0 / (ec1_q1 * ec1_q1 + ec1_q1);
    ec1_log = -log(ec1_q1 * ec1_q1 * ec1_den);
    ecrs1 = ec1_q0 * ec1_log;
    decrs1_drs = -2.0 * xc_cst->ec1_aa * xc_cst->ec1_a1 * ec1_log -
                 ec1_q0 * ec1_q1p * ec1_den;

    // xc_cst->alpha_zeta is introduced in order to remove singularities for
    // fully polarized systems.
    zetp_1_3 = (1.0 + zeta * xc_cst->alpha_zeta) * pow(zetpm1_3, 2.0);
    zetm_1_3 = (1.0 - zeta * xc_cst->alpha_zeta) * pow(zetmm1_3, 2.0);

    f_zeta = ((1.0 + zeta * xc_cst->alpha_zeta2) * zetp_1_3 +
              (1.0 - zeta * xc_cst->alpha_zeta2) * zetm_1_3 - 2.0) *
             xc_cst->factf_zeta;
    ...
    decrs_drs = decrs0_drs +
                f_zeta * (zeta4 * dgcrs_drs - dmacrs_drs * xc_cst->fsec_inv);
...
void Calculate_Vxc_LDA_PZ(SPARC_OBJ *pSPARC, double *rho) {
  ...
  for (i = 0; i < pSPARC->Nd_d; i++) {
    ...
    if (rs < 1.0) {
      Vxci = log(rs) * (A + (2.0 / 3.0) * C * rs) + (B - (1.0 / 3.0) * A) +
             (1.0 / 3.0) * (2.0 * D - C) * rs;
    } else {
      Vxci = (gamma1 + (7.0 / 6.0) * gamma1 * beta1 * pow(rs, 0.5) +
              (4.0 / 3.0) * gamma1 * beta2 * rs) /
             pow(1 + beta1 * pow(rs, 0.5) + beta2 * rs, 2.0);
    }
    //}
  ...
  if (pSPARC->spin_typ == 0) { // spin unpolarized
    if (strcmpi(pSPARC->XC, "GGA_PBE") == 0 ||
      ...
      Calculate_Vxc_GGA_PBE(pSPARC, &xc_cst, rho);
             ...
             strcmpi(pSPARC->XC, "vdWDF2") ==
                 0) { // it can also be replaced by pSPARC->vdWDFFlag != 0
      Calculate_Vxc_vdWExchangeLinearCorre(
          pSPARC, &xc_cst, rho); // compute energy and potential of Zhang-Yang
                                 // revised PBE exchange + LDA PW91 correlation
      Calculate_nonLinearCorr_E_V_vdWDF(
          pSPARC, rho); // the function is in /vdW/vdWDF/vdWDF.c
    } else {
      printf("Cannot recognize the XC option provided!\n");
      exit(EXIT_FAILURE);
    }
    ...
    if (strcmpi(pSPARC->XC, "GGA_PBE") == 0 ||
      ...
      Calculate_Vxc_vdWExchangeLinearCorre(
          pSPARC, &xc_cst, rho); // compute energy and potential of Zhang-Yang
                                 // revised PBE exchange + LDA PW91 correlation
      Calculate_nonLinearCorr_E_V_SvdWDF(
          pSPARC, rho); // the function is in /vdW/vdWDF/vdWDF.c
    } else {
      printf("Cannot recognize the XC option provided!\n");
      exit(EXIT_FAILURE);
...
void Calculate_Vxc_GGA_PBE(SPARC_OBJ *pSPARC, XCCST_OBJ *xc_cst, double *rho) {
  ...
  // double pi, piinv, third, twom1_3, sixpi2_1_3, sixpi2m1_3,
  // threefourth_divpi, gamma, gamma_inv, coeff_tt, sq_rsfac, sq_rsfac_inv;
  double rho_updn, rho_updnm1_3, rhom1_3, rhotot_inv, rhotmo6, rhoto6, rhomot,
      ex_lsd, rho_inv, coeffss, ss;
  double divss, dfxdss, fx, ex_gga, dssdn, dfxdn, dssdg, dfxdg, exc, rs, sqr_rs,
      rsm1_2;
  double ec0_q0, ec0_q1, ec0_q1p, ec0_den, ec0_log, ecrs0, decrs0_drs, ecrs,
      decrs_drs;
  double phi_zeta_inv, phi3_zeta, gamphi3inv, bb, dbb_drs, exp_pbe, cc, dcc_dbb,
      dcc_drs, coeff_aa, aa, daa_drs;
  double grrho2, dtt_dg, tt, xx, dxx_drs, dxx_dtt, pade_den, pade, dpade_dxx,
      dpade_drs, dpade_dtt, coeff_qq, qq, dqq_drs, dqq_dtt;
  double arg_rr, div_rr, rr, drr_dqq, drr_drs, drr_dtt, hh, dhh_dtt, dhh_drs,
      drhohh_drho;
  ...
  double *Drho_x, *Drho_y, *Drho_z, *DDrho_x, *DDrho_y, *DDrho_z, *sigma,
      *lapcT;
  ...
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, rho, Drho_x, 0,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, rho, Drho_y, 1,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, rho, Drho_z, 2,
                       pSPARC->dmcomm_phi);

  if (pSPARC->cell_typ > 10 && pSPARC->cell_typ < 20) {
    lapcT = (double *)malloc(6 * sizeof(double));
    lapcT[0] = pSPARC->lapcT[0];
    lapcT[1] = 2 * pSPARC->lapcT[1];
    lapcT[2] = 2 * pSPARC->lapcT[2];
    lapcT[3] = pSPARC->lapcT[4];
    lapcT[4] = 2 * pSPARC->lapcT[5];
    lapcT[5] = pSPARC->lapcT[8];
    for (i = 0; i < DMnd; i++) {
      sigma[i] = Drho_x[i] * (lapcT[0] * Drho_x[i] + lapcT[1] * Drho_y[i]) +
                 Drho_y[i] * (lapcT[3] * Drho_y[i] + lapcT[4] * Drho_z[i]) +
    ...
    for (i = 0; i < DMnd; i++) {
      sigma[i] =
          Drho_x[i] * Drho_x[i] + Drho_y[i] * Drho_y[i] + Drho_z[i] * Drho_z[i];
    }
  }
  ...
  for (i = 0; i < DMnd; i++) {
    ...
    coeffss = (1.0 / 4.0) * xc_cst->sixpi2m1_3 * xc_cst->sixpi2m1_3 *
              (rho_inv * rho_inv * rhomot * rhomot);
    ...
    if (strcmpi(pSPARC->XC, "GGA_PBE") == 0 ||
      ...
      // d2fxdss2 = -xc_cst->mu * 2.0 * xc_cst->mu_divkappa * (divss * divss *
      // divss);
    ...
    // If non spin-polarized, treat spin down contribution now, similar to spin
    // up
    exc = exc * 2.0;
    pSPARC->e_xc[i] = exc * rhotot_inv;
    ...
    if ((pSPARC->usefock > 0) && (pSPARC->usefock % 2 == 0) &&
      ...
      if (sigma[i] < 1E-14)
        sigma[i] = 1E-14;
      pbexsr(rho[i], sigma[i], pSPARC->hyb_range_pbe, &e_xc_sr, &XCPotential_sr,
             &Dxcdgrho_sr);
    ...
    // WARNING: Dxcdgrho = 0.5 * dvxcdgrho1 here in M-SPARC!! But the same in
    // the end.
    ...
    // Introduce tt : do not assume that the spin-dependent gradients are
    // collinear
    grrho2 = sigma[i];
    ...
    drhohh_drho = hh - xc_cst->third * rs * dhh_drs -
                  (7.0 / 3.0) * tt * dhh_dtt; //- zeta * dhh_dzeta
  ...
  if (pSPARC->cell_typ > 10 && pSPARC->cell_typ < 20) {
    for (i = 0; i < DMnd; i++) {
      temp1 = (Drho_x[i] * pSPARC->lapcT[0] + Drho_y[i] * pSPARC->lapcT[1] +
               Drho_z[i] * pSPARC->lapcT[2]) *
              pSPARC->Dxcdgrho[i];
      temp2 = (Drho_x[i] * pSPARC->lapcT[3] + Drho_y[i] * pSPARC->lapcT[4] +
               Drho_z[i] * pSPARC->lapcT[5]) *
              pSPARC->Dxcdgrho[i];
      temp3 = (Drho_x[i] * pSPARC->lapcT[6] + Drho_y[i] * pSPARC->lapcT[7] +
               Drho_z[i] * pSPARC->lapcT[8]) *
              pSPARC->Dxcdgrho[i];
      Drho_x[i] = temp1;
      Drho_y[i] = temp2;
      Drho_z[i] = temp3;
  ...
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, Drho_x,
                       DDrho_x, 0, pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, Drho_y,
                       DDrho_y, 1, pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 1, 0.0, Drho_z,
                       DDrho_z, 2, pSPARC->dmcomm_phi);
...
void Calculate_Vxc_GSGA_PBE(SPARC_OBJ *pSPARC, XCCST_OBJ *xc_cst, double *rho) {
  ...
  double rho_updn, rho_updnm1_3, rhotmo6, rhoto6, rhomot, ex_lsd, rho_inv,
      coeffss, ss;
  double divss, dfxdss, fx, ex_gga, dssdn, dfxdn, dssdg, dfxdg, exc, rs, sqr_rs,
      rsm1_2;
  double ec0_q0, ec0_q1, ec0_q1p, ec0_den, ec0_log, ecrs0, decrs0_drs, mac_q0,
  ...
  double bb, dbb_drs, dbb_dzeta, exp_pbe, cc, dcc_dbb, dcc_drs, dcc_dzeta,
      coeff_aa, aa, daa_drs, daa_dzeta;
  double grrho2, dtt_dg, tt, xx, dxx_drs, dxx_dzeta, dxx_dtt, pade_den, pade,
      dpade_dxx, dpade_drs, dpade_dtt, dpade_dzeta;
  double coeff_qq, qq, dqq_drs, dqq_dtt, dqq_dzeta, arg_rr, div_rr, rr, drr_dqq,
      drr_drs, drr_dtt, drr_dzeta;
  ...
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 3, 0.0, rho, Drho_x, 0,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 3, 0.0, rho, Drho_y, 1,
                       pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 3, 0.0, rho, Drho_z, 2,
                       pSPARC->dmcomm_phi);
  ...
  for (i = 0; i < DMnd; i++) {
    ...
    for (spn_i = 0; spn_i < 2; spn_i++) {
      ...
      dssdn = (-8.0 / 3.0) * (ss * rho_inv);
      dfxdn = dfxdss * dssdn;
      pSPARC->XCPotential[spn_i * DMnd + i] =
          ex_lsd * ((4.0 / 3.0) * fx + rho_updn * dfxdn);
      ...
      if ((pSPARC->usefock > 0) && (pSPARC->usefock % 2 == 0) &&
        ...
        // Use the same strategy as \rho for \grho here.
        // Without this threshold, numerical issue will make simulation fail.
        if (sigma[DMnd + spn_i * DMnd + i] < 1E-14)
          sigma[DMnd + spn_i * DMnd + i] = 1E-14;
        pbexsr(rho[DMnd + spn_i * DMnd + i] * 2.0,
               sigma[DMnd + spn_i * DMnd + i] * 4.0, pSPARC->hyb_range_pbe,
               &e_xc_sr, &XCPotential_sr, &Dxcdgrho_sr);
        exc -= pSPARC->exx_frac * e_xc_sr / 2.0;
        pSPARC->XCPotential[spn_i * DMnd + i] -=
            pSPARC->exx_frac * XCPotential_sr;
        pSPARC->Dxcdgrho[DMnd + spn_i * DMnd + i] -=
            pSPARC->exx_frac * Dxcdgrho_sr * 2.0;
      }
    }
    pSPARC->e_xc[i] = exc * rhotot_inv;
    ...
    phi_zeta = (zetpm1_3 * (1.0 + zeta * xc_cst->alpha_zeta) +
                zetmm1_3 * (1.0 - zeta * xc_cst->alpha_zeta)) *
               0.5;
    ...
    // From hh to the derivative of the energy wrt the density
    drhohh_drho = hh - xc_cst->third * rs * dhh_drs - zeta * dhh_dzeta -
                  (7.0 / 3.0) * tt * dhh_dtt;
  ...
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 3, 0.0, Drho_x,
                       DDrho_x, 0, pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 3, 0.0, Drho_y,
                       DDrho_y, 1, pSPARC->dmcomm_phi);
  Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, 3, 0.0, Drho_z,
                       DDrho_z, 2, pSPARC->dmcomm_phi);

  for (i = 0; i < DMnd; i++) {
    pSPARC->XCPotential[i] += -DDrho_x[i] - DDrho_y[i] - DDrho_z[i] -
                              DDrho_x[DMnd + i] - DDrho_y[DMnd + i] -
                              DDrho_z[DMnd + i];
    pSPARC->XCPotential[DMnd + i] +=
        -DDrho_x[i] - DDrho_y[i] - DDrho_z[i] - DDrho_x[2 * DMnd + i] -
        DDrho_y[2 * DMnd + i] - DDrho_z[2 * DMnd + i];
  }
...
//                        Vxci =
//                        (-2.0*A*(1.0+alpha1*Vxci))*log(1.0+1.0/(2.0*A*(beta1*pow(Vxci,0.5)
//                        + beta2*Vxci + beta3*pow(Vxci,1.5) +
//                        beta4*pow(Vxci,(p+1.0)))))
//                        -
//                        (Vxci/3.0)*(-2.0*A*alpha1*log(1.0+1.0/(2.0*A*(beta1*pow(Vxci,0.5)
//                        + beta2*Vxci + beta3*pow(Vxci,1.5) +
//                        beta4*pow(Vxci,(p+1.0)))))
//                        - ( (-2.0*A*(1.0+alpha1*Vxci))*(A*(
//                        beta1*pow(Vxci,-0.5)+ 2.0*beta2
//                        + 3.0*beta3*pow(Vxci,0.5)
//                        + 2.0*(p+1.0)*beta4*pow(Vxci,p) )))
//                         /( (2.0*A*( beta1*pow(Vxci,0.5) + beta2*Vxci +
//                         beta3*pow(Vxci,1.5) + beta4*pow(Vxci,(p+1.0)) ) )*
//                            (2.0*A*( beta1*pow(Vxci,0.5) + beta2*Vxci +
//                            beta3*pow(Vxci,1.5) + beta4*pow(Vxci,(p+1.0)) ) )+
//                            (2.0*A*( beta1*pow(Vxci,0.5) + beta2*Vxci +
//                            beta3*pow(Vxci,1.5) + beta4*pow(Vxci,(p+1.0)) ) )
...
//                        ) ;
//                    }
//                    Vxci = Vxci - C3*pow(rhoi,1.0/3.0) ; // add exchange
//                    potential pSPARC->XCPotential[count] = Vxci; count++;
//                }

//    if (rank == 0) {
//        printf("rank = %d, cbrt(0.325649642516) = %f,
//        pow(0.325649642516,1.0/3.0) = %f\n", rank, cbrt(0.325649642516),
//        pow(0.325649642516,1.0/3.0));
//    }
//}
...
void Calculate_Exc_LDA_PW(SPARC_OBJ *pSPARC, double *electronDens) {
  ...
  t3 = MPI_Wtime();
  if (!rank)
    printf("rank = %d, Exc = %18.14f , local calculation time: %.3f ms, "
           "Allreduce time: %.3f ms, Total time: %.3f ms\n",
  ...
  if (pSPARC->spin_typ == 0) { // spin unpolarized
    if (strcmpi(pSPARC->XC, "GGA_PBE") == 0 ||
      ...
      Calculate_Exc_GGA_PBE(pSPARC, electronDens);
               ...
               strcmpi(pSPARC->XC, "vdWDF2") == 0) {
      Calculate_Exc_GGA_vdWDF_ExchangeLinearCorre(
          pSPARC,
          electronDens); // actually the function has no difference from
                         // Calculate_Exc_GGA_PBE. Maybe thery can be unified.
    ...
    if (strcmpi(pSPARC->XC, "GGA_PBE") == 0 ||
      ...
      Calculate_Exc_GSGA_PBE(pSPARC, electronDens);
               ...
               strcmpi(pSPARC->XC, "vdWDF2") == 0) {
      Calculate_Exc_GSGA_vdWDF_ExchangeLinearCorre(
          pSPARC,
          electronDens); // actually the function has no difference from
                         // Calculate_Exc_GSGA_PBE. Maybe thery can be unified.
      Add_Exc_vdWDF(pSPARC); // the function is in /vdW/vdWDF/vdWDF.c
    } else {
      printf("Cannot recognize the XC option provided!\n");
      exit(EXIT_FAILURE);
 ...
 */
void pbexsr(double rho, double grho, double omega, double *e_xc_sr,
            double *XCPotential_sr, double *Dxcdgrho_sr) {
 ...
 *          Taken from Quantum Espresson
 */
void wpbe_analy_erfc_approx_grad(double rho, double s, double omega,
                                 double *Fx_wpbe, double *d1rfx,
                                 double *d1sfx) {
  ...
  double Hsbw, Hsbw2, Hsbw3, Hsbw4, Hsbw12, Hsbw32, Hsbw52, Hsbw72, d1sHsbw,
      d1rHsbw;
  double DHsbw, DHsbw2, DHsbw3, DHsbw4, DHsbw5, DHsbw12, DHsbw32, DHsbw52,
      DHsbw72;
  double DHsbw92, HsbwA94, HsbwA942, HsbwA943, HsbwA945, HsbwA9412, DHs, DHs2,
      DHs3;
  double DHs4, DHs72, DHs92, d1sDHs, DHsw, DHsw2, DHsw52, DHsw72, d1rDHsw;
  double G_a, d1sG_a, G_b, d1sG_b, EG, d1sEG;
  double term2, d1sterm2, term3, d1sterm3, d1rterm3, term4, d1sterm4, d1rterm4,
      term5, d1sterm5, d1rterm5;
  ...
  double d1rf5, f6, f6d1, d1sf6, d1rf6, f7, d1sf7, d1rf7, f8, f8d1, d1sf8,
      d1rf8;
  ...
  if (s > EGscut) {
    G_a = srpi *
              (15 * E + 6 * C * (1 + F * s2) * DHs + 4 * B * (DHs2) +
               8 * A * (DHs3)) *
              (1 / (16 * DHs72)) -
          f34 * M_PI * sqrt(A) * exp(f94 * H * s2 / A) *
              (1 - erf(f32 * s * sqrt(H / A)));
    d1sG_a = (1 / r32) * srpi *
             ((r36 * (2 * H + d1sH * s) / (A12 * sqrt(H / A))) +
              (1 / DHs92) *
                  (-8 * A * d1sDHs * DHs3 - r105 * d1sDHs * E -
                   r30 * C * d1sDHs * DHs * (1 + s2 * F) +
                   r12 * DHs2 * (-B * d1sDHs + C * s * (d1sF * s + 2 * F))) -
              ((r54 * exp(f94 * H * s2 / A) * srpi * s * (2 * H + d1sH * s) *
                erfc(f32 * sqrt(H / A) * s)) /
               A12));
    ...
    EG = -(f34 * M_PI + G_a) / G_b;
    d1sEG =
        (-4 * d1sG_a * G_b + d1sG_b * (4 * G_a + 3 * M_PI)) / (4 * G_b * G_b);
  ...
  term2 = (DHs2 * B + DHs * C + 2 * E + DHs * s2 * C * F + 2 * s2 * EG) /
          (2 * DHs3);

  d1sterm2 =
      (-6 * d1sDHs * (EG * s2 + E) +
       DHs2 * (-d1sDHs * B + s * C * (d1sF * s + 2 * F)) +
       2 * DHs * (2 * EG * s - d1sDHs * C + s2 * (d1sEG - d1sDHs * C * F))) /
      (2 * DHs4);

  term3 = -w *
          (4 * DHsw2 * B + 6 * DHsw * C + 15 * E + 6 * DHsw * s2 * C * F +
           15 * s2 * EG) /
          (8 * DHs * DHsw52);

  d1sterm3 = w *
             (2 * d1sDHs * DHsw *
                  (4 * DHsw2 * B + 6 * DHsw * C + 15 * E +
                   3 * s2 * (5 * EG + 2 * DHsw * C * F)) +
              DHs * (r75 * d1sDHs * (EG * s2 + E) +
                     4 * DHsw2 * (d1sDHs * B - 3 * s * C * (d1sF * s + 2 * F)) -
                     6 * DHsw *
                         (-3 * d1sDHs * C + s * (10 * EG + 5 * d1sEG * s -
                                                 3 * d1sDHs * s * C * F)))) /
             (16 * DHs2 * DHsw72);

  d1rterm3 = (-2 * d1rw * DHsw *
                  (4 * DHsw2 * B + 6 * DHsw * C + 15 * E +
                   3 * s2 * (5 * EG + 2 * DHsw * C * F)) +
              w * d1rDHsw *
                  (r75 * (EG * s2 + E) +
                   2 * DHsw * (2 * DHsw * B + 9 * C + 9 * s2 * C * F))) /
             (16 * DHs * DHsw72);

  term4 = -w3 * (DHsw * C + 5 * E + DHsw * s2 * C * F + 5 * s2 * EG) /
          (2 * DHs2 * DHsw52);

  d1sterm4 =
      (w3 *
       (4 * d1sDHs * DHsw * (DHsw * C + 5 * E + s2 * (5 * EG + DHsw * C * F)) +
        DHs * (r25 * d1sDHs * (EG * s2 + E) -
               2 * DHsw2 * s * C * (d1sF * s + 2 * F) +
               DHsw * (3 * d1sDHs * C + s * (-r20 * EG - 10 * d1sEG * s +
                                             3 * d1sDHs * s * C * F))))) /
      (4 * DHs3 * DHsw72);

  d1rterm4 =
      (w2 *
       (-6 * d1rw * DHsw * (DHsw * C + 5 * E + s2 * (5 * EG + DHsw * C * F)) +
        w * d1rDHsw * (r25 * (EG * s2 + E) + 3 * DHsw * C * (1 + s2 * F)))) /
      (4 * DHs2 * DHsw72);

  term5 = -w5 * (E + s2 * EG) / (DHs3 * DHsw52);

  d1sterm5 = (w5 * (6 * d1sDHs * DHsw * (EG * s2 + E) +
                    DHs * (-2 * DHsw * s * (2 * EG + d1sEG * s) +
                           5 * d1sDHs * (EG * s2 + E)))) /
             (2 * DHs4 * DHsw72);

  d1rterm5 = (w4 * 5 * (EG * s2 + E) * (-2 * d1rw * DHsw + d1rDHsw * w)) /
             (2 * DHs3 * DHsw72);
  ...
  if (HsbwA94 < expfcutoff) {
    ...
    piexperf =
        M_PI * (1.0 / (srpi * HsbwA9412) - 1.0 / (2.0 * sqrt(M_PI * HsbwA943)) +
                3.0 / (4.0 * sqrt(M_PI * HsbwA945)));
    expei = -(1.0 / HsbwA94) * (HsbwA942 + expei1 * HsbwA94 + expei2) /
            (HsbwA942 + expei3 * HsbwA94 + expei4);
  }

  // Calculate the derivatives (based on the orig. expression)
  piexperfd1 = -(3.0 * srpi * sqrt(Hsbw / A)) / (2.0 * Hsbw) +
               (9.0 * piexperf) / (4.0 * A);
  ...
  if (w == 0) {
    ...
    *d1rfx = X * (d1rterm1 + d1rterm3 + d1rterm4 + d1rterm5);
    ...
    np1 = -f32 * ea1 * A12 * w + r27 * ea3 * w3 / (8 * A12) -
    ...
    np2 = -A + f94 * ea2 * w2 - r81 * ea4 * w4 / (16 * A) +
    ...
    f6 = ea5 *
         (3 * srpi * (3 * DHsbw52 * (9 * Hsbw - 2 * A) + 4 * Hsbw32 * A2)) /
         (r32 * DHsbw52 * Hsbw32 * A);
    f6d1 = ea5 * srpi *
           ((r27 / (r32 * Hsbw52)) - (r81 / (r64 * Hsbw32 * A)) -
            ((15 * A) / (16 * DHsbw72)));
    d1sf6 = d1sHsbw * f6d1;
    d1rf6 = d1rHsbw * f6d1;

    f7 = ea6 * (((r32 * A) / DHsbw3 + (-r36 + (r81 * s2 * H) / A) / Hsbw2)) /
         r32;
    d1sf7 = ea6 *
            (3 * (r27 * d1sH * DHsbw4 * Hsbw * s2 +
                  8 * d1sHsbw * A * (3 * DHsbw4 - 4 * Hsbw3 * A) +
                  r54 * DHsbw4 * s * (Hsbw - d1sHsbw * s) * H)) /
            (r32 * DHsbw4 * Hsbw3 * A);
    d1rf7 = ea6 * d1rHsbw *
            ((f94 / Hsbw3) - ((3 * A) / DHsbw4) -
             ((r81 * s2 * H) / (16 * Hsbw3 * A)));

    f8 = ea7 *
         (-3 * srpi *
          (-r40 * Hsbw52 * A3 +
           9 * DHsbw72 * (r27 * Hsbw2 - 6 * Hsbw * A + 4 * A2))) /
         (r128 * DHsbw72 * Hsbw52 * A2);
    f8d1 = ea7 * srpi *
           ((r135 / (r64 * Hsbw72)) + (r729 / (r256 * Hsbw32 * A2)) -
    ...
    f9d1 = -((r81 * ea6 * eb1) / (16 * Hsbw3 * A)) +
           ea8 * ((r27 / (4 * Hsbw4)) + (r729 / (r128 * Hsbw2 * A2)) -
                  (r81 / (16 * Hsbw3 * A)) - ((r12 * A / DHsbw5)));
    d1sf9 = d1sHsbw * f9d1;
    d1rf9 = d1rHsbw * f9d1;

    t2t9 = f2 * w + f3 * w2 + f4 * w3 + f5 * w4 + f6 * w5 + f7 * w6 + f8 * w7 +
           f9 * w8;
    d1st2t9 = d1sf2 * w + d1sf3 * w2 + d1sf4 * w3 + d1sf5 * w4 + d1sf6 * w5 +
              d1sf7 * w6 + d1sf8 * w7 + d1sf9 * w8;
    d1rt2t9 = d1rw * f2 + d1rf2 * w + 2 * d1rw * f3 * w + d1rf3 * w2 +
              3 * d1rw * f4 * w2 + d1rf4 * w3 + 4 * d1rw * f5 * w3 +
              d1rf5 * w4 + 5 * d1rw * f6 * w4 + d1rf6 * w5 +
              6 * d1rw * f7 * w5 + d1rf7 * w6 + 7 * d1rw * f8 * w6 +
...
void Calculate_xc_energy_density(SPARC_OBJ *pSPARC, double *ExcRho) {
  ...
  Exc *= pSPARC->dV;
  if (!rank)
    printf("Exchange correlation energy (without hybrid) from Exc energy "
           "density: %f\n",
           Exc);
#endif
}
